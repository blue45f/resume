...
const mockPrisma: Record<string, any> = {
  ...
  resume: { update: jest.fn(), findUnique: jest.fn() },
...
function mockOwnershipPass(userId: string | null = null) {
  mockPrisma.resume.findUnique.mockResolvedValue({ userId });
}
...
describe('VersionsService', () => {
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers: [VersionsService, { provide: PrismaService, useValue: mockPrisma }],
    }).compile();
  ...
  describe('findAll', () => {
    ...
    it('다른 사용자의 이력서 버전 조회 → ForbiddenException', async () => {
      mockPrisma.resume.findUnique.mockResolvedValue({ userId: 'owner-1' });
      await expect(service.findAll('r1', 'other-user')).rejects.toThrow(ForbiddenException);
    });
  ...
  describe('findOne', () => {
    ...
    it('없는 버전 → NotFoundException', async () => {
      ...
      await expect(service.findOne('r1', 'fake')).rejects.toThrow(NotFoundException);
    });
  });
  ...
  describe('restore', () => {
    it('없는 버전 복원 → NotFoundException', async () => {
      mockPrisma.resumeVersion.findFirst.mockResolvedValue(null);
      await expect(service.restore('r1', 'fake')).rejects.toThrow(NotFoundException);
    });
    ...
    it('다른 사용자의 버전 복원 → ForbiddenException', async () => {
      ...
      await expect(service.restore('r1', 'v1', 'other-user')).rejects.toThrow(ForbiddenException);
    });
    ...
    it('손상된 snapshot JSON → BadRequestException', async () => {
      ...
      await expect(service.restore('r1', 'v1')).rejects.toThrow(BadRequestException);
    });
