...
describe('AuthGuard', () => {
  ...
  it('Cookie에서 토큰 추출', () => {
    ...
    expect(guard.canActivate(ctx)).toBe(true);
    expect((ctx.switchToHttp().getRequest() as any).user).toEqual({ id: 'cookie-user', role: 'user' });
  });
  ...
  it('Authorization 헤더가 우선, Cookie는 fallback', () => {
    ...
    expect((ctx.switchToHttp().getRequest() as any).user).toEqual({ id: 'header-user', role: 'admin' });
  });
});
