...
describe('TagsService', () => {
  ...
  describe('findAll', () => {
    it('태그 목록 + resumeCount 반환', async () => {
      ...
      expect(result[0]).toEqual({ id: 't1', name: '개발', color: '#3b82f6', resumeCount: 5 });
      expect(result[1]).toEqual({ id: 't2', name: '디자인', color: '#ef4444', resumeCount: 0 });
    });
    ...
    it('_count.resumes를 resumeCount로 변환', async () => {
      mockPrisma.tag.findMany.mockResolvedValue([
        { id: 't1', name: 'React', color: null, _count: { resumes: 42 } },
      ]);
      const result = await service.findAll();
  ...
  describe('create', () => {
    ...
    it('중복 이름 → ConflictException', async () => {
      ...
      await expect(service.create({ name: '개발' })).rejects.toThrow(ConflictException);
      await expect(service.create({ name: '개발' })).rejects.toThrow('이미 존재하는 태그입니다');
      expect(mockPrisma.tag.create).not.toHaveBeenCalled();
    });

    it('color 포함 생성', async () => {
      mockPrisma.tag.findUnique.mockResolvedValue(null);
      mockPrisma.tag.create.mockResolvedValue({ id: 't2', name: 'BE', color: '#10b981' });
      const result = await service.create({ name: 'BE', color: '#10b981' });
      expect(result.color).toBe('#10b981');
    });

    it('userId 포함 생성', async () => {
      mockPrisma.tag.findUnique.mockResolvedValue(null);
      mockPrisma.tag.create.mockResolvedValue({ id: 't3', name: '커스텀', color: null });
      await service.create({ name: '커스텀' }, 'user-1');
      expect(mockPrisma.tag.create).toHaveBeenCalledWith({
        data: { name: '커스텀', userId: 'user-1' },
      });
    });

    it('userId 없으면 null로 저장', async () => {
      mockPrisma.tag.findUnique.mockResolvedValue(null);
      mockPrisma.tag.create.mockResolvedValue({ id: 't4', name: '공용' });
      await service.create({ name: '공용' });
      expect(mockPrisma.tag.create).toHaveBeenCalledWith({
        data: { name: '공용', userId: null },
      });
    });

    it('name으로 unique 체크', async () => {
      mockPrisma.tag.findUnique.mockResolvedValue(null);
      mockPrisma.tag.create.mockResolvedValue({ id: 't5', name: 'DevOps' });
      await service.create({ name: 'DevOps' });
      expect(mockPrisma.tag.findUnique).toHaveBeenCalledWith({ where: { name: 'DevOps' } });
    });
  });
  ...
  describe('remove', () => {
    it('태그 삭제 성공 (userId 없는 공용 태그)', async () => {
      mockPrisma.tag.findUnique.mockResolvedValue({ id: 't1', userId: null });
    ...
    it('본인이 만든 태그 삭제 성공', async () => {
      mockPrisma.tag.findUnique.mockResolvedValue({ id: 't1', userId: 'user-1' });
      mockPrisma.tag.delete.mockResolvedValue({});
    ...
    it('다른 사용자의 태그 삭제 → ForbiddenException', async () => {
      mockPrisma.tag.findUnique.mockResolvedValue({ id: 't1', userId: 'user-1' });
      await expect(service.remove('t1', 'user-2')).rejects.toThrow(ForbiddenException);
      await expect(service.remove('t1', 'user-2')).rejects.toThrow(
        '이 태그를 삭제할 권한이 없습니다',
      );
  ...
  describe('removeTagFromResume', () => {
    it('이력서에서 태그 제거', async () => {
      ...
      expect(mockPrisma.tagsOnResumes.delete).toHaveBeenCalledWith({
        where: { resumeId_tagId: { resumeId: 'r1', tagId: 't1' } },
      });
    });
  });
});
