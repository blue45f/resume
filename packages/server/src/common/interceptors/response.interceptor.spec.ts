...
describe('ResponseInterceptor', () => {
  ...
  it('GET 요청은 래핑하지 않음', (done) => {
    interceptor
      .intercept(createContext('GET') as any, createHandler({ id: 1 }) as any)
      .subscribe((val) => {
  ...
  it('POST 응답은 { success, data }로 래핑', (done) => {
    interceptor
      .intercept(createContext('POST') as any, createHandler({ id: 1 }) as any)
      .subscribe((val) => {
  ...
  it('이미 success 필드가 있으면 래핑하지 않음', (done) => {
    interceptor
      .intercept(createContext('POST') as any, createHandler({ success: true }) as any)
      .subscribe((val) => {
