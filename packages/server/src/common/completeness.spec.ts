/**
 * Tests for resume completeness calculation logic.
 * The function is re-implemented here to avoid importing from src/ (import.meta.env).
 * Logic mirrors src/lib/completeness.ts exactly.
 */

// ---- Type definitions (from src/types/resume.ts) ----
...
// ---- Re-implementation of calculateCompleteness (mirrors src/lib/completeness.ts) ----
...
function calculateCompleteness(resume: Resume): CompletenessResult {
...
function createFullResume(): Resume {
  return {
    id: '1',
    title: 'Full Resume',
    createdAt: '2025-01-01',
    ...
    experiences: [
      {
        ...
        endDate: '2025-01',
        current: false,
        description: '대규모 서비스 아키텍처 설계 및 개발을 담당하였으며 팀을 리딩했습니다.',
      ...
      },
      {
        ...
        endDate: '2020-01',
        current: false,
        description: '웹 애플리케이션 프론트엔드 개발 및 API 연동 업무를 수행했습니다.',
      ...
      },
      {
        ...
        endDate: '2017-12',
        current: false,
        description: 'QA 자동화 스크립트 작성 및 테스트 업무를 수행했습니다.',
      },
    ],
    educations: [
      {
    ...
    projects: [
      {
        ...
        startDate: '2024-01',
        endDate: '2025-01',
        description: 'NestJS와 React를 사용한 이력서 관리 플랫폼을 설계하고 개발했습니다.',
      ...
      },
      {
        ...
        startDate: '2023-01',
        endDate: '2023-12',
        description: 'WebSocket 기반 실시간 채팅 서비스를 개발했습니다.',
        link: '',
      },
    ],
...
describe('calculateCompleteness', () => {
  ...
  it('short summary contributes 4 points, long summary contributes 8', () => {
    ...
    expect(piScore).toBe(4); // short summary only

    resume.personalInfo.summary =
      'This is a summary that is definitely longer than thirty characters in total.';
    result = calculateCompleteness(resume);
    piScore = result.sections.find((s) => s.label === '인적사항')!.score;
    expect(piScore).toBe(8); // long summary
  });
  ...
  it('three skills with 3+ items each gives full 15 points', () => {
    ...
    expect(skillSection.score).toBe(15); // 6+4+3+2
  });
});
