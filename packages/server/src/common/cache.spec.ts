...
describe('cache utilities', () => {
  ...
  describe('getCached', () => {
    ...
    it('returns the stored value within the TTL', () => {
      ...
      const result = getCached<{ foo: string }>('key1', 60_000);
      expect(result).toEqual({ foo: 'bar' });
    });

    it('returns null and evicts entry when expired', () => {
      // Manually insert an entry with a past timestamp
      cache.set('old-key', { data: 'stale', timestamp: Date.now() - 120_000 });
    ...
    it('returns value right at the boundary (not yet expired)', () => {
      const now = Date.now();
      cache.set('edge-key', { data: 'edge', timestamp: now });
  ...
  describe('setCache', () => {
    it('stores a string value', () => {
      setCache('str', 'hello');
      expect(getCached<string>('str', 60_000)).toBe('hello');
    });

    it('stores a number value', () => {
      setCache('num', 42);
      expect(getCached<number>('num', 60_000)).toBe(42);
    });

    it('stores an object value', () => {
      setCache('obj', { a: 1, b: [2, 3] });
      expect(getCached('obj', 60_000)).toEqual({ a: 1, b: [2, 3] });
    });
  ...
  describe('clearCache', () => {
    ...
    it('clears only entries matching the prefix', () => {
      ...
      expect(getCached<string>('post:1', 60_000)).toBe('hello');
    });

    it('does nothing if prefix matches no keys', () => {
      setCache('a', 1);
      clearCache('zzz:');
      expect(getCached<number>('a', 60_000)).toBe(1);
    });
  });
  ...
  describe('expiry behavior', () => {
    it('entry is still valid just before expiry', () => {
      ...
      cache.set('recent', { data: 'ok', timestamp: Date.now() - 3000 });
    ...
    it('entry is expired just after maxAge', () => {
      ...
      cache.set('expired', { data: 'gone', timestamp: Date.now() - 6000 });
    ...
    it('zero maxAge always expires', () => {
      ...
      cache.set('instant', { data: 'value', timestamp: Date.now() - 1 });
