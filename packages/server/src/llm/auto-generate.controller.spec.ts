...
import type { AutoGenerateDto } from './dto/auto-generate.dto';
...
describe('AutoGenerateController', () => {
  ...
  describe('preview', () => {
    it('autoGenerate 호출 결과 그대로 반환 (저장 안 함)', async () => {
      ...
      const res = await controller.preview({ rawText: '...' } as AutoGenerateDto);
      expect(res).toEqual({
        resume: { title: 'T' },
        tokensUsed: 100,
        provider: 'groq',
    ...
    it('instruction 전달', async () => {
      mockLlm.autoGenerate.mockResolvedValueOnce({});
      await controller.preview({ rawText: 'raw', instruction: '짧게' } as AutoGenerateDto);
  ...
  describe('create', () => {
    it('LLM 생성 + DB 저장 + 응답에 메타 포함', async () => {
      ...
      const res = await controller.create({ rawText: 'raw' } as AutoGenerateDto, reqWith('u1'));
    ...
    it('비로그인도 create 시 userId=undefined 로 저장 시도 (service 에서 판정)', async () => {
      ...
      await controller.create({ rawText: 'raw' } as AutoGenerateDto, reqWith());
  ...
  describe('파일 업로드', () => {
    it('preview: file 첨부 시 fileExtractor.extract 결과를 rawText 로 사용', async () => {
      ...
      await controller.preview({ rawText: '' } as AutoGenerateDto, fakeFile);
    ...
    it('preview: file 없을 때 dto.rawText 그대로 사용 (기존 흐름)', async () => {
      mockLlm.autoGenerate.mockResolvedValueOnce({ resume: {}, tokensUsed: 0 });
      await controller.preview({ rawText: '직접 입력' } as AutoGenerateDto, undefined);
