...
describe('HealthController', () => {
  ...
  describe('ready (GET /health/ready)', () => {
    ...
    it('DB 정상 + LLM 키 있음 → ok / 200', async () => {
      ...
      mockConfig.get.mockImplementation((k: string) => (k === 'GEMINI_API_KEY' ? 'key' : undefined))
      const res = makeRes()
  ...
  describe('adminStats (GET /health/admin/stats)', () => {
    ...
    it('일반 사용자 → ForbiddenException', async () => {
      await expect(controller.adminStats({ user: { id: 'u1', role: 'user' } })).rejects.toThrow(
        ForbiddenException
      )
    ...
    it('비로그인 → UnauthorizedException', async () => {
      await expect(controller.adminStats({ user: null })).rejects.toThrow(UnauthorizedException)
    })

    it('user.id 없음 → UnauthorizedException', async () => {
      await expect(controller.adminStats({ user: {} })).rejects.toThrow(UnauthorizedException)
    })
  })
  ...
  describe('getUsage (GET /health/usage)', () => {
    ...
    it('비로그인 → UnauthorizedException', async () => {
      await expect(controller.getUsage({ user: null })).rejects.toThrow(UnauthorizedException)
    })
  })
  ...
  describe('adminStats 인증 요구 상세', () => {
    it('req 객체에 user 프로퍼티 자체가 없음 → UnauthorizedException', async () => {
      await expect(controller.adminStats({})).rejects.toThrow(UnauthorizedException)
    })

    it('user.role이 undefined → ForbiddenException', async () => {
      await expect(controller.adminStats({ user: { id: 'u1', role: undefined } })).rejects.toThrow(
        ForbiddenException
      )
    })

    it('user.role이 빈 문자열 → ForbiddenException', async () => {
      await expect(controller.adminStats({ user: { id: 'u1', role: '' } })).rejects.toThrow(
        ForbiddenException
      )
    })
    ...
    it('서비스 에러 → 그대로 전파됨', async () => {
      ...
      await expect(controller.adminStats({ user: { id: 'a1', role: 'admin' } })).rejects.toThrow(
        'DB error'
      )
    })
  })
