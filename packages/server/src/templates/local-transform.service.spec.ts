...
const mockResume = {
  ...
  skills: [{ category: '프로그래밍', items: 'TypeScript, React, Node.js' }],
  projects: [
    {
      name: '이력서 플랫폼',
      role: '리드 개발자',
  ...
  languages: [{ name: '영어', testName: 'TOEIC', score: '950', testDate: '2023-01' }],
  awards: [{ name: '우수상', issuer: '해커톤', awardDate: '2023-11', description: '1등' }],
  activities: [
    {
      name: '오픈소스 기여',
      organization: 'GitHub',
...
describe('LocalTransformService', () => {
  let service: LocalTransformService;

  beforeEach(() => {
    service = new LocalTransformService();
  });
  ...
  describe('빈 데이터 처리', () => {
    ...
    it('personalInfo만 있는 경우', () => {
      const result = service.transform(
        ...
        { sections: ['personalInfo', 'experiences'] },
      );
