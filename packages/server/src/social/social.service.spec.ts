...
describe('SocialService', () => {
      ...
      mockPrisma.scoutMessage.findMany.mockResolvedValue([
        {
          id: 'sc-1',
          company: '네이버',
          sender: { id: 's1', name: '김리크루터', email: 'r@t.com' },
        },
      ]);
  ...
  describe('sendMessage', () => {
    ...
    it('자신에게 메시지 → ForbiddenException', async () => {
      await expect(service.sendMessage('user-1', 'user-1', '테스트')).rejects.toThrow(
        ForbiddenException,
      );
      await expect(service.sendMessage('user-1', 'user-1', '테스트')).rejects.toThrow(
        '자신에게 메시지를 보낼 수 없습니다',
    ...
    it('빈 메시지 → ForbiddenException', async () => {
      ...
      await expect(service.sendMessage('user-1', 'user-2', '')).rejects.toThrow(
        '메시지를 입력해주세요',
      );
    });
