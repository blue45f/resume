  ...
  async findAll(filters: StudyGroupListFilters) {
    ...
    if (filters.openOnly) {
      const existingMemberCount =
        typeof where.memberCount === 'object' && where.memberCount !== null ? where.memberCount : {}
      where.memberCount = {
        ...existingMemberCount,
  ...
  private normalizeAttachments(
    ...
    for (const item of raw) {
      ...
      if (url.startsWith('data:')) {
        const prefixOk = StudyGroupsService.ALLOWED_DATA_URL_PREFIXES.some((p) => url.startsWith(p))
