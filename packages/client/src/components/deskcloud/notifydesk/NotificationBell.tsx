 ...
 * ──────────────────────────────────────────────────────────────────────────
 * npm publish 가 막힌 동안 형제 앱(resume·offhours·…)에 그대로 복붙해서 쓰는 버전입니다.
 * 워크스페이스 의존(@notifydesk/shared·@notifydesk/widget) 0 — 필요한 클라이언트·아이콘·
 * 상대시간·스타일을 이 파일에 인라인했습니다. 동작/디자인은 @notifydesk/widget 의
 * <NotificationBell> 과 동일합니다.
 *
 * 사용:
 *   import { NotificationBell } from './NotificationBell'
 *   <NotificationBell recipientId="user_42" publishableKey="pk_…" endpoint="https://notify.example.com" />
 *
 * 백엔드 계약(퍼블리시 키 · Bearer 헤더 + Origin 검사):
 ...
 *
 * 접근성/디자인: 포커스 트랩 · Esc · 바깥클릭 닫기 · focus-visible ·
 * prefers-reduced-motion · 대비 ≥4.5:1 · 외부 CSS 프레임워크 0(스코프 .nd-* 스타일).
 * ──────────────────────────────────────────────────────────────────────────
 */
import {
  useCallback,
...
/** "방금 전" · "3분 전" · "2시간 전" · "3일 전" … 그 이상은 짧은 절대 날짜. */
