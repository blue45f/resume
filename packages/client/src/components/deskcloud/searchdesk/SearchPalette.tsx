...
function ResultsList(props: ResultsListProps): ReactElement {
  ...
  // 그룹별 시작 오프셋을 미리 계산해 렌더 중 변수 재할당(react-compiler 경고)을 피한다.
  const groupBases: number[] = []
  groups.reduce((acc, group) => {
    groupBases.push(acc)
    return acc + group.hits.length
  }, 0)
  return (
        ...
        <li key={group.category} role="presentation">
          ...
          <ul role="presentation" style={{ listStyle: 'none', margin: 0, padding: 0 }}>
            {group.hits.map((hit, hitIdx) => {
              const index = groupBases[groupIdx]! + hitIdx
...
export function SearchPalette(props: SearchPaletteProps): ReactElement | null {
  ...
  return (
    <div className="sk-root" style={rootStyle}>
      ...
      >
        {/* eslint-disable-next-line jsx-a11y/no-noninteractive-element-interactions -- dialog 컨테이너의 키보드(Esc/Arrow) 처리 */}
        <div
          ref={dialogRef}
          className="sk-dialog"
          role="dialog"
