...
function Rating({
  ...
  return (
    // 별점 그룹은 onKeyDown(화살표)로 키보드 조작하고, 내부 별이 roving-focus radio 역할을 한다.
    // eslint-disable-next-line jsx-a11y/interactive-supports-focus -- radiogroup operated via onKeyDown; focus lives on the star radios
    <div
      className="sd-stars"
      role="radiogroup"
...
export function FeedbackWidget(props: FeedbackWidgetProps): ReactElement | null {
  ...
  return (
    <div className="sd-root" style={rootStyle}>
      ...
      {open ? (
        // 백드롭 스크림(클릭으로 닫기); 키보드 사용자는 다이얼로그의 Escape 핸들러로 닫는다.
        // eslint-disable-next-line jsx-a11y/no-static-element-interactions -- scrim; keyboard close via dialog Escape handler
        <div
