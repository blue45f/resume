...
export function SkillSuggestDropdown({
  currentItems,
  onAdd,
}: {
  currentItems: string;
...
export function CompanyRoleSuggest({
  company,
  onSelect,
}: {
  company: string;
  ...
  return (
    <div className="mt-1 p-2 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg animate-fade-in w-full overflow-hidden">
      <div className="flex items-start justify-between gap-2 mb-1">
        <p className="text-xs text-blue-700 dark:text-blue-300 font-medium flex-1 min-w-0 truncate">
          {company}의 일반적인 직위:
        </p>
        <button
          onClick={() => setDismissed(true)}
...
export function InlineContentTip({
  text,
  section,
}: {
  text: string;
      ...
      <div className="flex items-start justify-between gap-2">
        <div className="flex-1">
          ...
          <p className="text-xs text-amber-600 dark:text-amber-400 mb-1.5">
            STAR 기법으로 작성하면 더 효과적입니다:
          </p>
          <pre className="text-[11px] text-amber-700 dark:text-amber-300 bg-amber-100/50 dark:bg-amber-900/30 p-2 rounded whitespace-pre-wrap font-sans leading-relaxed">
            {tip.template}
          </pre>
          {/* Mini score */}
          <div className="flex items-center gap-2 mt-2">
            ...
            <div className="flex-1 h-1 bg-amber-200 dark:bg-amber-800 rounded-full max-w-[80px]">
              <div
                ...
                style={{ width: `${Math.min(Math.round((plain.length / 50) * 100), 100)}%` }}
              />
            </div>
            <span className="text-[10px] font-medium text-amber-700 dark:text-amber-300">
              {Math.min(Math.round((plain.length / 50) * 100), 100)}/100
            </span>
          </div>
        </div>
        <button
