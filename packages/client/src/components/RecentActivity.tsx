...
export default function RecentActivity() {
  ...
  return (
    <div className="mb-6">
      ...
      >
        <span>최근 활동</span>
        <span className="text-xs bg-slate-200 dark:bg-slate-700 text-slate-500 dark:text-slate-400 rounded-full px-1.5 py-0.5">
          {activities.length}
        </span>
        <svg
                  ...
                  return (
                    ...
                    >
                      {isSocial && act.actorName ? (
                        <span
                          className={`flex-shrink-0 w-7 h-7 flex items-center justify-center rounded-full text-white text-xs font-bold ${getAvatarBg(act.actorName)}`}
                        >
