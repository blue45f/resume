...
export function analyzeAtsCompatibility(resume: Resume): AtsResult {
  ...
  if (!pi.email) {
    issues.push({
      ...
      section: '인적사항',
      message: '이메일 누락',
      tip: '채용 담당자가 연락할 수 없습니다. 이메일을 반드시 입력하세요.',
    });
    checklist.push({
      id: 'email',
      category: '인적사항',
  ...
  if (!pi.name) {
    issues.push({
      ...
      section: '인적사항',
      message: '이름 누락',
      tip: 'ATS는 이름이 없으면 이력서를 처리할 수 없습니다.',
    });
    checklist.push({
      id: 'name',
      category: '인적사항',
  ...
  if (summaryText.length < 50) {
    issues.push({
      severity: 'warning',
      section: '자기소개',
      message: '자기소개가 너무 짧습니다',
      tip: '50자 이상의 자기소개를 작성하면 ATS 통과율이 높아집니다. 핵심 역량과 경력 요약을 포함하세요.',
    });
    checklist.push({
      id: 'summary-length',
      category: '자기소개',
      ...
      passed: false,
      severity: 'warning',
      detail: `현재 ${summaryText.length}자`,
    });
    score -= 8;
  } else {
    passed.push('자기소개 충분');
  ...
  if (resume.experiences.length === 0) {
    issues.push({
      severity: 'error',
      section: '경력',
      message: '경력 섹션이 비어있습니다',
      tip: '최소 1개 이상의 경력을 입력하세요. 신입이라면 인턴/아르바이트도 포함하세요.',
    });
    checklist.push({
      id: 'exp-exists',
      category: '경력',
    ...
    for (const exp of resume.experiences) {
      if (!exp.startDate) {
        issues.push({
          severity: 'warning',
          section: '경력',
          message: `"${exp.company}" 시작일 누락`,
          tip: 'ATS는 근무 기간을 파싱합니다. 날짜를 입력하세요.',
        });
      ...
      if (descText.length < 30) {
        issues.push({
          severity: 'warning',
          section: '경력',
          message: `"${exp.company}" 업무 설명이 부족합니다`,
          tip: '구체적인 성과와 수치를 포함한 설명을 작성하세요. (예: "매출 20% 증가 기여")',
        });
  ...
  if (resume.educations.length === 0) {
    issues.push({
      ...
      section: '학력',
      message: '학력 정보 없음',
      tip: '학력 정보는 대부분의 ATS에서 필수 필드입니다.',
    });
    checklist.push({
      id: 'edu',
      category: '학력',
  ...
  if (resume.skills.length === 0) {
    issues.push({
      ...
      section: '기술',
      message: '기술 스택 누락',
      tip: 'ATS는 키워드 매칭으로 이력서를 필터링합니다. 기술 스택을 반드시 입력하세요.',
    });
    checklist.push({
      id: 'skills',
      category: '기술',
    ...
    const totalSkills = resume.skills.reduce(
      (sum, s) => sum + s.items.split(',').filter((i) => i.trim()).length,
      0,
    );
    if (totalSkills < 5) {
      issues.push({
        severity: 'warning',
        section: '기술',
        message: `기술 키워드가 ${totalSkills}개로 적습니다`,
        tip: '최소 5-10개의 기술 키워드를 입력하면 ATS 매칭률이 높아집니다.',
      });
      checklist.push({
        id: 'skills',
        category: '기술',
  ...
  checklist.push({
    ...
    passed: !hasTable,
    severity: 'warning',
    detail: hasTable ? 'ATS가 표를 파싱하지 못할 수 있습니다' : undefined,
  });
  if (hasTable) {
    issues.push({
      severity: 'warning',
      section: '형식',
      message: '표(table) 사용 감지',
      tip: 'ATS는 표를 제대로 파싱하지 못합니다. 텍스트 기반으로 변경하세요.',
    });
    score -= 5;
  }
  ...
  // Images check
  const hasImages =
    allText.includes('<img') || allText.includes('<IMG') || (pi.photo && pi.photo.length > 0);
  checklist.push({
    id: 'no-images',
    category: '형식',
    label: '이미지 최소화',
    passed: !hasImages,
    severity: 'info',
    detail: hasImages ? '이미지가 포함되어 있습니다' : undefined,
  });
  if (hasImages && !pi.photo) {
    issues.push({
      ...
      section: '형식',
      message: '본문에 이미지 사용',
      tip: 'ATS는 이미지 속 텍스트를 읽지 못합니다. 중요한 정보는 텍스트로 작성하세요.',
    });
    score -= 3;
  }
  ...
  checklist.push({
    ...
    severity: 'warning',
    detail: hasSpecialChars ? '일부 ATS에서 인식하지 못하는 특수문자가 있습니다' : undefined,
  });
  if (hasSpecialChars) {
    issues.push({
      severity: 'warning',
      section: '형식',
      message: '특수문자 사용 감지',
      tip: '일부 ATS는 특수문자(", ", -, -, 등)를 인식하지 못합니다. 표준 문자를 사용하세요.',
    });
    score -= 3;
  }

  // Header/footer content check (checking for content in header-like positions - name/summary)
  const nameHasExtraInfo =
    pi.name && (pi.name.includes('|') || pi.name.includes('/') || pi.name.length > 30);
  checklist.push({
    id: 'clean-header',
    category: '형식',
    label: '깔끔한 헤더 (이름만)',
    passed: !nameHasExtraInfo,
    severity: 'info',
    detail: nameHasExtraInfo ? '이름 필드에 추가 정보가 포함되어 있습니다' : undefined,
  });
  if (nameHasExtraInfo) {
    issues.push({
      severity: 'info',
      section: '형식',
      message: '이름 필드에 불필요한 정보 포함',
      tip: '이름 필드에는 이름만 입력하세요. 추가 정보는 적절한 섹션에 넣으세요.',
    });
    score -= 2;
  }
  ...
  checklist.push({
    ...
    severity: 'warning',
    detail: datesConsistent ? undefined : '날짜 형식이 일관되지 않습니다',
  });
  if (!datesConsistent) {
    issues.push({
      severity: 'warning',
      section: '형식',
      message: '날짜 형식이 일관되지 않음',
      tip: 'ATS는 일관된 날짜 형식을 선호합니다. YYYY-MM 또는 YYYY.MM 형식으로 통일하세요.',
    });
    score -= 4;
  }
  ...
  if (sectionCount < 3) {
    issues.push({
      severity: 'warning',
      section: '구조',
      message: `이력서 섹션이 ${sectionCount}개로 부족합니다`,
      tip: 'ATS는 경력, 학력, 기술 등 표준 섹션을 기대합니다. 최소 3개 이상의 섹션을 작성하세요.',
    });
    score -= 5;
  }
  ...
  if (textLength < 500) {
    issues.push({
      severity: 'warning',
      section: '전체',
      message: '이력서 내용이 너무 짧습니다',
      tip: '충분한 정보를 제공해야 ATS가 키워드를 매칭할 수 있습니다.',
    });
    checklist.push({
      id: 'length',
      category: '전체',
  ...
  checklist.push({
    ...
    passed: hasNumbers,
    severity: 'info',
    detail: hasNumbers ? undefined : '성과를 수치로 표현하면 더 효과적입니다',
  });
  if (!hasNumbers && resume.experiences.length > 0) {
    issues.push({
      severity: 'info',
      section: '내용',
      message: '수치화된 성과가 없습니다',
      tip: '성과를 수치로 표현하면 ATS 점수와 인사 담당자의 관심도가 높아집니다. (예: "매출 20% 증가", "사용자 1만명 확보")',
    });
    score -= 3;
  }
