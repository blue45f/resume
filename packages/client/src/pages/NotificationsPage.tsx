  ...
  return (
    <>
      ...
      >
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center gap-2">
            {unreadCount > 0 && !selectMode && (
              <button
                onClick={markAllRead}
                className="text-xs text-sky-700 dark:text-sky-400 hover:text-sky-800 font-medium px-3 py-1.5 rounded-lg hover:bg-sky-50 dark:hover:bg-sky-900/20 transition-colors"
              >
                모두 읽음
              </button>
            )}
        ...
        {/* Bulk actions bar */}
        {selectMode && (
          <div className="flex items-center gap-2 mb-4 p-3 bg-sky-50 dark:bg-sky-900/20 border border-sky-200 dark:border-sky-800 rounded-xl animate-fade-in">
            <span className="text-sm text-sky-700 dark:text-sky-400 font-medium flex-1">
          ...
          {FILTER_TABS.map((tab) => (
            <button
              ...
              className={`flex items-center gap-2 px-3.5 py-2 text-xs font-medium rounded-xl whitespace-nowrap transition-all ${
                filter === tab.key
                  ? 'bg-sky-700 text-white shadow-sm shadow-sky-200 dark:shadow-none'
        ...
        {loading ? (
          ...
          <div className="text-center py-16">
            <div className="w-16 h-16 bg-sky-100 dark:bg-sky-900/30 rounded-2xl flex items-center justify-center text-3xl mx-auto mb-4">
              🔔
            </div>
            ...
            <Link
              to={ROUTES.login}
              className="inline-flex items-center px-4 py-2 bg-sky-700 text-white text-sm font-medium rounded-xl hover:bg-sky-700 transition-colors"
            >
              로그인
            </Link>
          </div>
                    ...
                    return (
                      <div
                        ...
                        className={`group relative flex items-start gap-3 p-4 rounded-xl border transition-all duration-200 ${
                          isSelected
                            ? 'border-sky-400 bg-sky-50 dark:bg-sky-900/20 dark:border-sky-700'
                            : !n.read
                              ? 'bg-white dark:bg-slate-800 border-sky-200 dark:border-sky-800/60 shadow-sm shadow-sky-100 dark:shadow-none'
                      ...
                      >
                        ...
                        {selectMode && (
                          <button
                            ...
                            className={`shrink-0 mt-0.5 w-5 h-5 rounded-md border-2 flex items-center justify-center transition-all ${
                              isSelected
                                ? 'bg-sky-700 border-sky-700 shadow-sm'
                        ...
                        >
                          <div className="flex items-center gap-2 mb-0.5">
                            ...
                            {!n.read && <span className="w-1.5 h-1.5 bg-sky-500 rounded-full" />}
                          </div>
                          <p
