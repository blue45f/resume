        ...
        <div className="relative overflow-hidden">
          ...
          <div className="relative text-center py-12 sm:py-16 px-4">
            ...
            <div className="inline-flex items-center gap-3 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-full px-4 py-2 shadow-sm">
              <span
                className={`text-sm transition-colors ${!yearly ? 'text-slate-900 dark:text-slate-100 font-medium' : 'text-slate-400'}`}
              >
                월간
              </span>
              <button
              ...
              >
                <span
                  className={`absolute top-1 w-4 h-4 bg-white rounded-full shadow transition-transform ${yearly ? 'left-7' : 'left-1'}`}
                />
              </button>
              <span
                className={`text-sm transition-colors ${yearly ? 'text-slate-900 dark:text-slate-100 font-medium' : 'text-slate-400'}`}
                ...
                {planType === 'recruiter' ? (
                  <ul className="space-y-2.5 mb-6">
                    <li className="flex items-center gap-2.5 text-sm text-slate-600 dark:text-slate-400">
                      <span className="shrink-0 w-4 h-4 bg-green-100 dark:bg-green-900/30 rounded-full flex items-center justify-center">
                        <svg
                      ...
                      스카우트{' '}
                      {plan.features.scoutMessages === -1
                        ? '무제한'
                        : `월 ${plan.features.scoutMessages}회`}
                    </li>
                    <li className="flex items-center gap-2.5 text-sm text-slate-600 dark:text-slate-400">
                      <span className="shrink-0 w-4 h-4 bg-green-100 dark:bg-green-900/30 rounded-full flex items-center justify-center">
                        <svg
                      ...
                      채용 공고{' '}
                      {plan.features.jobPosts === -1 ? '무제한' : `${plan.features.jobPosts}개`}
                    </li>
                    <li className="flex items-center gap-2.5 text-sm text-slate-600 dark:text-slate-400">
                      <span className="shrink-0 w-4 h-4 bg-green-100 dark:bg-green-900/30 rounded-full flex items-center justify-center">
                        <svg
                    ...
                    <li className="flex items-center gap-2.5 text-sm text-slate-600 dark:text-slate-400">
                      <span
                        className={`shrink-0 w-4 h-4 rounded-full flex items-center justify-center ${plan.features.prioritySupport ? 'bg-green-100 dark:bg-green-900/30' : 'bg-slate-100 dark:bg-slate-700'}`}
                      >
                  ...
                  <ul className="space-y-2.5 mb-6">
                    <li className="flex items-center gap-2.5 text-sm text-slate-600 dark:text-slate-400">
                      ...
                      이력서{' '}
                      {plan.features.maxResumes === -1 ? '무제한' : `${plan.features.maxResumes}개`}
                    </li>
                    <li className="flex items-center gap-2.5 text-sm text-slate-600 dark:text-slate-400">
                      <span className="shrink-0 w-4 h-4 bg-green-100 dark:bg-green-900/30 rounded-full flex items-center justify-center">
                        <svg
                      ...
                      AI 변환{' '}
                      {plan.features.aiTransformsPerMonth === -1
                        ? '무제한'
                        : `월 ${plan.features.aiTransformsPerMonth}회`}
                    </li>
                    <li className="flex items-center gap-2.5 text-sm text-slate-600 dark:text-slate-400">
                      <span className="shrink-0 w-4 h-4 bg-green-100 dark:bg-green-900/30 rounded-full flex items-center justify-center">
                        <svg
                    ...
                    {[
                      ...
                      >
                        <span
                          className={`shrink-0 w-4 h-4 rounded-full flex items-center justify-center ${item.feat ? 'bg-green-100 dark:bg-green-900/30' : 'bg-slate-100 dark:bg-slate-700'}`}
                        >
                          {item.feat ? (
                            <svg
                              className="w-2.5 h-2.5 text-green-600 dark:text-green-400"
                ...
                {isCurrentPlan ? (
                  ...
                  <Link
                    to={
                      plan.id === 'free'
                        ? '/resumes/new'
                        : `/payment?plan=${plan.id}&period=${yearly ? 'yearly' : 'monthly'}`
                    }
                    className={`block w-full py-3 text-center text-sm font-semibold rounded-xl transition-all duration-200 ${
                      plan.popular
        ...
        {planType === 'personal' && (
          <div className="max-w-4xl mx-auto px-4 sm:px-6 py-12 sm:py-16">
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400 text-center mb-8">
              플랜별 제공 기능을 한눈에 비교해보세요
            </p>
            <div className="overflow-x-auto imp-card shadow-sm">
              <table className="w-full min-w-[500px]">
                <thead>
                  <tr className="border-b border-slate-200 dark:border-slate-700">
                    <th className="text-left py-4 px-4 text-sm font-semibold text-slate-500">
                      기능
                    </th>
                    <th className="text-center py-4 px-2 text-sm font-semibold text-slate-500">
                      무료
                    </th>
                    <th className="text-center py-4 px-2 text-sm font-bold text-blue-600 dark:text-blue-400">
                      <span className="inline-flex items-center gap-2 px-2.5 py-0.5 bg-blue-50 dark:bg-blue-900/20 rounded-full">
