...
export default function ComparePage() {
  ...
  const sections = [
    ...
    { key: 'educations', label: '학력', count: (r: Resume) => r.educations.length },
    {
      key: 'skills',
      label: '기술',
      count: (r: Resume) => r.skills.reduce((s, sk) => s + sk.items.split(',').length, 0),
    },
    ...
    return (
      <div className="py-2.5 sm:py-2">
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          <span className="text-xs text-slate-500 dark:text-slate-400 w-16 text-right tabular-nums hidden sm:block">
            {leftVal}
          </span>
          <div className="flex-1 flex gap-1 h-6 sm:h-5">
            ...
            <div className="flex-1 bg-slate-100 dark:bg-slate-700 rounded-r-full overflow-hidden">
              <div
                ...
                style={{ width: `${(rightVal / max) * 100}%` }}
              />
            </div>
          </div>
          <span className="text-xs text-slate-500 dark:text-slate-400 w-16 tabular-nums hidden sm:block">
            {rightVal}
          </span>
          <span className="text-xs text-slate-600 dark:text-slate-300 w-16 font-medium hidden sm:block">
            {label}
          </span>
          {/* Section match percentage */}
          <span
            className={`text-[10px] font-bold w-10 text-right hidden sm:block ${
              leftVal === rightVal
                ? 'text-slate-400'
                : leftVal > rightVal
                  ? 'text-blue-500'
          ...
          <div className="animate-fade-in-up space-y-6">
              ...
              <div className="grid grid-cols-2 gap-3 sm:gap-4 text-center">
                ...
                <div>
                  <span className="text-2xl sm:text-3xl font-bold text-emerald-600">
                    {calculateCompleteness(right).percentage}%
                  </span>
                  <p className="text-xs text-slate-500 mt-1 truncate">
                    {right.title || '제목 없음'}
                  </p>
            ...
            <div className="imp-card p-5">
              ...
              <div className="flex justify-center gap-6 mt-4 text-xs text-slate-400">
                ...
                <span className="flex items-center gap-1">
                  <span className="w-3 h-3 rounded-full bg-emerald-500" /> 이력서 B
                </span>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="bg-white dark:bg-slate-800 rounded-xl border border-emerald-200 dark:border-emerald-800 p-3 sm:p-4">
                <h4 className="text-sm font-semibold text-emerald-600 mb-2 truncate">
                  {right.personalInfo.name || '이름 없음'}
                </h4>
                <p className="text-xs text-slate-500 dark:text-slate-400 line-clamp-4 break-words">
                ...
                <div className="mb-3">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {skillDiff.shared.map((s) => (
                      <span
                        key={s}
                        className="px-2 py-1 text-xs font-medium bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-300 rounded-lg capitalize"
                      >
                        {s}
                      </span>
                    ))}
            ...
            <div className="imp-card p-5">
              ...
              <details className="mb-3 group">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-3">
                  ...
                  <div className="space-y-2">
                    {right.experiences.length > 0 ? (
                      right.experiences.map((exp) => (
                        <div
                          key={exp.id}
                          className={`p-2.5 rounded-lg text-xs border-l-3 ${
              ...
              <details className="mb-3 group">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-3">
                  <div className="space-y-2">
                    {left.educations.length > 0 ? (
                      left.educations.map((edu) => (
                        <div
                          key={edu.id}
                          className="p-2.5 bg-blue-50 dark:bg-blue-900/20 rounded-lg text-xs border-l-3 border-l-blue-400"
                        >
                          <p className="font-medium text-slate-700 dark:text-slate-300">
                            {edu.school}
                          </p>
                  ...
                  <div className="space-y-2">
                    {right.educations.length > 0 ? (
                      right.educations.map((edu) => (
                        <div
                          key={edu.id}
                          className="p-2.5 bg-emerald-50 dark:bg-emerald-900/20 rounded-lg text-xs border-l-3 border-l-emerald-400"
                        >
                          <p className="font-medium text-slate-700 dark:text-slate-300">
                            {edu.school}
                          </p>
