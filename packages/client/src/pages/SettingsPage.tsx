    ...
    >
      ...
      >
        <div className="flex items-center gap-2.5">
          ...
          <h2
            className={`text-base font-semibold ${danger ? 'text-red-600 dark:text-red-400' : 'text-slate-900 dark:text-slate-100'}`}
          >
            {title}
          </h2>
        </div>
...
const NAV_ITEMS = [
  {
    id: 'sec-profile',
    label: '프로필',
    keywords: '이름 이메일 아바타 프로필 사진 닉네임 username',
  },
    ...
    <>
      ...
      >
        ...
        <CollapsibleSection
          ...
          icon="🔄"
          title="사용자 유형"
          open={openSections.has('sec-usertype')}
          onToggle={() => toggleSection('sec-usertype')}
        >
              ...
              return (
                ...
                >
                  ...
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    />
                    <span
                      className={`text-sm font-medium ${isActive ? 'text-slate-900 dark:text-slate-100' : 'text-slate-600 dark:text-slate-400'}`}
                    >
                      {opt.label}
                    </span>
                  </div>
        ...
        {userType === 'personal' && (
          <CollapsibleSection
            id="sec-opento"
            icon="🟢"
            title="구직 중 상태 (Open to Work)"
            open={openSections.has('sec-opento')}
            onToggle={() => toggleSection('sec-opento')}
          >
            <p className="text-sm text-slate-500 dark:text-slate-400 mb-4">
              구직 중 상태를 공개하면 채용 담당자가 내 이력서를 더 쉽게 발견할 수 있습니다. 이력서
              탐색 페이지에서 초록색 배지로 표시됩니다.
            </p>
              ...
              <div className="flex items-center justify-between p-4 bg-slate-50 dark:bg-slate-700/50 rounded-xl border border-slate-200 dark:border-slate-600">
                <div>
                  <p className="text-sm font-medium text-slate-900 dark:text-slate-100 flex items-center gap-2">
                    <span
                      className={`inline-block w-2.5 h-2.5 rounded-full ${isOpenToWork ? 'bg-green-500' : 'bg-slate-400'}`}
                    />
        ...
        <CollapsibleSection
          ...
          icon="💳"
          title="구독 관리"
          open={openSections.has('sec-subscription')}
          onToggle={() => toggleSection('sec-subscription')}
        >
                  ...
                  <div key={u.feature}>
                    ...
                    {limit > 0 && (
                      <div className="bg-slate-100 dark:bg-slate-700 rounded-full h-1.5">
                        <div
                          className={`h-1.5 rounded-full transition-all ${pct >= 90 ? 'bg-red-500' : pct >= 70 ? 'bg-amber-500' : 'bg-blue-500'}`}
                          style={{ width: `${pct}%` }}
                        />
                      </div>
                    )}
        ...
        <CollapsibleSection
          ...
          icon="📊"
          title="최근 활동"
          open={openSections.has('sec-activity')}
          onToggle={() => toggleSection('sec-activity')}
        >
          <RecentActivityList />
        </CollapsibleSection>
        ...
        <CollapsibleSection
          ...
          icon="🔗"
          title="소셜 계정 연동"
          open={openSections.has('sec-social')}
          ...
          <div className="space-y-3">
            {[
              ...
              return (
                <div
                  key={p.id}
                  className={`flex items-center justify-between p-3 rounded-xl border ${p.color} dark:border-slate-600 transition-colors`}
                >
        ...
        {user.provider === 'local' && (
          <CollapsibleSection
            ...
            icon="🔐"
            title="비밀번호 변경"
            open={openSections.has('sec-password')}
            onToggle={() => toggleSection('sec-password')}
          >
            <form onSubmit={onChangePassword} className="space-y-3">
              ...
              <button
                ...
                className="px-5 py-2.5 bg-blue-600 text-white text-sm font-medium rounded-xl hover:bg-blue-700 disabled:opacity-50 transition-all duration-200"
              >
        ...
        <CollapsibleSection
          ...
          icon="🔔"
          title="알림 설정"
          open={openSections.has('sec-notifications')}
          onToggle={() => toggleSection('sec-notifications')}
        >
          <p className="text-sm text-slate-500 dark:text-slate-400 mb-4">
            받고 싶은 알림 유형을 선택하세요.
          </p>
            ...
            {[
              {
                key: 'email' as const,
                label: '이메일 알림',
                desc: '이력서 변환 완료, 지원 현황 등의 이메일 알림',
              },
              {
                key: 'scout' as const,
                label: '스카우트 알림',
                desc: '채용담당자의 스카우트 제안 알림',
              },
              {
                key: 'comment' as const,
                label: '댓글 알림',
                desc: '내 이력서에 달린 댓글 및 피드백 알림',
              },
            ].map((item) => (
              <div
                key={item.key}
                className="flex items-center justify-between p-3 rounded-xl border border-slate-200 dark:border-slate-600"
              >
                <div>
                ...
                >
                  <span
                    className={`absolute top-0.5 left-0.5 w-5 h-5 bg-white rounded-full shadow transition-transform duration-200 ${notifications[item.key] ? 'translate-x-5' : 'translate-x-0'}`}
                  />
                </button>
              </div>
            ))}
        ...
        <CollapsibleSection
          ...
          icon="🎨"
          title="테마 설정"
          open={openSections.has('sec-theme')}
          onToggle={() => toggleSection('sec-theme')}
        >
          <p className="text-sm text-slate-500 dark:text-slate-400 mb-4">
            인터페이스 테마를 선택하세요.
          </p>
          <div className="grid grid-cols-3 gap-3 mb-4">
            {[
              ...
              >
                ...
                <span
                  className={`text-sm font-medium ${currentTheme === opt.value ? 'text-blue-600 dark:text-blue-400' : 'text-slate-600 dark:text-slate-400'}`}
                >
                  {opt.label}
                </span>
              </button>
          ...
          <div className="rounded-xl border border-slate-200 dark:border-slate-600 overflow-hidden">
            ...
            <div className={`p-4 ${isDark ? 'bg-slate-900' : 'bg-white'}`}>
              <div className="flex items-center gap-3 mb-3">
                <div
                  className={`w-8 h-8 rounded-full ${isDark ? 'bg-slate-700' : 'bg-slate-200'}`}
                />
                <div className="flex-1 space-y-1.5">
                  <div
                    className={`h-2.5 rounded-full w-24 ${isDark ? 'bg-slate-700' : 'bg-slate-200'}`}
                  />
                  <div
                    className={`h-2 rounded-full w-16 ${isDark ? 'bg-slate-800' : 'bg-slate-100'}`}
                  />
                </div>
              </div>
              <div className="space-y-1.5">
                <div
                  className={`h-2 rounded-full w-full ${isDark ? 'bg-slate-800' : 'bg-slate-100'}`}
                />
                <div
                  className={`h-2 rounded-full w-3/4 ${isDark ? 'bg-slate-800' : 'bg-slate-100'}`}
                />
                <div
                  className={`h-2 rounded-full w-1/2 ${isDark ? 'bg-slate-800' : 'bg-slate-100'}`}
                />
              </div>
            </div>
          </div>
        ...
        <CollapsibleSection
          ...
          icon="📦"
          title="데이터 내보내기"
          open={openSections.has('sec-data')}
          onToggle={() => toggleSection('sec-data')}
        >
          <p className="text-sm text-slate-500 dark:text-slate-400 mb-4">
            모든 이력서, 지원 내역, 설정을 JSON 파일로 다운로드합니다.
          </p>
          <button
        ...
        <CollapsibleSection
          ...
          icon="⚠️"
          title="위험 영역"
          open={openSections.has('sec-danger')}
          onToggle={() => toggleSection('sec-danger')}
          danger
        >
          <p className="text-sm text-slate-500 dark:text-slate-400 mb-4">
            계정을 삭제하면 모든 이력서, 지원 내역, 버전 기록이 영구적으로 삭제됩니다. 이 작업은
            되돌릴 수 없습니다.
          </p>
          <button
      ...
      >
        <RadixAlertDialog.Portal>
          <RadixAlertDialog.Overlay className="fixed inset-0 z-[90] bg-black/50 backdrop-blur-sm animate-fade-in" />
          <RadixAlertDialog.Content className="fixed z-[91] left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-[calc(100%-2rem)] max-w-md bg-white dark:bg-neutral-800 rounded-2xl border border-red-200 dark:border-red-900 shadow-2xl p-6 max-h-[90vh] overflow-y-auto focus:outline-none">
            <div className="flex items-center gap-2 mb-4">
              ...
              <div>
                <RadixAlertDialog.Title className="text-lg font-semibold text-red-600 dark:text-red-400">
                  계정 삭제
                </RadixAlertDialog.Title>
                <RadixAlertDialog.Description className="text-xs text-neutral-500 dark:text-neutral-400">
                  이 작업은 되돌릴 수 없습니다
            ...
            <p className="text-sm text-slate-600 dark:text-slate-300 mb-2">
              확인을 위해{' '}
              <span className="font-mono font-bold text-red-600 dark:text-red-400">삭제</span>를
              입력하세요:
            </p>
            <input
              type="text"
