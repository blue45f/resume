...
import { getSocialLoginUrl, loginWithGoogleCredential } from '@/lib/auth'
...
// Google 은 GIS(ID-token) 버튼으로 분리 렌더링한다. GitHub/Kakao 는 redirect 코드 flow 유지.
const PROVIDERS = [
...
export default function LoginPage() {
  ...
  // 토큰 수령 후 공통 후처리 — /me 조회·저장 후 안전한 대상으로 이동.
  // 이메일 로그인/회원가입과 GIS Google 로그인이 동일 경로를 공유한다.
  ...
  // GIS 콜백 — credential(ID 토큰)을 백엔드로 보내 검증·JWT 발급 후 공통 후처리.
  const onGoogleCredential = async (credential: string) => {
    ...
    try {
      ...
      setAuthError(err instanceof Error ? err.message : 'Google 로그인에 실패했습니다')
    }
  }

  const googleBtnRef = useGoogleSignIn({
        ...
        <div className="w-full max-w-sm">
          ...
          <div className="bg-[var(--color-surface)] rounded-2xl border border-[var(--color-border)] p-7 sm:p-8 shadow-sm">
            ...
            <div className="space-y-2.5 mb-6">
              {/* Google — GIS(ID-token) 공식 버튼. VITE_GOOGLE_CLIENT_ID 설정 시에만 노출. */}
              {GOOGLE_CLIENT_ID && (
