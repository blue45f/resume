...
describe('LlmService', ()=>{
    ...
    beforeEach(async ()=>{
        ...
        const module = await _testing.Test.createTestingModule({
            providers: [
                ...
                {
                    provide: _anthropicprovider.AnthropicProvider,
                    useValue: anthropicProvider
                },
                {
                    provide: _geminiprovider.GeminiProvider,
                    useValue: geminiProvider
                },
                {
                    provide: _groqprovider.GroqProvider,
                    useValue: groqProvider
                },
                {
                    provide: _n8nwebhookprovider.N8nWebhookProvider,
                    useValue: n8nProvider
                },
                {
                    provide: _openaicompatibleprovider.OpenAiCompatibleProvider,
                    useValue: openAiProvider
                }
            ]
        }).compile();
        service = module.get(_llmservice.LlmService);
        jest.clearAllMocks();
    });
    ...
    describe('generateWithFallback', ()=>{
        ...
        it('모든 프로바이더 rate-limit → BadRequestException', async ()=>{
            ...
            await expect(service.generateWithFallback('system', 'user')).rejects.toThrow(_common.BadRequestException);
        });
    ...
    describe('buildSystemPrompt (간접 테스트 via transform)', ()=>{
        ...
        it('custom 프롬프트 길이 초과 → BadRequestException', async ()=>{
            ...
            await expect(service.transform('resume-1', dto)).rejects.toThrow(_common.BadRequestException);
        });
    ...
    describe('inlineAssist', ()=>{
        ...
        it('빈 텍스트 → BadRequestException', async ()=>{
            await expect(service.inlineAssist('', 'improve')).rejects.toThrow(_common.BadRequestException);
            await expect(service.inlineAssist(' ', 'improve')).rejects.toThrow(_common.BadRequestException);
        });
        it('2000자 초과 텍스트 → BadRequestException', async ()=>{
            const longText = 'x'.repeat(2001);
            await expect(service.inlineAssist(longText, 'improve')).rejects.toThrow(_common.BadRequestException);
    ...
    describe('analyzeJobMatch', ()=>{
        ...
        it('JD가 20자 미만 → BadRequestException', async ()=>{
            await expect(service.analyzeJobMatch('resume-1', '짧은 JD')).rejects.toThrow(_common.BadRequestException);
            await expect(service.analyzeJobMatch('resume-1', '짧은 JD')).rejects.toThrow('20자 이상');
        });
        it('JD가 빈 문자열 → BadRequestException', async ()=>{
            await expect(service.analyzeJobMatch('resume-1', '')).rejects.toThrow(_common.BadRequestException);
        });
        it('JD가 3000자 초과 → BadRequestException', async ()=>{
            const longJd = 'x'.repeat(3001);
            await expect(service.analyzeJobMatch('resume-1', longJd)).rejects.toThrow(_common.BadRequestException);
        ...
        it('LLM 응답 파싱 실패 → BadRequestException', async ()=>{
            ...
            await expect(service.analyzeJobMatch('resume-1', validJd)).rejects.toThrow(_common.BadRequestException);
    ...
    describe('generateInterviewQuestions', ()=>{
        ...
        it('LLM 응답 파싱 실패 → BadRequestException', async ()=>{
            ...
            await expect(service.generateInterviewQuestions('resume-1')).rejects.toThrow(_common.BadRequestException);
        });
    });
    // ──────────────────────────────────────────────────
    // analyzeFeedback
    ...
    describe('analyzeFeedback', ()=>{
        ...
        it('LLM 응답 파싱 실패 → BadRequestException', async ()=>{
            ...
            await expect(service.analyzeFeedback('resume-1')).rejects.toThrow(_common.BadRequestException);
        });
    });
    // ──────────────────────────────────────────────────
    // autoGenerate
    ...
    describe('autoGenerate', ()=>{
        ...
        it('LLM 응답 파싱 실패 → BadRequestException', async ()=>{
            ...
            await expect(service.autoGenerate('데이터')).rejects.toThrow(_common.BadRequestException);
