...
describe('LlmService', () => {
    ...
    describe('inlineAssist', () => {
        ...
        it('빈 텍스트 → BadRequestException', async () => {
            await expect(service.inlineAssist('', 'improve')).rejects.toThrow(common_1.BadRequestException);
            await expect(service.inlineAssist(' ', 'improve')).rejects.toThrow(common_1.BadRequestException);
        });
        it('2000자 초과 텍스트 → BadRequestException', async () => {
            const longText = 'x'.repeat(2001);
            await expect(service.inlineAssist(longText, 'improve')).rejects.toThrow(common_1.BadRequestException);
    ...
    describe('analyzeJobMatch', () => {
        ...
        it('JD가 20자 미만 → BadRequestException', async () => {
            await expect(service.analyzeJobMatch('resume-1', '짧은 JD')).rejects.toThrow(common_1.BadRequestException);
            await expect(service.analyzeJobMatch('resume-1', '짧은 JD')).rejects.toThrow('20자 이상');
        });
        it('JD가 빈 문자열 → BadRequestException', async () => {
            await expect(service.analyzeJobMatch('resume-1', '')).rejects.toThrow(common_1.BadRequestException);
        });
        it('JD가 3000자 초과 → BadRequestException', async () => {
            const longJd = 'x'.repeat(3001);
            await expect(service.analyzeJobMatch('resume-1', longJd)).rejects.toThrow(common_1.BadRequestException);
        ...
        it('LLM 응답 파싱 실패 → BadRequestException', async () => {
            ...
            await expect(service.analyzeJobMatch('resume-1', validJd)).rejects.toThrow(common_1.BadRequestException);
    ...
    describe('generateInterviewQuestions', () => {
        ...
        it('LLM 응답 파싱 실패 → BadRequestException', async () => {
            ...
            await expect(service.generateInterviewQuestions('resume-1')).rejects.toThrow(common_1.BadRequestException);
        });
    });
    describe('analyzeFeedback', () => {
        ...
        it('LLM 응답 파싱 실패 → BadRequestException', async () => {
            ...
            await expect(service.analyzeFeedback('resume-1')).rejects.toThrow(common_1.BadRequestException);
        });
    });
    describe('autoGenerate', () => {
        ...
        it('LLM 응답 파싱 실패 → BadRequestException', async () => {
            ...
            await expect(service.autoGenerate('데이터')).rejects.toThrow(common_1.BadRequestException);
