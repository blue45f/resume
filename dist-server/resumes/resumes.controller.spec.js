...
describe('ResumesController', () => {
    ...
    beforeEach(async () => {
        const module = await testing_1.Test.createTestingModule({
            ...
            providers: [
                ...
                { provide: export_service_1.ExportService, useValue: mockExportService },
                { provide: analytics_service_1.AnalyticsService, useValue: mockAnalyticsService },
            ],
        }).compile();
        controller = module.get(resumes_controller_1.ResumesController);
        jest.clearAllMocks();
    });
    describe('findAll (GET /resumes)', () => {
        it('로그인 사용자 → 내 이력서 목록 반환', async () => {
            const mockData = { data: [{ id: 'r1', title: '이력서1' }], total: 1 };
    ...
    describe('findPublicResumes (GET /resumes/public)', () => {
        it('검색 파라미터 없이 호출 → 기본값으로 searchPublic', async () => {
            ...
            expect(mockResumesService.searchPublic).toHaveBeenCalledWith({
                query: undefined, tag: undefined, sort: undefined,
                page: 1, limit: 20,
            });
        });
        it('검색어, 태그, 정렬 조건으로 검색', async () => {
            const mockData = { data: [{ id: 'r1' }], total: 1 };
            mockResumesService.searchPublic.mockResolvedValue(mockData);
            const result = await controller.findPublicResumes('개발자', 'react', 'views', '2', '10');
            expect(result).toEqual(mockData);
            expect(mockResumesService.searchPublic).toHaveBeenCalledWith({
                query: '개발자', tag: 'react', sort: 'views',
                page: 2, limit: 10,
            });
        });
        it('limit 최대 50 제한', async () => {
            mockResumesService.searchPublic.mockResolvedValue({ data: [], total: 0 });
            await controller.findPublicResumes(undefined, undefined, undefined, '1', '999');
            expect(mockResumesService.searchPublic).toHaveBeenCalledWith(expect.objectContaining({ limit: 50 }));
        });
    });
    describe('create (POST /resumes)', () => {
        it('로그인 사용자 → 이력서 생성 성공', async () => {
            const dto = { title: '새 이력서' };
    ...
    describe('isBookmarked (GET /resumes/:id/bookmark/status)', () => {
        it('로그인 + 북마크됨 → { bookmarked: true }', async () => {
            ...
            const result = await controller.isBookmarked('r1', { user: { id: 'u1' } });
            expect(result).toEqual({ bookmarked: true });
        });
    ...
    describe('exportText (GET /resumes/:id/export/text)', () => {
        ...
        it('존재하지 않는 이력서 → NotFoundException 전달', async () => {
            mockExportService.exportAsText.mockRejectedValue(new common_1.NotFoundException('이력서를 찾을 수 없습니다'));
            const res = mockResponse();
            await expect(controller.exportText('nonexistent', res))
        ...
        it('예상치 못한 오류 → InternalServerErrorException', async () => {
            ...
            const res = mockResponse();
            await expect(controller.exportText('r1', res))
