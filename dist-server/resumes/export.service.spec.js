...
describe('ExportService', () => {
    ...
    describe('exportAsMarkdown', () => {
        ...
        it('존재하지 않는 이력서 → NotFoundException', async () => {
            mockPrisma.resume.findUnique.mockResolvedValue(null);
            await expect(service.exportAsMarkdown('fake')).rejects.toThrow(common_1.NotFoundException);
        });
    ...
    describe('exportAsJson', () => {
        ...
        it('존재하지 않는 이력서 → NotFoundException', async () => {
            mockPrisma.resume.findUnique.mockResolvedValue(null);
            await expect(service.exportAsJson('fake')).rejects.toThrow(common_1.NotFoundException);
        });
    ...
    describe('한글 문자 처리', () => {
        const koreanResume = {
            ...
            experiences: [
                { company: '네이버', position: '시니어 백엔드 개발자', startDate: '2019-03', endDate: '', current: true, description: '<b>검색 엔진 최적화</b> 및 <i>실시간 데이터 파이프라인</i> 구축', sortOrder: 0 },
            ],
            educations: [
                { school: '카이스트', degree: '석사', field: '컴퓨터공학', startDate: '2015-03', endDate: '2017-02', sortOrder: 0 },
            ],
            skills: [
            ...
            languages: [
                { name: '일본어', testName: 'JLPT', score: 'N1', sortOrder: 0 },
            ],
            awards: [],
            activities: [],
        };
