...
let ExportService = class ExportService {
    ...
    async exportAsDocx(resumeId) {
        ...
        const sectionHeading = (title)=>new _docx.Paragraph({
                children: [
                    new _docx.TextRun({
                        text: title,
                        bold: true,
        ...
        const subHeading = (text)=>new _docx.Paragraph({
                children: [
                    new _docx.TextRun({
                        text,
                        bold: true,
        ...
        const metaLine = (text)=>new _docx.Paragraph({
                children: [
                    new _docx.TextRun({
                        text,
                        color: grayColor,
        ...
        const bodyParagraph = (text)=>new _docx.Paragraph({
                children: [
                    new _docx.TextRun({
                        text,
                        size: 18
            ...
            for (const l of resume.languages){
                sections.push(new _docx.Paragraph({
                    children: [
                        ...
                        new _docx.TextRun({
                            text: `${l.testName ? `  (${l.testName})` : ''}:  ${l.score}`,
                            size: 18
                        })
                    ],
                    spacing: {
            ...
            sections: [
                {
                    properties: {
                        page: {
                            margin: {
                                top: (0, _docx.convertInchesToTwip)(0.9),
                                right: (0, _docx.convertInchesToTwip)(1.0),
                                bottom: (0, _docx.convertInchesToTwip)(0.9),
                                left: (0, _docx.convertInchesToTwip)(1.0)
                            }
                        }
                    },
                    children: sections
        ...
        return await _docx.Packer.toBuffer(doc);
    }
    async exportAsHtml(resumeId) {
        ...
        if (resume.experiences?.length) {
            const items = resume.experiences.map((e)=>entry(`<strong>${esc(e.company)}</strong> · ${esc(e.position)}${e.department ? ` (${esc(e.department)})` : ''}`, `${e.startDate} ~ ${e.current ? '현재' : e.endDate}`, [
                    strip(e.description),
                    e.achievements ? `성과: ${strip(e.achievements)}` : '',
        ...
        if (resume.projects?.length) {
            const items = resume.projects.map((p)=>entry(`<strong>${esc(p.name)}</strong>${p.company ? ` @ ${esc(p.company)}` : ''}`, `${p.startDate} ~ ${p.endDate} | 역할: ${esc(p.role)}`, [
                    strip(p.description),
                    p.techStack ? `기술: ${p.techStack}` : '',
                    p.link ? `링크: ${p.link}` : ''
                ].filter(Boolean).join('\n'))).join('');
