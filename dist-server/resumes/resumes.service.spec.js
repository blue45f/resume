...
describe('ResumesService', () => {
    ...
    describe('findPublic', () => {
        ...
        it('커스텀 페이지네이션 (page=2, limit=5)', async () => {
            ...
            expect(mockPrisma.resume.findMany).toHaveBeenCalledWith(expect.objectContaining({ skip: 5, take: 5 }));
        });
    ...
    describe('searchPublic', () => {
        ...
        it('sort=views 일 때 viewCount 내림차순 정렬', async () => {
            ...
            expect(mockPrisma.resume.findMany).toHaveBeenCalledWith(expect.objectContaining({ orderBy: { viewCount: 'desc' } }));
        });
        it('sort 미지정 시 updatedAt 내림차순 정렬', async () => {
            mockPrisma.resume.findMany.mockResolvedValue([]);
            mockPrisma.resume.count.mockResolvedValue(0);
            await service.searchPublic({ page: 1, limit: 10 });
            expect(mockPrisma.resume.findMany).toHaveBeenCalledWith(expect.objectContaining({ orderBy: { updatedAt: 'desc' } }));
        });
        ...
        it('페이지네이션 계산이 올바른지 확인', async () => {
            ...
            expect(mockPrisma.resume.findMany).toHaveBeenCalledWith(expect.objectContaining({ skip: 20, take: 10 }));
        });
    });
    describe('create', () => {
    ...
    describe('transferOwnership', () => {
        it('소유권 이전 성공', async () => {
            ...
            mockPrisma.user.findUnique = jest.fn().mockResolvedValue({ id: 'user-2', name: '김철수' });
