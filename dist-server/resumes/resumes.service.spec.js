...
describe('ResumesService', ()=>{
    ...
    beforeEach(async ()=>{
        ...
        service = module.get(_resumesservice.ResumesService);
        jest.clearAllMocks();
    ...
    describe('findPublic', ()=>{
        it('기본 페이지네이션 (page=1, limit=20)', async ()=>{
            ...
            expect(mockPrisma.resume.findMany).toHaveBeenCalledWith(expect.objectContaining({
                where: {
                    visibility: 'public'
                },
                skip: 0,
    ...
    describe('findOne', ()=>{
        it('존재하지 않는 이력서 → NotFoundException', async ()=>{
            ...
            await expect(service.findOne('fake-id')).rejects.toThrow(_common.NotFoundException);
        });
        it('비공개 이력서 - 소유자가 아닌 사용자 → ForbiddenException', async ()=>{
            mockPrisma.resume.findUnique.mockResolvedValue(mockResume);
            await expect(service.findOne('resume-1', 'other-user')).rejects.toThrow(_common.ForbiddenException);
        });
        ...
        it('공개 이력서 - 비소유자 접근 시 조회수 증가', async ()=>{
            ...
            expect(result.id).toBe('resume-1');
            expect(mockPrisma.resume.update).toHaveBeenCalledWith(expect.objectContaining({
                data: {
                    viewCount: {
                        increment: 1
                    }
    ...
    describe('setVisibility', ()=>{
        ...
        it('잘못된 visibility 값 → BadRequestException', async ()=>{
            await expect(service.setVisibility('resume-1', 'invalid', 'user-1')).rejects.toThrow(_common.BadRequestException);
        });
        it('빈 문자열 visibility → BadRequestException', async ()=>{
            await expect(service.setVisibility('resume-1', '', 'user-1')).rejects.toThrow(_common.BadRequestException);
        });
        it('다른 사용자가 공개 설정 변경 → ForbiddenException', async ()=>{
            mockPrisma.resume.findUnique.mockResolvedValue(mockResume);
            await expect(service.setVisibility('resume-1', 'public', 'other-user')).rejects.toThrow(_common.ForbiddenException);
        });
    ...
    describe('update - 소유권 검증', ()=>{
        it('다른 사용자의 이력서 수정 → ForbiddenException', async ()=>{
            mockPrisma.resume.findUnique.mockResolvedValue(mockResume);
            await expect(service.update('resume-1', {
                title: 'hack'
            }, 'other-user')).rejects.toThrow(_common.ForbiddenException);
        });
    ...
    describe('remove - 소유권 검증', ()=>{
        it('다른 사용자의 이력서 삭제 → ForbiddenException', async ()=>{
            mockPrisma.resume.findUnique.mockResolvedValue(mockResume);
            await expect(service.remove('resume-1', 'other-user')).rejects.toThrow(_common.ForbiddenException);
        });
    ...
    describe('findByShortCode', ()=>{
        ...
        it('숏코드로 공개 이력서 조회 성공', async ()=>{
            ...
            expect(mockPrisma.resume.findFirst).toHaveBeenCalledWith(expect.objectContaining({
                where: {
                    id: {
                        startsWith: 'abcd1234'
                    },
    ...
    describe('transferOwnership', ()=>{
        ...
        it('존재하지 않는 이력서 → NotFoundException', async ()=>{
            mockPrisma.resume.findUnique.mockResolvedValue(null);
            await expect(service.transferOwnership('fake-id', 'user-2')).rejects.toThrow(_common.NotFoundException);
        });
        it('존재하지 않는 대상 사용자 → NotFoundException', async ()=>{
            mockPrisma.resume.findUnique.mockResolvedValue(mockResume);
            mockPrisma.user.findUnique = jest.fn().mockResolvedValue(null);
            await expect(service.transferOwnership('resume-1', 'fake-user')).rejects.toThrow(_common.NotFoundException);
            await expect(service.transferOwnership('resume-1', 'fake-user')).rejects.toThrow('대상 사용자를 찾을 수 없습니다');
        });
    });
    // ──────────────────────────────────────────────────
    ...
    describe('remove - cascade behavior', ()=>{
        ...
        it('비소유자 + 일반 권한으로 삭제 → ForbiddenException', async ()=>{
            mockPrisma.resume.findUnique.mockResolvedValue(mockResume);
            await expect(service.remove('resume-1', 'other-user', 'user')).rejects.toThrow(_common.ForbiddenException);
        });
    ...
    describe('findBySlug', ()=>{
        ...
        it('사용자 없음 → NotFoundException', async ()=>{
            ...
            await expect(service.findBySlug('unknown', 'test')).rejects.toThrow(_common.NotFoundException);
        });
        it('비공개 이력서 → NotFoundException', async ()=>{
            ...
            await expect(service.findBySlug('hong', 'private-slug')).rejects.toThrow(_common.NotFoundException);
        });
    });
    // ──────────────────────────────────────────────────
    // bookmarks
    ...
    describe('bookmarks', ()=>{
        it('북마크 추가', async ()=>{
            mockPrisma.bookmark.create = jest.fn().mockResolvedValue({
                id: 'b1'
            });
        ...
        it('북마크 해제', async ()=>{
            mockPrisma.bookmark.deleteMany = jest.fn().mockResolvedValue({
                count: 1
            });
    ...
    describe('skill endorsements', ()=>{
        ...
        it('toggleEndorse — 추천 추가 (없는 경우)', async ()=>{
            ...
            mockPrisma.skillEndorsement.create.mockResolvedValue({
                id: 'new-1'
            });
        ...
        it('toggleEndorse — 추천 취소 (이미 있는 경우)', async ()=>{
            ...
            expect(mockPrisma.skillEndorsement.delete).toHaveBeenCalledWith({
                where: {
                    id: 'end-1'
                }
            });
