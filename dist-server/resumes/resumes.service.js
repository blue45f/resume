...
Object.defineProperty(exports, "ResumesService", {
    enumerable: true,
    get: function() {
...
let ResumesService = class ResumesService {
    /** 이력서 열람 알림 — 소유자가 다를 때만 1시간 쿨타임으로 알림 생성 */ async sendViewNotification(resumeId, resumeTitle, ownerId, viewerId) {
        if (!ownerId) return;
        try {
    ...
    async findBySlug(username, slug) {
        ...
        if (!user) throw new _common.NotFoundException('사용자를 찾을 수 없습니다');
        ...
        if (resume.visibility === 'private') {
            throw new _common.NotFoundException('이력서를 찾을 수 없습니다');
        }
        // 조회수 증가 (비동기, 에러 무시)
    ...
    async findOne(id, userId) {
        ...
        if (resume.visibility === 'private' && resume.userId && resume.userId !== userId) {
            throw new _common.ForbiddenException('이 이력서에 접근할 권한이 없습니다');
        }
    ...
    /** 소유권 검증 - 수정/삭제 시 사용 (admin은 모든 이력서 접근 가능) */ async verifyOwnership(resumeId, userId, role) {
        const resume = await this.prisma.resume.findUnique({
            where: {
                id: resumeId
            }
        ...
        if (resume.userId && resume.userId !== userId) {
            throw new _common.ForbiddenException('이 이력서를 수정할 권한이 없습니다');
        }
        return resume;
    }
    async setVisibility(id, visibility, userId, role) {
        if (![
            ...
            throw new _common.BadRequestException('유효하지 않은 공개 설정입니다. public, private, link-only 중 하나를 선택하세요');
        }
    ...
    async updateSlug(id, slug, userId, role) {
        await this.verifyOwnership(id, userId, role);
        const sanitized = slug.toLowerCase().replace(/[^\w가-힣-]/g, '').replace(/-+/g, '-').replace(/^-|-$/g, '').slice(0, 60);
        if (!sanitized) throw new _common.BadRequestException('유효한 슬러그를 입력해 주세요');
        // Check uniqueness
        ...
        if (existing) throw new _common.BadRequestException('이미 사용 중인 슬러그입니다');
        await this.prisma.resume.update({
            where: {
                id
            },
    ...
    generateSlug(title) {
        return (title || 'untitled').replace(/[^\w가-힣\s-]/g, '').replace(/\s+/g, '-').replace(/-+/g, '-').replace(/^-|-$/g, '').toLowerCase().slice(0, 60) || 'untitled';
    }
    ...
    async transferOwnership(id, newUserId) {
        ...
        if (!user) throw new _common.NotFoundException('대상 사용자를 찾을 수 없습니다');
        await this.prisma.resume.update({
            where: {
                id
            },
        ...
        return {
            success: true,
            message: `이력서가 ${user.name}에게 이전되었습니다`
        };
    }
    async remove(id, userId, role) {
        ...
        try {
            const { v2: cloudinary } = await Promise.resolve().then(()=>/*#__PURE__*/ _interop_require_wildcard(require("cloudinary")));
    ...
    async duplicate(id, userId) {
        const source = await this.prisma.resume.findUnique({
            where: {
                id
            },
            include: FULL_INCLUDE
        });
        if (!source) throw new _common.NotFoundException('이력서를 찾을 수 없습니다');
    ...
    /** 조회수 증가 (다운로드/내보내기 시 사용) */ incrementViewCount(id) {
        this.prisma.resume.update({
            where: {
                id
            },
    ...
    /** 스킬 추천 목록 조회 — { skill: { count, endorsed } } */ async getEndorsements(resumeId, viewerId) {
        ...
        for (const row of rows){
            if (!result[row.skill]) result[row.skill] = {
                count: 0,
                endorsed: false
            };
    ...
    /** 스킬 추천 토글 (없으면 추가, 있으면 삭제) */ async toggleEndorse(resumeId, userId, skill) {
        const existing = await this.prisma.skillEndorsement.findUnique({
            where: {
                resumeId_userId_skill: {
                    resumeId,
                    userId,
                    skill
                }
        ...
        if (existing) {
            await this.prisma.skillEndorsement.delete({
                where: {
                    id: existing.id
                }
            });
    ...
    formatFull(resume) {
        const pick = (arr, fields)=>arr?.map((item)=>Object.fromEntries(fields.map((f)=>[
                        f,
                        item[f]
                    ]))) ?? [];
        return {
            ...
            languages: pick(resume.languages, [
                'id',
                'name',
                'testName',
                'score',
