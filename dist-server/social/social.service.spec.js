...
describe('SocialService', ()=>{
    ...
    beforeEach(async ()=>{
        ...
        service = module.get(_socialservice.SocialService);
        jest.clearAllMocks();
    });
    // ──────────────────────────────────────────────────
    // follow
    ...
    describe('follow', ()=>{
        ...
        it('자신을 팔로우 → ForbiddenException', async ()=>{
            await expect(service.follow('user-1', 'user-1')).rejects.toThrow(_common.ForbiddenException);
    ...
    describe('getFollowers', ()=>{
        ...
        it('followingId로 쿼리', async ()=>{
            ...
            expect(mockPrisma.follow.findMany).toHaveBeenCalledWith(expect.objectContaining({
                where: {
                    followingId: 'user-1'
                }
            }));
    ...
    describe('sendScout', ()=>{
        ...
        it('리크루터가 스카우트 전송 성공', async ()=>{
            ...
            mockPrisma.scoutMessage.create.mockResolvedValue({
                id: 'sc-1',
                ...scoutData,
                senderId: 'user-1'
            });
        ...
        it('개인 회원이 스카우트 → ForbiddenException', async ()=>{
            ...
            await expect(service.sendScout('user-1', scoutData)).rejects.toThrow(_common.ForbiddenException);
        ...
        it('2000자 초과 메시지 → ForbiddenException', async ()=>{
            ...
            await expect(service.sendScout('user-1', {
                ...scoutData,
                message: longMessage
    ...
    describe('markScoutRead', ()=>{
        it('스카우트 읽음 처리 성공', async ()=>{
            mockPrisma.scoutMessage.updateMany.mockResolvedValue({
                count: 1
            });
    ...
    describe('sendMessage', ()=>{
        ...
        it('자신에게 메시지 → ForbiddenException', async ()=>{
            await expect(service.sendMessage('user-1', 'user-1', '테스트')).rejects.toThrow(_common.ForbiddenException);
            await expect(service.sendMessage('user-1', 'user-1', '테스트')).rejects.toThrow('자신에게 메시지를 보낼 수 없습니다');
        });
        it('빈 메시지 → ForbiddenException', async ()=>{
            await expect(service.sendMessage('user-1', 'user-2', '')).rejects.toThrow(_common.ForbiddenException);
    ...
    describe('getMessages', ()=>{
        it('상대방과의 메시지 목록 반환 + 읽지 않은 메시지 읽음 처리', async ()=>{
            ...
            mockPrisma.directMessage.updateMany.mockResolvedValue({
                count: 1
            });
    ...
    describe('getConversations', ()=>{
        it('대화 목록을 최신순으로 반환', async ()=>{
            ...
            mockPrisma.directMessage.findFirst.mockResolvedValueOnce({
                content: '오래된 메시지',
                createdAt: oldDate,
                senderId: 'user-1'
            }) // user-2 대화
            ...
            mockPrisma.directMessage.count.mockResolvedValueOnce(0) // user-2 unread
        ...
        it('대화가 없으면 빈 배열', async ()=>{
            mockPrisma.directMessage.findMany.mockResolvedValueOnce([]).mockResolvedValueOnce([]);
            const result = await service.getConversations('user-1');
            expect(result).toEqual([]);
        });
    });
    ...
    describe('mutual follow detection', ()=>{
        ...
        it('단방향 팔로우 시 한쪽만 true', async ()=>{
            // user-1 → user-2 존재
            mockPrisma.follow.findFirst.mockResolvedValueOnce({
                id: 'f1'
            });
            expect(await service.isFollowing('user-1', 'user-2')).toBe(true);
            // user-2 → user-1 없음
