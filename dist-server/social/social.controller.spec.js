...
describe('SocialController', ()=>{
    ...
    beforeEach(async ()=>{
        const module = await _testing.Test.createTestingModule({
            ...
            providers: [
                {
                    provide: _socialservice.SocialService,
                    useValue: mockSocialService
                }
            ]
        }).compile();
        controller = module.get(_socialcontroller.SocialController);
        jest.clearAllMocks();
    });
    // ──────────────────────────────────────────────────
    ...
    describe('follow (POST /social/follow/:userId)', ()=>{
        it('로그인 사용자 → 팔로우 성공', async ()=>{
            mockSocialService.follow.mockResolvedValue({
                followed: true
            });
            const result = await controller.follow('target-user', {
                user: {
                    id: 'u1'
                }
            });
        ...
        it('비로그인 → 에러 메시지 반환', ()=>{
            const result = controller.follow('target-user', {
                user: null
            });
            expect(result).toEqual({
                error: '로그인 필요'
    ...
    describe('unfollow (DELETE /social/follow/:userId)', ()=>{
        it('로그인 사용자 → 언팔로우 성공', async ()=>{
            mockSocialService.unfollow.mockResolvedValue({
                followed: false
            });
            const result = await controller.unfollow('target-user', {
                user: {
                    id: 'u1'
                }
            });
        ...
        it('비로그인 → 에러 메시지 반환', ()=>{
            const result = controller.unfollow('target-user', {
                user: null
            });
            expect(result).toEqual({
                error: '로그인 필요'
    ...
    // ──────────────────────────────────────────────────
    // GET /api/social/followers — 팔로워 목록
    // ──────────────────────────────────────────────────
    describe('getFollowers (GET /social/followers)', ()=>{
        ...
        it('비로그인 → 빈 배열 반환', ()=>{
            const result = controller.getFollowers({
                user: null
            });
            expect(result).toEqual([]);
    ...
    // ──────────────────────────────────────────────────
    // GET /api/social/following — 팔로잉 목록
    // ──────────────────────────────────────────────────
    describe('getFollowing (GET /social/following)', ()=>{
        ...
        it('비로그인 → 빈 배열 반환', ()=>{
            const result = controller.getFollowing({
                user: null
            });
            expect(result).toEqual([]);
        });
    ...
    // ──────────────────────────────────────────────────
    // POST /api/social/scout — 스카우트 메시지 보내기
    // ──────────────────────────────────────────────────
    describe('sendScout (POST /social/scout)', ()=>{
        ...
        it('로그인 사용자 → 스카우트 전송 성공', async ()=>{
            ...
            const result = await controller.sendScout(scoutBody, {
                user: {
                    id: 'u1'
                }
            });
        ...
        it('비로그인 → 에러 메시지 반환', ()=>{
            const result = controller.sendScout(scoutBody, {
                user: null
            });
            expect(result).toEqual({
                error: '로그인 필요'
        ...
        it('resumeId 없이도 스카우트 전송 가능', async ()=>{
            ...
            mockSocialService.sendScout.mockResolvedValue({
                id: 's2'
            });
            const result = await controller.sendScout(bodyNoResume, {
                user: {
                    id: 'u1'
                }
            });
    ...
    // ──────────────────────────────────────────────────
    // GET /api/social/scouts — 받은 스카우트 목록
    // ──────────────────────────────────────────────────
    describe('getScouts (GET /social/scouts)', ()=>{
        it('로그인 사용자 → 받은 스카우트 목록 반환', async ()=>{
            ...
            const result = await controller.getScouts({
                user: {
                    id: 'u1'
                }
            });
        ...
        it('비로그인 → 빈 배열 반환', ()=>{
            const result = controller.getScouts({
                user: null
            });
            expect(result).toEqual([]);
        });
    ...
    // ──────────────────────────────────────────────────
    // POST /api/social/scouts/:id/read — 스카우트 읽음 처리
    // ──────────────────────────────────────────────────
    describe('markRead (POST /social/scouts/:id/read)', ()=>{
        it('로그인 사용자 → 읽음 처리 성공', async ()=>{
            mockSocialService.markScoutRead.mockResolvedValue({
                success: true
            });
            const result = await controller.markRead('s1', {
                user: {
                    id: 'u1'
                }
            });
        ...
        it('비로그인 → { success: false }', ()=>{
            const result = controller.markRead('s1', {
                user: null
            });
            expect(result).toEqual({
                success: false
    ...
    // ──────────────────────────────────────────────────
    // POST /api/social/messages/:receiverId — 쪽지 보내기
    // ──────────────────────────────────────────────────
    describe('sendMessage (POST /social/messages/:receiverId)', ()=>{
        it('로그인 사용자 → 쪽지 전송 성공', async ()=>{
            ...
            const result = await controller.sendMessage('u2', '안녕하세요', {
                user: {
                    id: 'u1'
                }
            });
        ...
        it('비로그인 → 에러 메시지 반환', ()=>{
            const result = controller.sendMessage('u2', '내용', {
                user: null
            });
            expect(result).toEqual({
                error: '로그인 필요'
    ...
    describe('getMessages (GET /social/messages/:partnerId)', ()=>{
        it('로그인 사용자 → 대화 내용 반환', async ()=>{
            ...
            const result = await controller.getMessages('u2', {
                user: {
                    id: 'u1'
                }
            });
        ...
        it('비로그인 → 빈 배열 반환', ()=>{
            const result = controller.getMessages('u2', {
                user: null
            });
            expect(result).toEqual([]);
    ...
    describe('getConversations (GET /social/messages)', ()=>{
        ...
        it('비로그인 → 빈 배열 반환', ()=>{
            const result = controller.getConversations({
                user: null
            });
            expect(result).toEqual([]);
        });
