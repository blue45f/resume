...
describe('HealthController', ()=>{
    ...
    beforeEach(async ()=>{
        const module = await _testing.Test.createTestingModule({
            ...
            providers: [
                ...
                {
                    provide: _adminstatsservice.AdminStatsService,
                    useValue: mockStatsService
                },
                {
                    provide: _usageservice.UsageService,
    ...
    describe('publicStats (GET /health/stats)', ()=>{
        it('사용자수, 이력서수, 조회수, 템플릿수, 커뮤니티, 채용 반환', async ()=>{
            ...
            mockPrisma.resume.aggregate.mockResolvedValue({
                _sum: {
                    viewCount: 5000
                }
            });
    ...
    describe('adminStats (GET /health/admin/stats)', ()=>{
        ...
        it('일반 사용자 → ForbiddenException', async ()=>{
            await expect(controller.adminStats({
                user: {
                    id: 'u1',
                    role: 'user'
                }
    ...
    describe('getUsage (GET /health/usage)', ()=>{
        ...
        it('비로그인 → UnauthorizedException', async ()=>{
            await expect(controller.getUsage({
                user: null
            })).rejects.toThrow(_common.UnauthorizedException);
        });
    });
    ...
    describe('adminStats 인증 요구 상세', ()=>{
        it('req 객체에 user 프로퍼티 자체가 없음 → UnauthorizedException', async ()=>{
            await expect(controller.adminStats({})).rejects.toThrow(_common.UnauthorizedException);
        });
    ...
    describe('Draft API', ()=>{
        ...
        it('임시저장 저장', async ()=>{
            ...
            const result = await controller.saveDraft('community_post', {
                title: '임시'
            }, mockReq);
            expect(result.success).toBe(true);
        });
        it('임시저장 삭제', async ()=>{
            mockPrisma.draft.deleteMany.mockResolvedValue({
                count: 1
            });
