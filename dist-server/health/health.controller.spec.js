...
describe('HealthController', () => {
    ...
    describe('adminStats 인증 요구 상세', () => {
        it('req 객체에 user 프로퍼티 자체가 없음 → UnauthorizedException', async () => {
            await expect(controller.adminStats({})).rejects.toThrow(common_1.UnauthorizedException);
        });
        it('user.role이 undefined → ForbiddenException', async () => {
            await expect(controller.adminStats({ user: { id: 'u1', role: undefined } })).rejects.toThrow(common_1.ForbiddenException);
        });
        it('user.role이 빈 문자열 → ForbiddenException', async () => {
            await expect(controller.adminStats({ user: { id: 'u1', role: '' } })).rejects.toThrow(common_1.ForbiddenException);
        });
        ...
        it('서비스 에러 → 그대로 전파됨', async () => {
            ...
            await expect(controller.adminStats({ user: { id: 'a1', role: 'admin' } })).rejects.toThrow('DB error');
        });
    });
