...
describe('NotificationsService', () => {
    ...
    beforeEach(async () => {
        const module = await testing_1.Test.createTestingModule({
            providers: [notifications_service_1.NotificationsService, { provide: prisma_service_1.PrismaService, useValue: mockPrisma }],
        }).compile();
    ...
    it('읽지 않은 알림 목록 반환', async () => {
        ...
        expect(mockPrisma.notification.findMany).toHaveBeenCalledWith(expect.objectContaining({ where: { userId: 'user-1', read: false } }));
    });
    it('알림 생성', async () => {
    ...
    it('모든 알림 읽음 처리', async () => {
        ...
        expect(mockPrisma.notification.updateMany).toHaveBeenCalledWith(expect.objectContaining({ where: { userId: 'user-1', read: false } }));
    });
    it('읽지 않은 알림 수', async () => {
