...
const mockResume = {
    ...
    skills: [
        { category: '프로그래밍', items: 'TypeScript, React, Node.js' },
    ],
    projects: [
        {
            name: '이력서 플랫폼',
    ...
    languages: [
        { name: '영어', testName: 'TOEIC', score: '950', testDate: '2023-01' },
    ],
    awards: [
        { name: '우수상', issuer: '해커톤', awardDate: '2023-11', description: '1등' },
    ],
    activities: [
        {
            name: '오픈소스 기여',
...
describe('LocalTransformService', () => {
    let service;
    beforeEach(() => {
        service = new local_transform_service_1.LocalTransformService();
    });
    ...
    describe('transform', () => {
        ...
        it('빈 섹션 목록은 기본 순서 사용', () => {
            const result = service.transform(mockResume, { sections: [] });
            expect(result).toContain('# 홍길동');
            expect(result).toContain('## 경력');
        });
        it('자격증 섹션 렌더링', () => {
    ...
    describe('빈 데이터 처리', () => {
        it('빈 이력서 데이터 처리', () => {
            const result = service.transform({}, { sections: ['personalInfo', 'experiences', 'skills'] });
            expect(result).toBe('');
        });
        it('personalInfo만 있는 경우', () => {
            const result = service.transform({ personalInfo: { name: '테스트' } }, { sections: ['personalInfo', 'experiences'] });
