...
Object.defineProperty(global, 'localStorage', {
    value: feedbackStorageMock
});
// ─── 헬퍼: FeedbackPage의 로직을 순수 함수로 추출 ───
function createFeedbackItem(type, title, content, authorName) {
    return {
...
function voteItem(items, id) {
    return items.map((item)=>item.id === id ? {
            ...item,
            votes: item.votes + 1
        } : item);
}
function sortByVotes(items) {
    return [
        ...items
...
describe('Feedback 시스템', ()=>{
    ...
    describe('localStorage 저장/로드', ()=>{
        ...
        it('피드백 저장 후 로드', ()=>{
            ...
            expect(feedbackStorageMock.setItem).toHaveBeenCalledWith('feedback-items', expect.any(String));
    ...
    describe('투표', ()=>{
        it('특정 아이템의 votes 증가', ()=>{
            const items = [
                {
                    ...createFeedbackItem('bug', '버그', '내용', '유저'),
                    id: 'item-1',
                    votes: 0
                },
                {
                    ...createFeedbackItem('feature', '기능', '내용', '유저'),
                    id: 'item-2',
                    votes: 5
                }
            ];
    ...
    describe('정렬', ()=>{
        it('투표 수 기준 내림차순 정렬', ()=>{
            const items = [
                {
                    ...createFeedbackItem('bug', '적음', '내용', '유저'),
                    votes: 1
                },
                {
                    ...createFeedbackItem('feature', '많음', '내용', '유저'),
                    votes: 10
                },
                {
        ...
        it('투표 정렬은 원본을 변경하지 않음', ()=>{
            const items = [
                {
                    ...createFeedbackItem('bug', 'A', '내용', '유저'),
                    votes: 3
                },
                {
                    ...createFeedbackItem('feature', 'B', '내용', '유저'),
                    votes: 1
                }
            ];
            const sorted = sortByVotes(items);
        ...
        it('동일 투표 수 → 순서 유지 (안정 정렬)', ()=>{
            const items = [
                {
                    ...createFeedbackItem('bug', '첫째', '내용', '유저'),
                    votes: 5
                },
                {
                    ...createFeedbackItem('feature', '둘째', '내용', '유저'),
                    votes: 5
                }
            ];
            const sorted = sortByVotes(items);
