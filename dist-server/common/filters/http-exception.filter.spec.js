...
describe('GlobalExceptionFilter', () => {
    ...
    let mockHost;
    beforeEach(() => {
        filter = new http_exception_filter_1.GlobalExceptionFilter();
        mockJson = jest.fn();
    ...
    it('HttpException → 해당 상태코드 반환', () => {
        filter.catch(new common_1.BadRequestException('잘못된 요청'), mockHost);
        expect(mockStatus).toHaveBeenCalledWith(400);
    });
    it('BadRequestException → 메시지 포함', () => {
        const body = catchAndGetBody(new common_1.BadRequestException('필드가 비어있습니다'));
        expect(body.statusCode).toBe(400);
        expect(body.message).toBe('필드가 비어있습니다');
    });
    it('NotFoundException → 404 반환', () => {
        const body = catchAndGetBody(new common_1.NotFoundException('리소스를 찾을 수 없습니다'));
        expect(body.statusCode).toBe(404);
        expect(body.message).toBe('리소스를 찾을 수 없습니다');
    });
    it('ForbiddenException → 403 반환', () => {
        const body = catchAndGetBody(new common_1.ForbiddenException('접근 권한 없음'));
        expect(body.statusCode).toBe(403);
        expect(body.message).toBe('접근 권한 없음');
    });
    it('UnauthorizedException → 401 반환', () => {
        const body = catchAndGetBody(new common_1.UnauthorizedException());
        expect(body.statusCode).toBe(401);
    });
    it('HttpException 문자열 응답 → message에 반영', () => {
        const body = catchAndGetBody(new common_1.HttpException('커스텀 에러', 422));
        expect(body.statusCode).toBe(422);
        expect(body.message).toBe('커스텀 에러');
    });
    it('HttpException 객체 응답 → message와 error 모두 반영', () => {
        const body = catchAndGetBody(new common_1.HttpException({ message: '상세 에러', error: 'Validation Error' }, 400));
    ...
    it('응답에 timestamp 포함', () => {
        const body = catchAndGetBody(new common_1.BadRequestException('test'));
