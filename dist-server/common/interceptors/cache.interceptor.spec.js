...
describe('CacheHeaderInterceptor', () => {
    ...
    beforeEach(() => {
        ...
        interceptor = new cache_interceptor_1.CacheHeaderInterceptor(reflector);
    });
    ...
    function createHandler(data = { id: 1 }) {
    ...
    it('reflector.get에 올바른 키와 핸들러를 전달한다', (done) => {
        ...
        interceptor.intercept(ctx, createHandler()).subscribe(() => {
            expect(reflector.get).toHaveBeenCalledWith(cache_interceptor_1.CACHE_TTL_KEY, handler);
            done();
        });
    });
