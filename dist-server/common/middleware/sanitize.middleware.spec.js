...
const _sanitizemiddleware = require("./sanitize.middleware");
const _common = require("@nestjs/common");
describe('SanitizeMiddleware', ()=>{
    let middleware;
    beforeEach(()=>{
        middleware = new _sanitizemiddleware.SanitizeMiddleware();
    });
    const run = (body)=>{
        const req = {
            body
    ...
    it('<img> 태그를 제거한다', ()=>{
        const { req } = run({
            title: '<img src=x onerror=alert(1)>제목'
        });
        expect(req.body.title).toBe('제목');
    });
    it('<a> 태그를 제거한다', ()=>{
        const { req } = run({
            company: '<a href="http://evil.com">링크</a>'
        });
        expect(req.body.company).toBe('링크');
    });
    it('<div>, <span> 등 일반 태그를 제거한다', ()=>{
        const { req } = run({
            role: '<div><span>엔지니어</span></div>'
        });
        expect(req.body.role).toBe('엔지니어');
    });
    it('<iframe> 태그를 제거한다', ()=>{
        const { req } = run({
            bio: '<iframe src="http://evil.com"></iframe>안녕'
        });
    ...
    it('HTML 허용 필드에서 <script> 태그를 제거한다', ()=>{
        const { req } = run({
            summary: '<p>소개</p><script>alert("xss")</script>'
        });
    ...
    it('HTML 허용 필드에서 이벤트 핸들러 속성을 제거한다', ()=>{
        const { req } = run({
            description: '<p onmouseover="alert(1)">텍스트</p>'
        });
    ...
    it('HTML 허용 필드에서 javascript: URI를 제거한다', ()=>{
        const { req } = run({
            text: '<a href="javascript:alert(1)">링크</a>'
        });
        expect(req.body.text).not.toContain('javascript:');
    });
    it('HTML 허용 필드에서 <iframe>을 제거한다', ()=>{
        const { req } = run({
            summary: '<iframe src="http://evil.com"></iframe><p>내용</p>'
        });
    ...
    it('<script> 태그를 제거한다', ()=>{
        const { req } = run({
            name: '<script>document.cookie</script>홍길동'
        });
    ...
    it('이벤트 핸들러가 포함된 태그를 제거한다', ()=>{
        const { req } = run({
            name: '<img src=x onerror="alert(1)">이름'
        });
    ...
    it('javascript: URI를 포함한 태그를 제거한다', ()=>{
        const { req } = run({
            name: '<a href="javascript:alert(1)">클릭</a>'
        });
    ...
    it('data: URI를 포함한 태그를 제거한다', ()=>{
        const { req } = run({
            name: '<object data="data:text/html,<script>alert(1)</script>">test</object>'
        });
