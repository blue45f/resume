...
describe('AuthService - OAuth State (HMAC)', ()=>{
    let service;
    beforeEach(async ()=>{
        service = Object.create(_authservice.AuthService.prototype);
...
describe('AuthService - Admin (setUserRole / getAllUsers)', ()=>{
    ...
    describe('setUserRole', ()=>{
        ...
        it('일반 사용자가 역할 변경 시도 → UnauthorizedException', async ()=>{
            ...
            await expect(service.setUserRole('user-1', 'user-2', 'admin')).rejects.toThrow('관리자만 역할을 변경할 수 있습니다');
        ...
        it('유효하지 않은 역할 → UnauthorizedException', async ()=>{
            ...
            await expect(service.setUserRole('admin-1', 'user-1', 'invalid_role')).rejects.toThrow('유효하지 않은 역할입니다');
            expect(mockPrisma.user.update).not.toHaveBeenCalled();
        });
        it('존재하지 않는 관리자 → UnauthorizedException', async ()=>{
            mockPrisma.user.findUnique.mockResolvedValue(null);
            await expect(service.setUserRole('nonexistent', 'user-1', 'admin')).rejects.toThrow('관리자만 역할을 변경할 수 있습니다');
        });
    });
...
describe('AuthService - register', ()=>{
    ...
    it('중복 이메일 → UnauthorizedException', async ()=>{
        ...
        await expect(service.register('dup@test.com', 'password123', '중복유저')).rejects.toThrow('이미 가입된 이메일입니다');
    });
    it('이메일 미입력 → UnauthorizedException', async ()=>{
        await expect(service.register('', 'password123', '이름')).rejects.toThrow('이메일, 비밀번호, 이름은 필수입니다');
    });
    it('비밀번호 미입력 → UnauthorizedException', async ()=>{
        await expect(service.register('test@test.com', '', '이름')).rejects.toThrow('이메일, 비밀번호, 이름은 필수입니다');
    });
    it('비밀번호 8자 미만 → UnauthorizedException', async ()=>{
        await expect(service.register('test@test.com', 'short', '이름')).rejects.toThrow('비밀번호는 8자 이상이어야 합니다');
    });
    it('이름 미입력 → UnauthorizedException', async ()=>{
        await expect(service.register('test@test.com', 'password123', '')).rejects.toThrow('이메일, 비밀번호, 이름은 필수입니다');
    });
    ...
    it('userType=company 시 companyName 필수', async ()=>{
        await expect(service.register('co@test.com', 'password123', '기업', 'company')).rejects.toThrow('기업 계정은 회사명이 필수입니다');
    });
...
describe('AuthService - login', ()=>{
    ...
    it('잘못된 비밀번호 → UnauthorizedException', async ()=>{
        ...
        await expect(service.login('test@test.com', 'wrongpassword')).rejects.toThrow('이메일 또는 비밀번호가 올바르지 않습니다');
    });
    it('존재하지 않는 이메일 → UnauthorizedException', async ()=>{
        mockPrisma.user.findUnique.mockResolvedValue(null);
        await expect(service.login('no@test.com', 'password')).rejects.toThrow('이메일 또는 비밀번호가 올바르지 않습니다');
    });
    it('passwordHash 없는 소셜 계정 → UnauthorizedException', async ()=>{
        ...
        await expect(service.login('social@test.com', 'anypassword')).rejects.toThrow('이메일 또는 비밀번호가 올바르지 않습니다');
    });
    it('이메일 미입력 → UnauthorizedException', async ()=>{
        await expect(service.login('', 'password')).rejects.toThrow('이메일과 비밀번호를 입력해주세요');
    });
    it('비밀번호 미입력 → UnauthorizedException', async ()=>{
        await expect(service.login('test@test.com', '')).rejects.toThrow('이메일과 비밀번호를 입력해주세요');
    });
});
// ──────────────────────────────────────────────────
// getProfile
...
describe('AuthService - getProfile', ()=>{
    ...
    it('유효한 사용자 → passwordHash 제외한 프로필 반환', async ()=>{
        ...
        mockPrisma.follow.count.mockResolvedValueOnce(10) // followerCount
    ...
    it('존재하지 않는 사용자 → UnauthorizedException', async ()=>{
        mockPrisma.user.findUnique.mockResolvedValue(null);
        await expect(service.getProfile('nonexistent')).rejects.toThrow(_common.UnauthorizedException);
    });
...
describe('AuthService - getPublicPortfolio', ()=>{
    ...
    it('topSkills 집계 — 스킬 아이템 중복 제거', async ()=>{
        ...
        expect(result.topSkills.filter((s)=>s === 'React')).toHaveLength(1);
    });
