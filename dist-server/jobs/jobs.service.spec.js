...
describe('JobsService', () => {
    ...
    describe('create', () => {
        ...
        it('기업 회원이 공고 생성 성공', async () => {
            mockPrisma.user.findUnique.mockResolvedValue({ id: 'u1', userType: 'company', companyName: '카카오' });
        ...
        it('개인 회원이 공고 생성 → ForbiddenException', async () => {
            ...
            await expect(service.create('u1', { position: 'FE' })).rejects.toThrow('채용 공고는 리크루터 또는 기업 회원만 등록할 수 있습니다');
            expect(mockPrisma.jobPost.create).not.toHaveBeenCalled();
        });
        it('존재하지 않는 사용자 → ForbiddenException', async () => {
            mockPrisma.user.findUnique.mockResolvedValue(null);
            await expect(service.create('fake', { position: 'FE' })).rejects.toThrow(common_1.ForbiddenException);
        });
        it('company 미입력 시 user.companyName 사용', async () => {
            mockPrisma.user.findUnique.mockResolvedValue({ id: 'u1', userType: 'recruiter', companyName: '라인' });
        ...
        it('필드 미입력 시 기본값 적용', async () => {
            mockPrisma.user.findUnique.mockResolvedValue({ id: 'u1', userType: 'recruiter', companyName: '' });
        ...
        it('type과 status 커스텀 값', async () => {
            mockPrisma.user.findUnique.mockResolvedValue({ id: 'u1', userType: 'company', companyName: '쿠팡' });
    ...
    describe('update', () => {
        ...
        it('다른 사용자가 수정 → ForbiddenException', async () => {
            mockPrisma.jobPost.findUnique.mockResolvedValue(mockJob);
            await expect(service.update('j1', 'u2', { position: 'hack' })).rejects.toThrow(common_1.ForbiddenException);
            expect(mockPrisma.jobPost.update).not.toHaveBeenCalled();
        });
        it('없는 공고 수정 → NotFoundException', async () => {
            mockPrisma.jobPost.findUnique.mockResolvedValue(null);
            await expect(service.update('fake', 'u1', {})).rejects.toThrow(common_1.NotFoundException);
        });
    });
    describe('remove', () => {
        ...
        it('다른 사용자가 삭제 → ForbiddenException', async () => {
            mockPrisma.jobPost.findUnique.mockResolvedValue(mockJob);
            await expect(service.remove('j1', 'u2')).rejects.toThrow(common_1.ForbiddenException);
        ...
        it('없는 공고 삭제 → NotFoundException', async () => {
            mockPrisma.jobPost.findUnique.mockResolvedValue(null);
            await expect(service.remove('fake', 'u1')).rejects.toThrow(common_1.NotFoundException);
        });
        it('일반 사용자(role=user)가 타인 공고 삭제 → ForbiddenException', async () => {
            mockPrisma.jobPost.findUnique.mockResolvedValue(mockJob);
            await expect(service.remove('j1', 'u2', 'user')).rejects.toThrow(common_1.ForbiddenException);
        });
    });
});
