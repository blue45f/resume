...
describe('AttachmentsService', ()=>{
    ...
    beforeEach(async ()=>{
        ...
        service = module.get(_attachmentsservice.AttachmentsService);
        jest.clearAllMocks();
    ...
    describe('upload', ()=>{
        ...
        it('다른 사용자의 이력서 업로드 시도 → ForbiddenException', async ()=>{
            mockPrisma.resume.findUnique.mockResolvedValue(mockResume);
            const file = createMockFile();
            await expect(service.upload('resume-1', file, 'document', '', 'other-user')).rejects.toThrow(_common.ForbiddenException);
        });
        ...
        it('파일 크기 초과 → BadRequestException', async ()=>{
            ...
            await expect(service.upload('resume-1', file, 'document', '', 'user-1')).rejects.toThrow(_common.BadRequestException);
        });
        ...
        it('존재하지 않는 이력서 → NotFoundException', async ()=>{
            ...
            const file = createMockFile();
            await expect(service.upload('fake', file, 'document', '', 'user-1')).rejects.toThrow(_common.NotFoundException);
        });
    ...
    describe('findAll', ()=>{
        ...
        it('비공개 이력서의 첨부파일 - 타인 접근 → ForbiddenException', async ()=>{
            ...
            await expect(service.findAll('resume-1', 'other-user')).rejects.toThrow(_common.ForbiddenException);
        });
    ...
    describe('getFileData', ()=>{
        ...
        it('존재하지 않는 파일 → NotFoundException', async ()=>{
            ...
            await expect(service.getFileData('fake')).rejects.toThrow(_common.NotFoundException);
        });
        it('비공개 이력서의 첨부파일 - 다른 사용자 → NotFoundException', async ()=>{
            ...
            await expect(service.getFileData('att-1', 'other-user')).rejects.toThrow(_common.NotFoundException);
        });
    });
    describe('remove', ()=>{
        ...
        it('타인의 첨부파일 삭제 시도 → ForbiddenException', async ()=>{
            ...
            await expect(service.remove('att-1', 'other-user')).rejects.toThrow(_common.ForbiddenException);
        });
        ...
        it('존재하지 않는 첨부파일 → NotFoundException', async ()=>{
            mockPrisma.attachment.findUnique.mockResolvedValue(null);
            await expect(service.remove('fake', 'user-1')).rejects.toThrow(_common.NotFoundException);
        });
    });
        ...
        beforeEach(async ()=>{
            ...
            cloudinary.uploader.upload_stream.mockImplementation((_opts, callback)=>{
                const stream = {
                            ...
                            secure_url: 'https://res.cloudinary.com/test/upload/v1/resume-attachments/resume-1/uuid.pdf'
                        })
                };
                return stream;
            });
            const module = await _testing.Test.createTestingModule({
                providers: [
                    ...
                    },
                    {
                        provide: _config.ConfigService,
                        useValue: {
                            get: (key)=>key.startsWith('CLOUDINARY') ? 'test-value' : null
                        }
                    }
                ]
            }).compile();
            cloudinaryService = module.get(_attachmentsservice.AttachmentsService);
            jest.clearAllMocks();
        });
