...
export default function JobQuestionsPanel({
  ...
  const handleUpvote = async (q: JobInterviewQuestion) => {
    ...
    try {
      ...
      setItems(prev =>
        prev.map(it =>
          it.id === q.id ? { ...it, upvotes: q.upvotes, myVote: q.myVote } : it,
        ),
      );
    }
  };
  ...
  const handleCreateQuestion = async () => {
    ...
    try {
      ...
      setShowAdd(false);
      setAddForm({ question: '', sampleAnswer: '', category: 'technical', difficulty: 'intermediate' });
      await load();
    } catch (e) {
      ...
      {loading ? (
        ...
        <ul className="space-y-2">
          {items.map((q) => {
            ...
            const difficultyClass = DIFFICULTY_COLORS[q.difficulty] || 'bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300';
            return (
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex flex-wrap items-center gap-1.5 mb-1">
                      <span className={`px-1.5 py-0.5 text-[10px] font-medium rounded ${difficultyClass}`}>
                        {categoryLabel}
                      </span>
