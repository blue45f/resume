      ...
      {permission !== 'granted' && (
        <div className="space-y-3">
          ...
          <div className="flex flex-col items-center justify-center py-8 sm:py-10 text-center bg-slate-50 dark:bg-slate-800/40 rounded-xl border border-dashed border-slate-200 dark:border-slate-700">
            <svg className="w-10 h-10 text-slate-400 mb-3" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
      ...
      {permission === 'granted' && (
        <div className="space-y-4">
          ...
          <div className="relative w-full overflow-hidden rounded-xl bg-black aspect-video">
            ...
            {/* Timer */}
            <div className="absolute top-3 right-3 px-2.5 py-1 rounded-md bg-black/60 backdrop-blur-sm text-white text-xs font-mono tabular-nums">
              {formatTime(elapsed)}{maxDurationSec ? ` / ${formatTime(maxDurationSec)}` : ''}
            </div>
          </div>
