...
const recommendationColors: Record<string, { bg: string; text: string; label: string }> = {
  good: { bg: 'bg-green-100 dark:bg-green-900/30', text: 'text-green-700 dark:text-green-400', label: '적정' },
  low: { bg: 'bg-red-100 dark:bg-red-900/30', text: 'text-red-700 dark:text-red-400', label: '부족' },
  high: { bg: 'bg-amber-100 dark:bg-amber-900/30', text: 'text-amber-700 dark:text-amber-400', label: '과다' },
};
          ...
          {detailTab === 'checklist' && (
            <div className="space-y-4">
              {Object.entries(checklistByCategory).map(([category, items]) => (
                <div key={category}>
                  <h4 className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-2">{category}</h4>
                      ...
                      >
                        ...
                        {item.passed ? (
                          ...
                          <span className={`w-5 h-5 rounded-full flex items-center justify-center shrink-0 ${
                            item.severity === 'error' ? 'bg-red-500' : item.severity === 'warning' ? 'bg-amber-500' : 'bg-slate-400'
                          }`}>
                            <svg className="w-3 h-3 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <div className="flex-1 min-w-0">
                          <span className={`text-sm ${item.passed ? 'text-green-700 dark:text-green-400' : 'text-slate-700 dark:text-slate-300'}`}>
                            {item.label}
                          </span>
                          {item.detail && (
                        ...
                        {!item.passed && (
                          <span className={`text-[10px] font-bold uppercase px-1.5 py-0.5 rounded ${
                            item.severity === 'error' ? 'bg-red-100 text-red-600' : item.severity === 'warning' ? 'bg-amber-100 text-amber-600' : 'bg-slate-100 text-slate-500'
                          }`}>
              ...
              {result.keywordDensity.length > 0 ? (
                <>
                  <p className="text-xs text-slate-500 dark:text-slate-400">
                    기술 키워드가 이력서 전체에 얼마나 자주 등장하는지 분석합니다. 각 키워드가 최소 2회 이상 사용되는 것이 좋습니다.
                  </p>
                  <div className="space-y-1.5">
                    {result.keywordDensity.map((kd, i) => {
                      const rec = recommendationColors[kd.recommendation];
                      return (
                        <div key={i} className="flex items-center gap-2 px-3 py-2 bg-slate-50 dark:bg-slate-700/30 rounded-lg">
                          <span className="text-sm text-slate-700 dark:text-slate-300 font-medium flex-1 min-w-0 truncate">{kd.keyword}</span>
                          <span className="text-xs text-slate-500 dark:text-slate-400 shrink-0">{kd.count}회</span>
                          <div className="w-16 bg-slate-200 dark:bg-slate-600 rounded-full h-1.5 shrink-0">
                            <div
                              className={`h-1.5 rounded-full transition-all ${
                                kd.recommendation === 'good' ? 'bg-green-500' : kd.recommendation === 'low' ? 'bg-red-400' : 'bg-amber-400'
                              }`}
                  ...
                  <div className="p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg text-xs text-blue-700 dark:text-blue-400 space-y-1">
                    ...
                    <ul className="space-y-0.5 pl-3">
                      <li>- &quot;부족&quot; 키워드를 경력 설명이나 프로젝트에 자연스럽게 추가하세요</li>
                      <li>- 기술 스택 섹션뿐 아니라 경력 설명에도 키워드를 포함하세요</li>
                      <li>- &quot;과다&quot; 키워드는 자연스럽지 않게 보일 수 있으니 적절히 조절하세요</li>
                    </ul>
                  </div>
                </>
              ) : (
