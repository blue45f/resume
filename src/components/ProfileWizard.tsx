...
const STEPS: WizardStep[] = [
  {
    ...
    getMissing: (r) => {
      ...
      if (!pi.summary || pi.summary.replace(/<[^>]*>/g, '').length <= 30) missing.push('자기소개 (30자 이상)');
      if (!pi.photo) missing.push('프로필 사진');
      if (!pi.website && !pi.github) missing.push('웹사이트 또는 GitHub');
      return missing;
    },
    whyMatters: '인적사항은 첫인상을 결정합니다. 채용담당자가 가장 먼저 확인하는 영역이며, 완성된 인적사항은 신뢰감을 줍니다.',
  },
  {
    key: 'experiences',
    label: '경력',
    ...
    checkComplete: (r) => r.experiences.length >= 1 && r.experiences.some(e => e.description && e.description.length > 30),
    getMissing: (r) => {
      const missing: string[] = [];
      if (r.experiences.length === 0) missing.push('경력 사항 1개 이상');
      else {
        if (!r.experiences.some(e => e.description && e.description.length > 30)) missing.push('업무 내용 상세 기술');
        if (!r.experiences.some(e => e.techStack)) missing.push('사용 기술 스택');
        if (!r.experiences.some(e => e.achievements)) missing.push('주요 성과');
      }
      return missing;
    },
    whyMatters: '경력은 이력서에서 가장 높은 비중을 차지합니다. 구체적인 성과와 기술을 포함하면 서류 통과율이 크게 올라갑니다.',
  },
  {
    key: 'educations',
    label: '학력',
    ...
    whyMatters: '학력 정보는 특히 신입이나 경력 전환 시 중요합니다. ATS(자동서류심사)에서도 학력은 핵심 필터링 항목입니다.',
  },
  {
    key: 'skills',
    label: '기술',
    ...
    checkComplete: (r) => r.skills.length >= 2 && r.skills.some(s => s.items && s.items.split(',').length >= 3),
    getMissing: (r) => {
      const missing: string[] = [];
      if (r.skills.length === 0) missing.push('기술 카테고리 1개 이상');
      else {
        if (r.skills.length < 2) missing.push('기술 카테고리 추가 (2개 이상 권장)');
        if (!r.skills.some(s => s.items && s.items.split(',').length >= 3)) missing.push('각 카테고리에 3개 이상 기술');
      }
      return missing;
    },
    whyMatters: '기술 스택은 JD(채용공고) 매칭의 핵심입니다. 명확하게 정리된 기술 목록은 채용담당자의 눈에 바로 들어옵니다.',
  },
  {
    key: 'projects',
    label: '프로젝트',
    ...
    checkComplete: (r) => r.projects.length >= 1 && r.projects.some(p => p.description && p.description.length > 30),
    getMissing: (r) => {
      const missing: string[] = [];
      if (r.projects.length === 0) missing.push('프로젝트 1개 이상');
      else {
        if (!r.projects.some(p => p.description && p.description.length > 30)) missing.push('프로젝트 상세 설명');
        if (!r.projects.some(p => p.techStack)) missing.push('사용 기술 스택');
        if (!r.projects.some(p => p.link)) missing.push('프로젝트 링크');
      }
      return missing;
    },
    whyMatters: '프로젝트는 실무 역량을 보여주는 가장 좋은 방법입니다. 포트폴리오와 함께 지원하면 면접 확률이 40% 이상 높아집니다.',
  },
];
