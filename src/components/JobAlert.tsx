...
export default function JobAlert({ jobs }: Props) {
  ...
  const [form, setForm] = useState({ keywords: '', jobType: 'all', salaryMin: '', salaryMax: '', location: '' });
  ...
  const handleSave = () => {
    if (editingId) {
      setAlerts(prev => prev.map(a => a.id === editingId ? { ...a, ...form, keywords: form.keywords, jobType: form.jobType, salaryMin: form.salaryMin, salaryMax: form.salaryMax, location: form.location } : a));
      setEditingId(null);
    } else {
  ...
  const handleEdit = (alert: JobAlertConfig) => {
    setForm({ keywords: alert.keywords, jobType: alert.jobType, salaryMin: alert.salaryMin, salaryMax: alert.salaryMax, location: alert.location });
    ...
    <>
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40" onClick={() => setOpen(false)}>
          <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-xl w-full max-w-lg max-h-[85vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between p-5 border-b border-slate-200 dark:border-slate-700">
              <div>
                <h2 className="text-lg font-bold text-slate-900 dark:text-slate-100">채용 알림 설정</h2>
                <p className="text-xs text-slate-500 mt-0.5">관심 조건을 설정하면 매칭 공고를 알려드립니다</p>
              </div>
            ...
            <div className="p-5 space-y-4">
              ...
              <div className="space-y-3 bg-slate-50 dark:bg-slate-900/50 rounded-xl p-4">
                ...
                <div>
                  <label className="block text-xs font-medium text-slate-600 dark:text-slate-400 mb-1">키워드 (쉼표로 구분)</label>
                  <input
                    type="text"
                    value={form.keywords}
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-xs font-medium text-slate-600 dark:text-slate-400 mb-1">채용 형태</label>
                    <select
                      value={form.jobType}
                  ...
                  <div>
                    <label className="block text-xs font-medium text-slate-600 dark:text-slate-400 mb-1">지역</label>
                    <input
                      type="text"
                      value={form.location}
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-xs font-medium text-slate-600 dark:text-slate-400 mb-1">최소 연봉 (만원)</label>
                    <input
                      type="number"
                  ...
                  <div>
                    <label className="block text-xs font-medium text-slate-600 dark:text-slate-400 mb-1">최대 연봉 (만원)</label>
                    <input
                      type="number"
                ...
                <div className="flex gap-2">
                  ...
                  {editingId && (
                    <button
                      onClick={() => { setEditingId(null); setForm({ keywords: '', jobType: 'all', salaryMin: '', salaryMax: '', location: '' }); }}
              ...
              {alerts.length > 0 && (
                <div>
                  <h3 className="text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">설정된 알림 ({alerts.length})</h3>
                  <div className="space-y-2">
                    {alerts.map(alert => (
                      <div key={alert.id} className={`flex items-center gap-3 p-3 rounded-xl border transition-colors ${alert.enabled ? 'border-blue-200 dark:border-blue-800 bg-blue-50/50 dark:bg-blue-900/10' : 'border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900/30 opacity-60'}`}>
                        <button onClick={() => toggleEnabled(alert.id)} className="shrink-0" title={alert.enabled ? '비활성화' : '활성화'}>
                          {alert.enabled ? (
                            ...
                            <svg className="w-5 h-5 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10" strokeWidth={2} /></svg>
                          )}
                        </button>
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-medium text-slate-800 dark:text-slate-200 truncate">{alert.keywords}</p>
                          <div className="flex flex-wrap gap-1.5 mt-1">
                            {alert.jobType && alert.jobType !== 'all' && (
                              <span className="px-1.5 py-0.5 text-[10px] bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-400 rounded">
                                {{ fulltime: '정규직', contract: '계약직', parttime: '파트타임', intern: '인턴' }[alert.jobType] || alert.jobType}
                              </span>
                            )}
                            {alert.location && (
                              <span className="px-1.5 py-0.5 text-[10px] bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-400 rounded">{alert.location}</span>
                            )}
                            {(alert.salaryMin || alert.salaryMax) && (
                              <span className="px-1.5 py-0.5 text-[10px] bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 dark:text-emerald-400 rounded">
                        ...
                        <div className="flex items-center gap-1 shrink-0">
                          <button onClick={() => handleEdit(alert)} className="p-1 rounded hover:bg-slate-200 dark:hover:bg-slate-600 transition-colors" title="수정">
                          ...
                          <button onClick={() => handleDelete(alert.id)} className="p-1 rounded hover:bg-red-100 dark:hover:bg-red-900/30 transition-colors" title="삭제">
              ...
              {alerts.length === 0 && (
                <div className="text-center py-6">
                  ...
                  <p className="text-sm text-slate-500 dark:text-slate-400">설정된 알림이 없습니다</p>
                  <p className="text-xs text-slate-400 dark:text-slate-500 mt-1">위에서 키워드와 조건을 설정하세요</p>
                </div>
              )}
            </div>
          </div>
