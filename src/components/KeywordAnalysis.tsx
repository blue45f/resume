...
function extractKeywords(text: string): Map<string, number> {
  ...
  const stopWords = new Set(['the', 'and', 'for', 'with', 'from', 'that', 'this', 'have', 'been', 'etc', '등', '및', '또는', '위한', '대한', '통해', '기반']);
    ...
    <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-4 no-print">
      ...
      {expanded && (
        <div className="mt-3 space-y-3 animate-fade-in">
          {/* Top keywords */}
          <div>
            <p className="text-xs text-slate-500 dark:text-slate-400 mb-1.5">주요 키워드 (빈도순)</p>
            <div className="flex flex-wrap gap-1.5">
              {sorted.map(([word, count]) => (
                <span key={word} className="inline-flex items-center gap-1 px-2 py-0.5 text-xs bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 rounded-full">
          ...
          {/* JD comparison */}
          <div>
            <button onClick={() => setShowJd(!showJd)} className="text-xs text-blue-600 dark:text-blue-400 hover:underline">
                ...
                {jd && (
                  <div className="grid grid-cols-2 gap-2">
                    <div className="p-2 bg-green-50 dark:bg-green-900/20 rounded-lg">
                      <p className="text-xs font-medium text-green-700 dark:text-green-400 mb-1">매칭 키워드 ({matchedKeys.length})</p>
                      <div className="flex flex-wrap gap-1">
                        {matchedKeys.map(([k]) => (
                          <span key={k} className="text-xs px-1.5 py-0.5 bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400 rounded">{k}</span>
                        ))}
                      </div>
                    </div>
                    <div className="p-2 bg-red-50 dark:bg-red-900/20 rounded-lg">
                      <p className="text-xs font-medium text-red-700 dark:text-red-400 mb-1">누락 키워드 ({missingKeys.length})</p>
                      <div className="flex flex-wrap gap-1">
                        {missingKeys.map(([k]) => (
                          <span key={k} className="text-xs px-1.5 py-0.5 bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400 rounded">{k}</span>
                        ))}
                      </div>
                    </div>
                  </div>
