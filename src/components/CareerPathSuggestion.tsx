...
function analyzeCareerPaths(resume: Resume): CareerNode {
  ...
  const allPaths: { title: string; skills: string[]; salaryRange: string; salaryMin: number; salaryMax: number; timeline: string; description: string; relevance: string[] }[] = [
    {
      title: '시니어 프론트엔드 개발자',
    ...
    },
    {
      ...
      salaryMin: 8500, salaryMax: 13000,
      timeline: '3~5년',
    ...
    },
    {
      ...
      salaryMin: 7500, salaryMax: 12000,
      timeline: '2~3년',
    ...
    },
    {
      ...
      salaryMin: 7500, salaryMax: 13000,
      timeline: '2~4년',
    ...
    },
    {
      ...
      salaryMin: 6500, salaryMax: 11000,
      timeline: '1~3년',
  ...
  const scoredPaths = allPaths.map(p => {
    ...
    const relevanceBonus = p.relevance.some(r =>
      currentSkills.has(r) || positionLower.includes(r) || allSkills.includes(r)
    ) ? 15 : 0;
  ...
  return (
    <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-4 no-print">
      <button onClick={() => setExpanded(!expanded)} className="w-full flex items-center justify-between">
        <h3 className="text-sm font-semibold text-slate-800 dark:text-slate-200">커리어 패스 로드맵</h3>
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-1.5">
                          ...
                          <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${
                            p.match >= 70 ? 'bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400' :
                            p.match >= 40 ? 'bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400' :
                      ...
                      <div className="mt-1 bg-slate-200 dark:bg-slate-700 rounded-full h-1 ml-3">
                        <div
                          className={`h-1 rounded-full transition-all duration-500 ${
                            p.match >= 70 ? 'bg-green-500' : p.match >= 40 ? 'bg-blue-500' : 'bg-amber-500'
                          }`}
                          style={{ width: `${p.match}%` }}
                        />
                      </div>
                    ...
                    {isSelected && (
                      <div className="mt-2 ml-3 space-y-2.5 animate-fade-in">
                        ...
                        <div className="flex items-center gap-2 p-2 bg-emerald-50 dark:bg-emerald-900/20 rounded-lg border border-emerald-200 dark:border-emerald-800">
                          ...
                          <span className="text-xs font-medium text-emerald-700 dark:text-emerald-400">연봉 범위: {p.salaryRange}</span>
                        </div>
                        ...
                        <div className="flex items-center gap-2 p-2 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-200 dark:border-blue-800">
                          ...
                          <span className="text-xs font-medium text-blue-700 dark:text-blue-400">예상 소요 기간: {p.timeline}</span>
                        </div>

                        {/* Skills list */}
                        <div>
                          <p className="text-[10px] font-semibold text-slate-500 dark:text-slate-400 mb-1 uppercase tracking-wider">이 경로에 필요한 기술</p>
                          <div className="flex flex-wrap gap-1">
                            {p.currentSkills.map(s => (
                              <span key={s} className="px-1.5 py-0.5 text-[10px] bg-green-50 dark:bg-green-900/20 text-green-600 dark:text-green-400 rounded border border-green-200 dark:border-green-800 flex items-center gap-0.5">
                                <svg className="w-2.5 h-2.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" /></svg>
                                {s}
                              </span>
                            ))}
                            {p.missingSkills.map(s => (
                              <span key={s} className="px-1.5 py-0.5 text-[10px] bg-red-50 dark:bg-red-900/20 text-red-500 dark:text-red-400 rounded border border-red-200 dark:border-red-800">
                                + {s}
                              </span>
                            ))}
                          </div>
                          <p className="text-[10px] text-slate-400 mt-1">
                            보유: {p.currentSkills.length}/{p.requiredSkills.length} ({p.match}% 매칭)
                          </p>
                        </div>

                        {/* Save roadmap button */}
                        <button
                          onClick={(e) => { e.stopPropagation(); handleSaveRoadmap(p.title); }}
