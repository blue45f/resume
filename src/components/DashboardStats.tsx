...
export default function DashboardStats() {
  ...
  const stats = [
    ...
    { label: '총 조회', value: data.summary.totalViews, prevValue: prev.totalViews, icon: '👁', color: 'text-purple-600', link: '/' },
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          ...
          {viewsChartData.length > 0 && (
            <div className="sm:col-span-2 bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-4">
              <h3 className="text-xs font-semibold text-slate-600 dark:text-slate-400 mb-3">최근 7일 조회수 추이</h3>
              <ResponsiveContainer width="100%" height={120}>
                <AreaChart data={viewsChartData} margin={{ top: 5, right: 10, left: -20, bottom: 0 }}>
                  <defs>
                  ...
                  />
                  <Area type="monotone" dataKey="value" stroke="#6366f1" strokeWidth={2} fill="url(#colorViews)" />
                </AreaChart>
              </ResponsiveContainer>
            </div>
          )}
          ...
          {data.summary.totalResumes > 0 && (
            <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-4">
              <h3 className="text-xs font-semibold text-slate-600 dark:text-slate-400 mb-1">이력서 공개 현황</h3>
              <ResponsiveContainer width="100%" height={120}>
                <PieChart>
                  <Pie
                    data={pieData}
              ...
              <div className="flex justify-center gap-3 mt-1">
                {pieData.map((entry, i) => (
                  <div key={entry.name} className="flex items-center gap-1">
                    <span className="w-2 h-2 rounded-full inline-block" style={{ backgroundColor: CHART_COLORS[i] }} />
