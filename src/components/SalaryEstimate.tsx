...
const SKILL_PREMIUM: Record<string, number> = {
  react: 200, typescript: 200, python: 250, java: 180, 'node.js': 180,
  kubernetes: 300, docker: 150, aws: 250, terraform: 280, graphql: 150,
  tensorflow: 350, pytorch: 350, 'machine learning': 400, rust: 300,
  go: 250, 'next.js': 180, postgresql: 120, redis: 130, elasticsearch: 200,
};

function detectJobCategory(resume: Resume): JobCategory {
  const skills = resume.skills.map(s => s.items.toLowerCase()).join(' ');
  if (/react|node|python|java|typescript|개발|프론트|백엔드|devops|docker/.test(skills)) return 'IT';
...
function computeSalary(
  ...
  const currentSkills = resume.skills.flatMap(s => s.items.split(',').map(i => i.trim().toLowerCase()));
  ...
  const percentile = Math.min(95, Math.max(15, 30 + experienceYears * 8 + Math.min(currentSkills.length * 3, 20)));
...
function getNegotiationTips(resume: Resume, salary: SalaryResult): NegotiationTip[] {
  ...
  if (salary.mine > salary.marketAvg) {
    ...
    tips.push({
      ...
      description: '시장 수요가 높은 기술(클라우드, AI/ML)을 추가 학습하면 연봉 협상에서 유리합니다.',
      icon: '📈',
    });
  }
  ...
  tips.push({
    title: '타이밍을 활용하세요',
    description: '상반기(3-4월)와 하반기(9-10월) 채용 시즌에 이직하면 더 나은 조건을 받을 수 있습니다.',
    icon: '🕐',
  });
  ...
  tips.push({
    title: '데이터 기반 협상',
    description: '면접 전 해당 포지션의 연봉 범위를 조사하고, 본인 예상치의 10-15% 위에서 협상을 시작하세요.',
    icon: '💡',
  });
      ...
      {items.map(item => {
        ...
        return (
          <div key={item.label} className="flex items-center gap-2">
            <span className="text-xs text-slate-500 dark:text-slate-400 w-20 shrink-0 text-right">{item.label}</span>
            <div className="flex-1 h-6 bg-slate-100 dark:bg-slate-700 rounded overflow-hidden">
              <div className={`h-full rounded transition-all duration-700 ${item.color}`} style={{ width: `${pct}%` }} />
            </div>
...
function GrowthChart({ projections }: { projections: GrowthProjection[] }) {
  ...
  return (
    <div className="bg-slate-50 dark:bg-slate-900/50 rounded-lg p-3">
      <p className="text-xs font-semibold text-slate-600 dark:text-slate-400 mb-2">5년 연봉 성장 예측</p>
      <svg viewBox={`0 0 100 ${chartHeight}`} className="w-full" preserveAspectRatio="none" style={{ height: '120px' }}>
        <defs>
        ...
        {[0.25, 0.5, 0.75].map(frac => (
          <line key={frac} x1="0" x2="100" y1={chartHeight - frac * (chartHeight - 20) - 10} y2={chartHeight - frac * (chartHeight - 20) - 10}
            stroke="currentColor" className="text-slate-200 dark:text-slate-700" strokeWidth="0.3" />
        ))}
      ...
      <div className="flex justify-between mt-1">
        {projections.map((p, i) => (
          <div key={i} className="text-center">
            ...
            <p className="text-[10px] font-semibold text-slate-600 dark:text-slate-300">{p.salary.toLocaleString()}</p>
          </div>
        ))}
      </div>
    </div>
...
export default function SalaryEstimate({ resume }: Props) {
  ...
  const [activeTab, setActiveTab] = useState<'calculator' | 'compare' | 'tips' | 'projection'>('calculator');
  ...
  const tips = useMemo(
    () => getNegotiationTips(resume, salary),
    [resume, salary],
  );
  ...
  return (
    <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-4 no-print">
      <button onClick={() => setExpanded(!expanded)} className="w-full flex items-center justify-between">
        <h3 className="text-sm font-semibold text-slate-800 dark:text-slate-200">예상 연봉 & 급여 인텔리전스</h3>
        <div className="flex items-center gap-2">
          <span className="text-sm font-bold text-green-600">{salary.mine.toLocaleString()}만원</span>
        ...
        <div className="mt-3 space-y-4 animate-fade-in">
          ...
          {activeTab === 'calculator' && (
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="text-[10px] font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wider">직종</label>
                  <select
                    value={jobCategory}
                ...
                <div>
                  <label className="text-[10px] font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wider">경력년수</label>
                  <input
                    type="range" min={0} max={20} value={experienceYears}
                ...
                <div>
                  <label className="text-[10px] font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wider">지역</label>
                  <select
                    value={region}
                ...
                <div>
                  <label className="text-[10px] font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wider">기업규모</label>
                  <select
                    value={companySize}
              ...
              {(() => {
                ...
                return (
                  <div className="bg-emerald-50 dark:bg-emerald-900/20 rounded-lg p-2.5 border border-emerald-200 dark:border-emerald-800">
                    <p className="text-[10px] font-semibold text-emerald-700 dark:text-emerald-400 mb-1">기술 프리미엄 반영</p>
                    <div className="flex flex-wrap gap-1">
                      {premiumSkills.map(sk => (
                        <span key={sk.name} className="px-1.5 py-0.5 text-[10px] bg-emerald-100 dark:bg-emerald-900/40 text-emerald-700 dark:text-emerald-400 rounded">
          ...
          {activeTab === 'compare' && (
            <div className="space-y-4">
              ...
              <div className="bg-slate-50 dark:bg-slate-900/50 rounded-lg p-3">
                <p className="text-xs font-semibold text-slate-600 dark:text-slate-400 mb-3">나의 연봉 위치</p>
                <BarCompare
                  maxVal={Math.max(salary.mine, salary.marketAvg, salary.top25)}
                  items={[
                    { label: '나의 예상', value: salary.mine, color: 'bg-gradient-to-r from-emerald-400 to-emerald-500' },
                    { label: '시장 평균', value: salary.marketAvg, color: 'bg-gradient-to-r from-blue-400 to-blue-500' },
                    { label: '상위 25%', value: salary.top25, color: 'bg-gradient-to-r from-purple-400 to-purple-500' },
                  ]}
                />
                <div className="mt-2 text-center">
                  {salary.mine >= salary.marketAvg ? (
                    <p className="text-xs text-emerald-600 dark:text-emerald-400 font-medium">
                      시장 평균 대비 +{Math.round(((salary.mine - salary.marketAvg) / salary.marketAvg) * 100)}%
                    </p>
                  ) : (
                    <p className="text-xs text-amber-600 dark:text-amber-400 font-medium">
                      시장 평균 대비 -{Math.round(((salary.marketAvg - salary.mine) / salary.marketAvg) * 100)}%
                    </p>
                  )}
                </div>
              </div>
              ...
              {/* Industry comparison */}
              <div className="bg-slate-50 dark:bg-slate-900/50 rounded-lg p-3">
                <p className="text-xs font-semibold text-slate-600 dark:text-slate-400 mb-3">업종별 평균 연봉 비교 ({experienceYears}년차 기준)</p>
                <BarCompare
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-3 gap-2">
                <div className="text-center p-2 bg-slate-50 dark:bg-slate-900/50 rounded-lg">
                  ...
                  <p className="text-sm font-bold text-slate-700 dark:text-slate-300">{projections[0].salary.toLocaleString()}만</p>
                </div>
                <div className="text-center p-2 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
                  <p className="text-[10px] text-blue-500">3년 후</p>
                  <p className="text-sm font-bold text-blue-700 dark:text-blue-400">{projections[3].salary.toLocaleString()}만</p>
                </div>
                <div className="text-center p-2 bg-emerald-50 dark:bg-emerald-900/20 rounded-lg">
                  <p className="text-[10px] text-emerald-500">5년 후</p>
                  <p className="text-sm font-bold text-emerald-700 dark:text-emerald-400">{projections[5].salary.toLocaleString()}만</p>
                </div>
              </div>

              <p className="text-[10px] text-slate-400 text-center">
                예상 5년 누적 성장률: +{Math.round(((projections[5].salary - projections[0].salary) / projections[0].salary) * 100)}%
              </p>
            </div>
          )}
          ...
          {activeTab === 'tips' && (
            <div className="space-y-2">
              <p className="text-xs font-semibold text-slate-600 dark:text-slate-400">연봉 협상 팁</p>
              {tips.map((tip, i) => (
                <div key={i} className="flex gap-2.5 p-2.5 bg-slate-50 dark:bg-slate-900/50 rounded-lg border border-slate-100 dark:border-slate-700">
                  <span className="text-lg shrink-0 mt-0.5">{tip.icon}</span>
                  <div>
                    <p className="text-xs font-semibold text-slate-700 dark:text-slate-300">{tip.title}</p>
                    <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-0.5 leading-relaxed">{tip.description}</p>
                  </div>
                </div>
              ))}
            </div>
