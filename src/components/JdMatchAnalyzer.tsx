    ...
    <div className="space-y-3">
        ...
        <div className="flex-1 relative">
          <div className="bg-blue-50 dark:bg-blue-900/30 border-2 border-blue-300 dark:border-blue-700 rounded-2xl p-3 min-h-[80px]">
            ...
            <div className="flex flex-wrap gap-1">
              {onlyResume.map((s, i) => (
                <span key={i} className="px-2 py-0.5 text-xs bg-blue-100 dark:bg-blue-800 text-blue-700 dark:text-blue-300 rounded-full">{s}</span>
              ))}
          ...
          <div className="bg-green-50 dark:bg-green-900/30 border-2 border-green-300 dark:border-green-700 rounded-2xl p-3 min-h-[80px]">
            ...
            <div className="flex flex-wrap gap-1">
              {skillsGap.overlap.map((s, i) => (
                <span key={i} className="px-2 py-0.5 text-xs bg-green-100 dark:bg-green-800 text-green-700 dark:text-green-300 rounded-full font-medium">{s}</span>
              ))}
          ...
          <div className="bg-red-50 dark:bg-red-900/30 border-2 border-red-300 dark:border-red-700 rounded-2xl p-3 min-h-[80px]">
            ...
            <div className="flex flex-wrap gap-1">
              {onlyRequired.map((s, i) => (
                <span key={i} className="px-2 py-0.5 text-xs bg-red-100 dark:bg-red-800 text-red-700 dark:text-red-300 rounded-full">{s}</span>
              ))}
      ...
      <div>
        <div className="flex justify-between text-xs text-slate-500 mb-1">
          <span>기술 커버리지</span>
          <span>{skillsGap.requiredSkills.length > 0 ? Math.round((skillsGap.overlap.length / skillsGap.requiredSkills.length) * 100) : 0}%</span>
        </div>
        <div className="h-2 bg-slate-100 dark:bg-slate-700 rounded-full overflow-hidden">
          <div
            className="h-full bg-gradient-to-r from-green-400 to-green-600 rounded-full transition-all duration-700"
            style={{ width: `${skillsGap.requiredSkills.length > 0 ? (skillsGap.overlap.length / skillsGap.requiredSkills.length) * 100 : 0}%` }}
          />
        </div>
      </div>
    </div>
...
export default function JdMatchAnalyzer({ resumeId, resume, onClose }: Props) {
  ...
  const runAnalysis = async () => {
    ...
    try {
      ...
      const jdWords = jd.toLowerCase().replace(/[^\w가-힣\s]/g, ' ').split(/\s+/).filter(w => w.length > 2);
      ...
      const verdict = yearsMatch
        ? (totalYears >= parseInt(yearsMatch[1]) ? '충족' : '부족')
        : '확인 필요';
  ...
  const runCoverLetter = async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error((await res.json().catch(() => ({}))).message || '자소서 생성 실패');
      const data = await res.json();
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-end" role="dialog" aria-label="JD 매칭 분석">
      <div className="absolute inset-0 bg-black/30" onClick={onClose} />
      <div className="relative w-full max-w-2xl h-full bg-white dark:bg-slate-800 shadow-2xl overflow-y-auto">
        {/* Header */}
        <div className="sticky top-0 bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 px-4 py-3 flex items-center justify-between z-10">
          <h2 className="font-bold text-slate-900 dark:text-slate-100">JD 매칭 분석기</h2>
          <button onClick={onClose} className="p-1 text-slate-400 hover:text-slate-600 rounded text-xl" aria-label="닫기">&times;</button>
        </div>
          ...
          {([
            ...
            >
              ...
              {t.id === 'history' && history.length > 0 && (
                <span className="ml-1 text-xs bg-slate-200 dark:bg-slate-600 text-slate-600 dark:text-slate-300 px-1.5 py-0.5 rounded-full">{history.length}</span>
              )}
            </button>
          ))}
        </div>
            ...
            <div className="space-y-5">
              ...
              <div>
                ...
                <div className="flex items-center justify-between mt-2">
                  <span className="text-xs text-slate-400">{jd.length}자{jd.length < 20 && jd.length > 0 ? ' (최소 20자 이상 입력)' : ''}</span>
                  <button
                    onClick={runAnalysis}
              ...
              {result && (
                <div className="space-y-5 animate-fade-in">
                  ...
                  <div className="text-center p-6 bg-gradient-to-br from-slate-50 to-blue-50 dark:from-slate-900/50 dark:to-blue-900/20 rounded-2xl">
                    ...
                    <div className="mt-3">
                      <span className="text-lg font-bold text-slate-800 dark:text-slate-200">{result.matchGrade} 매칭</span>
                    </div>
                    <p className="text-sm text-slate-600 dark:text-slate-400 mt-2 max-w-md mx-auto">{result.summary}</p>
                  </div>

                  {/* Keyword Match */}
                  <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl p-4">
                    <h4 className="text-sm font-semibold text-slate-800 dark:text-slate-200 mb-3">키워드 매칭</h4>
                    <div className="space-y-3">
                      <div>
                        <span className="text-xs font-medium text-green-700 dark:text-green-400 mb-1 block">발견된 키워드 ({result.keywords.found.length})</span>
                        <div className="flex flex-wrap gap-1.5">
                          {result.keywords.found.slice(0, 15).map((kw, i) => (
                            <span key={i} className="px-2 py-1 text-xs bg-green-100 dark:bg-green-900/40 text-green-700 dark:text-green-400 rounded-full">{kw}</span>
                          ))}
                        </div>
                      </div>
                      <div>
                        <span className="text-xs font-medium text-red-700 dark:text-red-400 mb-1 block">누락된 키워드 ({result.keywords.missing.length})</span>
                        <div className="flex flex-wrap gap-1.5">
                          {result.keywords.missing.slice(0, 15).map((kw, i) => (
                            <span key={i} className="px-2 py-1 text-xs bg-red-100 dark:bg-red-900/40 text-red-700 dark:text-red-400 rounded-full">{kw}</span>
                          ))}
                        </div>
                      </div>
                    </div>
                  ...
                  {/* Skills Gap - Venn */}
                  <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl p-4">
                    <h4 className="text-sm font-semibold text-slate-800 dark:text-slate-200 mb-3">기술 스택 갭 분석</h4>
                    <SkillsVenn skillsGap={result.skillsGap} />
                  </div>

                  {/* Experience Match */}
                  <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl p-4">
                    <h4 className="text-sm font-semibold text-slate-800 dark:text-slate-200 mb-3">경력 매칭</h4>
                    <div className="grid grid-cols-3 gap-3 text-center">
                      <div className="p-3 bg-slate-50 dark:bg-slate-700 rounded-lg">
                        <div className="text-xs text-slate-500 dark:text-slate-400 mb-1">요구 경력</div>
                        <div className="text-sm font-bold text-slate-800 dark:text-slate-200">{result.experienceMatch.required}</div>
                      </div>
                      <div className="p-3 bg-slate-50 dark:bg-slate-700 rounded-lg">
                        <div className="text-xs text-slate-500 dark:text-slate-400 mb-1">보유 경력</div>
                        <div className="text-sm font-bold text-slate-800 dark:text-slate-200">{result.experienceMatch.actual}</div>
                      </div>
                      <div className={`p-3 rounded-lg ${
                        ...
                        <div className="text-xs text-slate-500 dark:text-slate-400 mb-1">판정</div>
                        <div className={`text-sm font-bold ${
                          result.experienceMatch.verdict === '충족'
                            ...
                            : result.experienceMatch.verdict === '부족'
                              ? 'text-red-600'
                              : 'text-amber-600'
                        }`}>{result.experienceMatch.verdict}</div>
                      </div>
                    </div>
                  </div>
                  ...
                  {result.recommendations && result.recommendations.length > 0 && (
                    <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl p-4">
                      <h4 className="text-sm font-semibold text-slate-800 dark:text-slate-200 mb-3">수정 제안</h4>
                  ...
                  <div className="flex gap-3">
                    ...
                    >
                      {optimizing ? (
                        <><span className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />최적화 중...</>
                      ) : '맞춤 이력서 생성'}
                    </button>
                    <button
              ...
              {!coverLetter ? (
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    <h3 className="text-sm font-semibold text-slate-800 dark:text-slate-200">생성된 자소서</h3>
            ...
            <div className="space-y-4">
              {history.length === 0 ? (
                <div className="text-center py-12">
                  ...
                  <p className="text-sm text-slate-500 dark:text-slate-400">아직 JD 매칭 히스토리가 없습니다.</p>
                </div>
              ) : (
                <>
                  <div className="flex justify-between items-center">
                    <span className="text-sm text-slate-500">{history.length}개 기록</span>
                    <button onClick={clearHistory} className="text-xs text-red-500 hover:text-red-700 transition-colors">전체 삭제</button>
                  </div>
                  <div className="space-y-2">
                    {history.map(entry => {
                      const scoreColor = entry.score >= 80 ? 'text-green-600 bg-green-50' : entry.score >= 60 ? 'text-amber-600 bg-amber-50' : 'text-red-600 bg-red-50';
                      return (
                        <div key={entry.id} className="flex items-center gap-3 p-3 bg-slate-50 dark:bg-slate-700/50 rounded-xl border border-slate-200 dark:border-slate-600">
                          <div className={`w-12 h-12 rounded-xl flex items-center justify-center font-bold text-lg ${scoreColor}`}>
                            {entry.score}
                          </div>
                          <div className="flex-1 min-w-0">
                            <p className="text-sm text-slate-700 dark:text-slate-300 truncate">{entry.jdSnippet}</p>
                            <p className="text-xs text-slate-400 mt-0.5">
                              {new Date(entry.date).toLocaleDateString('ko-KR')} &middot; {entry.grade} 등급
                            </p>
                          </div>
                        </div>
                      );
