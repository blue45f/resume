...
function BenchmarkBadge({ scorePct, benchmark }: { scorePct: number; benchmark: number }) {
  ...
  return (
    <span className={`text-[10px] font-semibold ${isPositive ? 'text-green-600 dark:text-green-400' : 'text-red-500 dark:text-red-400'}`}>
      평균 대비 {isPositive ? '+' : ''}{diff}%
    </span>
  );
}
            ...
            return (
              <div key={s.label} className="group">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[10px] text-slate-400 dark:text-slate-500 w-8 text-right tabular-nums">{scorePct}%</span>
                </div>

                {/* Actionable tip */}
                {tip && (
                  <div className="mt-1 flex items-start gap-1.5">
                    <svg className="w-3 h-3 text-amber-500 mt-0.5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}>
                      <path strokeLinecap="round" strokeLinejoin="round" d="M12 18v-5.25m0 0a6.01 6.01 0 001.5-.189m-1.5.189a6.01 6.01 0 01-1.5-.189m3.75 7.478a12.06 12.06 0 01-4.5 0m3.75 2.383a14.406 14.406 0 01-3 0M14.25 18v-.192c0-.983.658-1.823 1.508-2.316a7.5 7.5 0 10-7.517 0c.85.493 1.509 1.333 1.509 2.316V18" />
                    </svg>
                    <p className="text-[11px] text-amber-600 dark:text-amber-400 leading-snug">{tip}</p>
                  </div>
                )}
              </div>
            );
        ...
        <div className="px-3 py-2.5 mb-4 rounded-lg bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/20 border border-blue-100 dark:border-blue-800">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-xs font-semibold text-blue-700 dark:text-blue-400">업계 평균 대비</span>
            </div>
            <span className={`text-sm font-bold ${result.percentage >= 54 ? 'text-green-600 dark:text-green-400' : 'text-red-500 dark:text-red-400'}`}>
              {result.percentage >= 54 ? '+' : ''}{result.percentage - 54}%
            </span>
          </div>
