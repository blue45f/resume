...
export default function SkillEndorsement({ resumeId, skills }: Props) {
  ...
  const allSkillNames = skills.flatMap(s =>
    s.items.split(',').map(item => item.trim()).filter(Boolean),
  );
      ...
      try {
        ...
        if (res.ok) {
          ...
          setEndorsements(prev => ({
            ...prev,
            [skill]: {
              count: result.count ?? (prev[skill]?.endorsed ? (prev[skill]?.count || 1) - 1 : (prev[skill]?.count || 0) + 1),
  ...
  return (
    <div className="mt-4 no-print">
      <div className="flex items-center gap-2 mb-3">
        ...
        {!user && (
          <span className="text-xs text-slate-400 dark:text-slate-500">로그인하면 추천할 수 있습니다</span>
        )}
      </div>
