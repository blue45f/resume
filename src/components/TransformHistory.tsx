...
export default function TransformHistory({ resumeId }: Props) {
  ...
  useEffect(() => {
    fetchTransformHistory(resumeId).then(setHistory).catch(() => {});
  }, [resumeId]);
        ...
        <div className="mt-3 space-y-2 animate-fade-in">
          {history.map(h => (
            ...
            >
              <div className="flex items-center justify-between">
                <span className="text-slate-700 dark:text-slate-300 font-medium">{h.model || 'AI'}</span>
          ...
          {selected && (
            <div className="p-3 bg-slate-50 dark:bg-slate-900 rounded-lg border border-slate-200 dark:border-slate-700 animate-fade-in">
              ...
              <pre className="text-xs text-slate-600 dark:text-slate-300 whitespace-pre-wrap max-h-60 overflow-y-auto">{selected.text}</pre>
            </div>
          )}
        </div>
      )}
