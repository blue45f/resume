...
const INDUSTRY_BADGES: Record<string, { label: string; color: string }> = {
  IT: { label: 'IT/소프트웨어', color: 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400' },
  finance: { label: '금융', color: 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400' },
  manufacturing: { label: '제조', color: 'bg-orange-100 text-orange-700 dark:bg-orange-900/30 dark:text-orange-400' },
  default: { label: '기타', color: 'bg-slate-100 text-slate-600 dark:bg-slate-700 dark:text-slate-400' },
};

const SIZE_BADGES = [
  { min: 10, label: '스타트업', color: 'bg-violet-100 text-violet-700 dark:bg-violet-900/30 dark:text-violet-400' },
  { min: 5, label: '중소기업', color: 'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400' },
  { min: 2, label: '중견기업', color: 'bg-cyan-100 text-cyan-700 dark:bg-cyan-900/30 dark:text-cyan-400' },
  { min: 0, label: '기업', color: 'bg-slate-100 text-slate-600 dark:bg-slate-700 dark:text-slate-400' },
];
...
function detectIndustry(job: JobPost): string {
  ...
  if (/react|node|python|java|개발|프론트|백엔드|devops|cloud|aws|데이터|ai|ml/.test(text)) return 'IT';
      ...
      <div className="flex items-start gap-3 mb-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          {job.user?.companyName && (
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">{job.user.companyName}</p>
          )}
        </div>
      </div>
        ...
        <div>
          ...
          <div className="space-y-1.5 max-h-32 overflow-y-auto">
            {otherJobs.slice(0, 5).map(oj => (
              ...
              >
                <p className="text-xs font-medium text-slate-700 dark:text-slate-300 truncate">{oj.position}</p>
                <p className="text-xs text-slate-400 truncate">{oj.location} {oj.salary && `· ${oj.salary}`}</p>
              </button>
            ))}
            {otherJobs.length > 5 && (
              <p className="text-xs text-blue-500 text-center py-1">+{otherJobs.length - 5}개 더 보기</p>
            )}
          </div>
        </div>
      )}
