...
const CATEGORY_META: Record<Category, { label: string; icon: string }> = {
  ...
  ats: { label: 'ATS 호환성', icon: 'M9.75 17L9 20l-1 1h8l-1-1-.75-3M3 13h18M5 17h14a2 2 0 002-2V5a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z' },
  readability: { label: '가독성', icon: 'M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z' },
};

const SEVERITY_STYLES: Record<Severity, { bg: string; border: string; text: string; badge: string }> = {
  info: {
...
function computeLocalTips(data: ResumeData, activeTab: string): CoachTip[] {
  ...
  // --- Completeness ---
  if (!pi.name) {
    tips.push({ id: 'c-name', category: 'completeness', severity: 'critical', title: '이름 미입력', description: '이름은 이력서의 필수 항목입니다.' });
  }
  if (!pi.email) {
    tips.push({ id: 'c-email', category: 'completeness', severity: 'critical', title: '이메일 미입력', description: '채용 담당자가 연락할 수 있도록 이메일을 입력하세요.' });
  }
  if (!pi.phone) {
    tips.push({ id: 'c-phone', category: 'completeness', severity: 'warning', title: '전화번호 미입력', description: '전화번호를 입력하면 면접 연락을 빠르게 받을 수 있습니다.' });
  }
  if (!stripHtml(pi.summary)) {
    tips.push({ id: 'c-summary', category: 'completeness', severity: 'warning', title: '자기소개 미작성', description: '자기소개는 첫인상을 결정합니다. 3~5문장으로 핵심 역량을 요약하세요.' });
  } else if (stripHtml(pi.summary).length < 50) {
    tips.push({ id: 'c-summary-short', category: 'completeness', severity: 'info', title: '자기소개가 짧습니다', description: '50자 이상으로 작성하면 더 효과적입니다. 핵심 기술과 경력 목표를 포함하세요.' });
  }
  if (data.experiences.length === 0) {
    tips.push({ id: 'c-exp', category: 'completeness', severity: 'warning', title: '경력 미입력', description: '경력사항을 추가하면 이력서 완성도가 높아집니다.' });
  }
  if (data.skills.length === 0) {
    tips.push({ id: 'c-skills', category: 'completeness', severity: 'warning', title: '기술 스택 미입력', description: 'ATS 시스템이 키워드를 인식하려면 기술을 반드시 입력해야 합니다.' });
  }
  if (data.educations.length === 0) {
    tips.push({ id: 'c-edu', category: 'completeness', severity: 'info', title: '학력 미입력', description: '학력 정보를 추가하면 이력서가 더 완성도 있어집니다.' });
  }
  if (!pi.github && !pi.website) {
    tips.push({ id: 'c-links', category: 'completeness', severity: 'info', title: '포트폴리오 링크 없음', description: 'GitHub이나 개인 웹사이트를 추가하면 기술력을 어필할 수 있습니다.' });
  }
    ...
    data.experiences.forEach((exp, i) => {
      ...
      if (desc.length > 0 && desc.length < 50) {
        tips.push({
          ...
          description: '이 경력에 대해 더 자세히 작성해보세요. STAR 기법(상황-과제-행동-결과)으로 작성하면 더 효과적입니다.',
        });
      }
      if (!exp.achievements || stripHtml(exp.achievements).length === 0) {
        tips.push({
          ...
          severity: 'info',
          title: `경력 ${i + 1}: 성과 미작성`,
          description: '정량적 성과(예: "매출 30% 증가", "응답 시간 50% 단축")를 추가하면 설득력이 높아집니다.',
        });
      }
      if (!exp.techStack) {
        tips.push({
  ...
  // --- ATS tips ---
  const allText = [pi.summary, ...data.experiences.map(e => e.description), ...data.skills.map(s => s.items)].join(' ');
  if (pi.summary && /<img|<table|<iframe/i.test(pi.summary)) {
    tips.push({ id: 'ats-html', category: 'ats', severity: 'critical', title: 'ATS 비호환 콘텐츠', description: '이미지나 표는 ATS가 인식하지 못합니다. 텍스트 기반으로 작성하세요.' });
  }
  if (allText.length > 0 && !/[a-zA-Z]/.test(allText) && data.skills.length > 0) {
    tips.push({ id: 'ats-eng', category: 'ats', severity: 'info', title: '영문 키워드 부재', description: '글로벌 ATS는 영문 기술명을 인식합니다. "리액트" 대신 "React"도 병기하세요.' });
  }

  // --- Readability ---
  if (stripHtml(pi.summary).length > 500) {
    tips.push({ id: 'read-summary-long', category: 'readability', severity: 'info', title: '자기소개가 길어요', description: '300자 내외가 적당합니다. 핵심만 간결하게 정리하세요.' });
  }
...
function computeSectionScores(data: ResumeData): SectionScore[] {
  ...
  if (data.experiences.length > 0) {
    ...
    scores.push({ section: 'experience', label: '경력', score: Math.round(expScores.reduce((a, b) => a + b, 0) / expScores.length) });
  } else {
  ...
  if (data.skills.length > 0) {
    ...
    scores.push({ section: 'skills', label: '기술', score: Math.round((filled / data.skills.length) * 100) });
  } else {
  ...
  if (data.educations.length > 0) {
    ...
    scores.push({ section: 'education', label: '학력', score: Math.round(eduScores.reduce((a, b) => a + b, 0) / eduScores.length) });
  } else {
  ...
  if (data.projects.length > 0) {
    ...
    scores.push({ section: 'projects', label: '프로젝트', score: Math.round(projScores.reduce((a, b) => a + b, 0) / projScores.length) });
  }
...
function ScoreBar({ score }: { score: number }) {
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <span className="text-xs font-medium text-slate-600 dark:text-slate-400 w-8 text-right">{score}</span>
    </div>
  );
}
...
- 기술: ${data.skills.map(s => s.items).join(', ').substring(0, 200) || '미입력'}
...
- 프로젝트 수: ${data.projects.length}개`;
      ...
      if (jsonMatch) {
        const parsed = JSON.parse(jsonMatch[0]) as Array<{ category: Category; severity: Severity; title: string; description: string }>;
    ...
    <FeatureGate feature="aiCoaching" fallback={null}>
      ...
      {!open && (
        ...
        >
          ...
          <span className="writing-vertical text-xs font-medium tracking-wider hidden sm:block" style={{ writingMode: 'vertical-rl' }}>AI 코치</span>
          {criticalCount > 0 && (
      ...
      {open && (
        <>
          ...
          <div className={`fixed z-50 bg-white dark:bg-slate-800 shadow-2xl border-l border-slate-200 dark:border-slate-700 flex flex-col transition-all duration-300
          ...
          >
            ...
            <div className="flex items-center justify-between px-4 py-3 border-b border-slate-200 dark:border-slate-700 bg-gradient-to-r from-purple-50 to-blue-50 dark:from-purple-900/20 dark:to-blue-900/20 shrink-0">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-sm font-bold text-slate-800 dark:text-slate-200">AI 이력서 코치</h2>
              </div>
              <button
            ...
            <div className="flex-1 overflow-y-auto">
              ...
              <div className="px-4 py-3 border-b border-slate-100 dark:border-slate-700">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-xs font-medium text-slate-500 dark:text-slate-400">전체 완성도</span>
                  <span className={`text-lg font-bold ${overallScore >= 80 ? 'text-green-600' : overallScore >= 50 ? 'text-amber-600' : 'text-red-600'}`}>
                    {overallScore}점
                  </span>
                </div>
              ...
              <div className="px-4 py-3 space-y-2">
                {filteredTips.length === 0 && (
                  <div className="text-center py-6">
                    <svg className="w-10 h-10 mx-auto text-green-400 mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.5}>
                      <path strokeLinecap="round" strokeLinejoin="round" d="M9 12.75L11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                    </svg>
                    <p className="text-sm text-slate-500 dark:text-slate-400">
                      {activeCategory === 'all' ? '모든 항목이 양호합니다!' : '이 카테고리에 개선 사항이 없습니다.'}
                    </p>
                  </div>
                )}
                {filteredTips.map(tip => {
                  ...
                  return (
                    <div key={tip.id} className={`p-3 rounded-lg border ${style.bg} ${style.border} transition-all duration-200`}>
                      <div className="flex items-start gap-2">
                        <svg className={`w-4 h-4 mt-0.5 shrink-0 ${style.text}`} fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}>
                          <path strokeLinecap="round" strokeLinejoin="round" d={catMeta.icon} />
                        </svg>
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-1.5 mb-0.5">
                            <span className={`text-xs font-semibold ${style.text}`}>{tip.title}</span>
                            <span className={`text-[10px] px-1.5 py-0.5 rounded-full font-medium ${style.badge}`}>
                              {tip.severity === 'critical' ? '필수' : tip.severity === 'warning' ? '권장' : '참고'}
                            </span>
                          </div>
                          <p className="text-xs text-slate-600 dark:text-slate-400 leading-relaxed">{tip.description}</p>
                          {tip.fixAction && (
                            <button
            ...
            <div className="shrink-0 px-4 py-3 border-t border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900/50">
              ...
              <p className="text-[10px] text-slate-400 dark:text-slate-500 text-center mt-1">LLM을 사용하여 더 상세한 개선안을 제공합니다</p>
            </div>
          </div>
        </>
      )}
