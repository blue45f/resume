...
const JOBSEEKER_ITEMS: NavItem[] = [
  ...
  },
  {
    ...
    match: (p) => ['/settings', '/notifications', '/messages', '/bookmarks', '/applications', '/jobs'].some(r => p.startsWith(r)),
    activeColor: 'text-blue-600 dark:text-blue-400',
  },
];
...
export default function MobileBottomNav() {
  ...
  const activeColor = isRecruiter ? 'text-purple-600 dark:text-purple-400' : 'text-blue-600 dark:text-blue-400';
    ...
    <nav className="mobile-bottom-nav no-print" aria-label="모바일 탐색">
      {items.map(item => {
        ...
        return (
          ...
          >
            {item.highlight ? (
              <span className={`w-10 h-10 ${highlightGradient} rounded-full flex items-center justify-center shadow-lg -mt-3`}>
                {item.icon(false)}
              </span>
            ) : (
              item.icon(active)
