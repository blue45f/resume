...
const STAGES = [
  { key: 'applied', label: '지원완료', color: 'bg-blue-500', lightColor: 'bg-blue-100 dark:bg-blue-900/30', textColor: 'text-blue-600 dark:text-blue-400', avgDays: 0 },
  { key: 'screening', label: '서류심사', color: 'bg-purple-500', lightColor: 'bg-purple-100 dark:bg-purple-900/30', textColor: 'text-purple-600 dark:text-purple-400', avgDays: 7 },
  { key: 'interview', label: '면접', color: 'bg-amber-500', lightColor: 'bg-amber-100 dark:bg-amber-900/30', textColor: 'text-amber-600 dark:text-amber-400', avgDays: 14 },
  { key: 'offer', label: '최종합격', color: 'bg-green-500', lightColor: 'bg-green-100 dark:bg-green-900/30', textColor: 'text-green-600 dark:text-green-400', avgDays: 7 },
];

const TERMINAL_STAGES: Record<string, { label: string; color: string; lightColor: string; textColor: string }> = {
  rejected: { label: '불합격', color: 'bg-red-500', lightColor: 'bg-red-100 dark:bg-red-900/30', textColor: 'text-red-600 dark:text-red-400' },
  withdrawn: { label: '취소', color: 'bg-slate-400', lightColor: 'bg-slate-100 dark:bg-slate-700', textColor: 'text-slate-500 dark:text-slate-400' },
};
...
export default function ApplicationTimeline({ applicationId, status, appliedDate, createdAt, updatedAt }: Props) {
  ...
  const timeline = useMemo(() => {
    ...
    const entries: TimelineEntry[] = STAGES.map((stage, i) => {
      ...
      if (date && i > 0) {
        ...
        if (prevDate) {
          daysInStage = Math.max(0, Math.round((new Date(date).getTime() - new Date(prevDate).getTime()) / 86400000));
        }
      }
      ...
      if (isCurrent) {
        ...
        daysInStage = Math.max(0, Math.round((Date.now() - new Date(entryDate).getTime()) / 86400000));
      }
  ...
  const barColor = status === 'rejected'
  ...
  return (
    <div className="mt-3">
      ...
      <div className="flex items-center gap-2 mb-1">
        ...
        >
          {totalDays}일 경과
          <svg className={`w-3 h-3 inline ml-0.5 transition-transform ${expanded ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <div key={entry.key} className="flex items-start gap-3 pb-3 last:pb-0">
                ...
                <div className={`flex-1 -mt-0.5 ${entry.isCurrent ? '' : 'opacity-75'}`}>
                  <div className="flex items-center gap-2">
                    <span className={`text-xs font-semibold ${entry.isActive ? entry.textColor : 'text-slate-400'}`}>
                      {entry.label}
                    </span>
                    {entry.isCurrent && !entry.isTerminal && (
                      <span className={`px-1.5 py-0.5 text-xs rounded-full ${entry.lightColor} ${entry.textColor} font-medium animate-pulse`}>
                        현재
                      </span>
                    )}
                    {entry.isTerminal && (
                      <span className={`px-1.5 py-0.5 text-xs rounded-full ${entry.lightColor} ${entry.textColor} font-medium`}>
                        종료
                      </span>
                    )}
                  </div>
            ...
            <div className="mt-2 pt-2 border-t border-dashed border-slate-200 dark:border-slate-700">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs text-slate-500 dark:text-slate-400">
                  다음 단계: <span className="font-medium text-slate-700 dark:text-slate-300">{nextStep.label}</span>
                  {nextStep.avgDays > 0 && (
