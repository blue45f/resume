...
export default function ShareMenu({ url, title, description }: ShareMenuProps) {
  ...
  const shareKakao = () => {
    navigator.clipboard.writeText(url);
    toast('링크가 복사되었습니다. 카카오톡에서 붙여넣기하세요.', 'success');
    setOpen(false);
  };
    ...
    <div className="relative" ref={ref}>
      ...
      {open && (
        <div className="absolute right-0 top-full mt-1 w-44 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl shadow-lg z-50 py-1 animate-scale-in">
          <button onClick={copyLink} className="w-full px-3 py-2 text-left text-sm text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 flex items-center gap-2">
          ...
          {navigator.share && (
            <button onClick={nativeShare} className="w-full px-3 py-2 text-left text-sm text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 flex items-center gap-2">
          ...
          <button onClick={shareKakao} className="w-full px-3 py-2 text-left text-sm text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 flex items-center gap-2">
            <span className="w-4 h-4 flex items-center justify-center text-xs font-bold text-yellow-600">K</span>
            카카오톡
          </button>
          <button onClick={shareTwitter} className="w-full px-3 py-2 text-left text-sm text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 flex items-center gap-2">
            <span className="w-4 h-4 flex items-center justify-center text-xs font-bold text-sky-500">X</span>
            X (Twitter)
          </button>
          <button onClick={shareLinkedIn} className="w-full px-3 py-2 text-left text-sm text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 flex items-center gap-2">
            <span className="w-4 h-4 flex items-center justify-center text-xs font-bold text-blue-700">in</span>
            LinkedIn
          </button>
          <button onClick={shareEmail} className="w-full px-3 py-2 text-left text-sm text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 flex items-center gap-2">
