...
import { transformResumeStream, localTransform, fetchPresets, fetchTemplates, fetchLlmProviders } from '@/lib/api';
...
export default function LlmTransformPanel({ resumeId, onClose }: Props) {
  ...
  useEffect(() => {
    ...
    fetchLlmProviders(resumeId).then(providers => {
  ...
  const handleLocalTransform = useCallback(async () => {
    ...
    setResult('');
    try {
      const data = localSource === 'template' && selectedTemplate
