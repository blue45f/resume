...
function ThemeSeparator({ themeId }: { themeId: string }) {
  switch (themeId) {
    ...
    case 'creative':
    case 'startup':
      return <div className="h-1 my-6 rounded-full bg-gradient-to-r from-purple-400 via-pink-400 to-orange-300 opacity-60" />;
    case 'tech':
      return <div className="my-5 font-mono text-[10px] text-slate-400 tracking-widest text-center select-none">/* ─────────────────────────────── */</div>;
    case 'elegant':
      return <div className="my-6 text-center text-amber-400 tracking-[1em] text-xs select-none">&bull; &bull; &bull;</div>;
    case 'newspaper':
      return (
        <div className="my-5">
    ...
    case 'portfolio':
      return <div className="h-1.5 my-7 rounded-full bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 opacity-50" />;
    case 'professional':
...
/* ------------------------------------------------------------------ */
function SkillsDisplay({ skills, themeId, accentColor }: { skills: SkillType[]; themeId: string; accentColor: string }) {
  ...
  if (['modern', 'startup', 'dark', 'pastel'].includes(themeId)) {
    ...
    return (
      <div className="space-y-3">
        {skills.map(skill => (
          <div key={skill.id}>
            <div className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-1.5">{skill.category}</div>
            <div className="flex flex-wrap gap-1.5">
              {skill.items.split(',').map((item, i) => (
                <span key={i} className={`px-2.5 py-1 text-xs font-medium rounded-full border ${pillColor}`}>{item.trim()}</span>
              ))}
            </div>
          </div>
        ))}
        ...
        {skills.map(skill => (
          <div key={skill.id}>
            <div className="text-xs font-bold text-[#7ee787] font-mono mb-1.5">
              <span className="text-[#484f58]">const </span>{skill.category.replace(/\s+/g, '_').toLowerCase()}<span className="text-[#484f58]"> = </span>[
            </div>
            <div className="flex flex-wrap gap-1.5 ml-4">
              {skill.items.split(',').map((item, i) => (
                <span key={i} className="px-2 py-0.5 text-xs font-mono bg-[#161b22] text-[#79c0ff] border border-[#30363d] rounded">{`"${item.trim()}"`}</span>
              ))}
            </div>
  ...
  if (['elegant', 'executive'].includes(themeId)) {
    return (
      <div className="space-y-3">
        {skills.map(skill => (
          <div key={skill.id}>
            <div className="text-xs font-semibold text-slate-700 uppercase tracking-widest mb-1.5" style={{ fontFamily: "'Georgia', serif" }}>{skill.category}</div>
            <ul className="list-none space-y-0.5 ml-1">
              {skill.items.split(',').map((item, i) => (
                <li key={i} className="text-sm text-slate-600 flex items-start gap-2" style={{ fontFamily: "'Georgia', serif" }}>
  ...
  if (themeId === 'minimal') {
    return (
      <div className="space-y-4">
        {skills.map(skill => (
          <div key={skill.id}>
            <div className="text-[10px] text-slate-400 uppercase tracking-[0.3em] mb-1">{skill.category}</div>
...
/* ------------------------------------------------------------------ */
function TechTags({ text, color = 'slate', themeId }: { text: string; color?: 'slate' | 'blue'; themeId?: string }) {
  if (themeId === 'tech') {
    return (
      <div className="flex flex-wrap gap-1 mt-1.5">
        {text.split(',').map((tag, i) => (
          <span key={i} className="px-1.5 py-0.5 text-xs font-mono bg-[#161b22] text-[#79c0ff] border border-[#30363d] rounded">{tag.trim()}</span>
        ))}
      </div>
    );
  }
...
/* ------------------------------------------------------------------ */
function Section({ title, children, theme, isFirst }: { title: string; children: React.ReactNode; theme: ResumeTheme; isFirst?: boolean }) {
...
function ContactInfo({ pi, theme }: { pi: Resume['personalInfo']; theme: ResumeTheme }) {
  const isDarkHeader = ['professional', 'startup', 'tech', 'creative', 'dark', 'portfolio'].includes(theme.id);
  return (
    <div className="space-y-1 text-sm">
      <div className={`flex flex-wrap items-center gap-x-1 gap-y-0.5 ${isDarkHeader ? 'text-white/80' : 'text-slate-600'}`}>
        {[
      ...
      {(pi.website || pi.github || (pi.links && pi.links.length > 0)) && (
        <div className={`flex flex-wrap items-center gap-x-1 gap-y-0.5 ${isDarkHeader ? 'text-white/70' : ''}`}>
          {pi.website && (
...
function ExperienceBlock({ exp, themeId }: { exp: Resume['experiences'][0]; themeId: string }) {
  return (
    <div>
      ...
      {exp.achievements && (
        <div className={`mt-2 pl-3 border-l-2 py-1.5 pr-2 rounded-r ${
          themeId === 'tech' ? 'border-[#238636] bg-[#0d1117]/5' :
          themeId === 'creative' ? 'border-purple-300 bg-purple-50/50' :
          'border-blue-300 bg-blue-50/50'
        }`}>
          <SafeHtml html={exp.achievements} className={`text-sm leading-relaxed break-words ${
...
/* ------------------------------------------------------------------ */
function RemainingContent({ resume, theme, sectionIndex }: { resume: Resume; theme: ResumeTheme; sectionIndex: number }) {
  ...
  const headerContent = (
    <div className={hasPhoto ? 'flex gap-5' : ''}>
      ...
      <div className="flex-1 min-w-0">
        <h1 className={`text-2xl sm:text-3xl font-extrabold tracking-tight ${isDarkHeader ? 'text-white' : 'text-slate-900'}`}>
          {pi.name || '이름'}
        </h1>
      ...
      <div ref={ref} className="resume-preview bg-white max-w-[210mm] mx-auto shadow-lg print:shadow-none overflow-hidden" style={{ fontFamily: theme.fontFamily }}>
        ...
        <div className="flex min-h-0">
          ...
          <aside className="w-[200px] shrink-0 bg-blue-50 border-r border-blue-100 p-5 space-y-5 print:w-[180px]">
            {skills.length > 0 && (
              <div>
                <h3 className="text-[10px] font-bold text-blue-600 uppercase tracking-wider mb-2 border-b border-blue-200 pb-1">{t('resume.skills')}</h3>
                <SkillsDisplay skills={skills} themeId={theme.id} accentColor={theme.accentColor} />
              </div>
            )}
            {resume.languages.length > 0 && (
              <div>
                <h3 className="text-[10px] font-bold text-blue-600 uppercase tracking-wider mb-2 border-b border-blue-200 pb-1">{t('resume.languages')}</h3>
                <div className="space-y-1">
                  {resume.languages.map(lang => (
            ...
            {resume.certifications.length > 0 && (
              <div>
                <h3 className="text-[10px] font-bold text-blue-600 uppercase tracking-wider mb-2 border-b border-blue-200 pb-1">{t('resume.certifications')}</h3>
                <div className="space-y-1">
                  {resume.certifications.map(cert => (
    ...
    return (
      <div ref={ref} className="resume-preview bg-white px-12 sm:px-20 py-12 sm:py-16 max-w-[210mm] mx-auto shadow-lg print:shadow-none print:p-0 overflow-hidden" style={{ fontFamily: theme.fontFamily }}>
        ...
        <div className={`${theme.bodyStyle} mt-8 space-y-0`}>
          {pi.summary && (
            <Section title={t('resume.summary')} theme={theme} isFirst>
              <SafeHtml html={pi.summary} className="text-sm text-slate-500 leading-loose break-words" />
            </Section>
          )}
          {experiences.length > 0 && (
              ...
              <div className="space-y-8">
                {experiences.map(exp => (
                  <div key={exp.id}>
                    ...
                    {exp.techStack && (
                      <div className="mt-2 text-xs text-slate-400 tracking-wide">{exp.techStack}</div>
                    )}
                  </div>
                ))}
              </div>
      ...
      <div ref={ref} className="resume-preview bg-gradient-to-b from-slate-50 to-white max-w-[210mm] mx-auto shadow-lg print:shadow-none overflow-hidden" style={{ fontFamily: theme.fontFamily }}>
        <header className={theme.headerStyle}>
          <div className={hasPhoto ? 'flex gap-5 items-center' : ''}>
            {hasPhoto && (
              <img src={pi.photo} alt="" loading="lazy" decoding="async" className="w-[100px] h-[100px] object-cover rounded-full border-4 border-white/30 shrink-0 shadow-lg" />
            )}
            <div className="flex-1 min-w-0">
              <h1 className="text-3xl sm:text-4xl font-extrabold text-white tracking-tight">{pi.name || '이름'}</h1>
              <div className="mt-2.5"><ContactInfo pi={pi} theme={theme} /></div>
            </div>
          </div>
        </header>
                  ...
                  <div className="space-y-1">
                    {resume.languages.map(lang => (
                      <div key={lang.id} className="text-sm">
                        ...
                        {lang.score && <span className="text-purple-600 ml-2 font-medium">{lang.score}</span>}
                      </div>
                    ))}
                  </div>
                </div>
                  ...
                  <div className="space-y-1">
                    {resume.awards.map(award => (
                      <div key={award.id} className="text-sm">
                        ...
                        {award.issuer && <span className="text-slate-500 ml-2">{award.issuer}</span>}
                      </div>
                    ))}
                  </div>
                </div>
      ...
      <div ref={ref} className="resume-preview bg-[#f6f8fa] max-w-[210mm] mx-auto shadow-lg print:shadow-none overflow-hidden" style={{ fontFamily: theme.fontFamily }}>
        <header className={theme.headerStyle}>
          ...
          <h1 className="text-2xl sm:text-3xl font-bold text-[#58a6ff] font-mono">
            <span className="text-[#484f58]">$ whoami </span>{pi.name || '이름'}
          </h1>
          <div className="mt-3 font-mono text-xs text-[#8b949e] space-y-0.5">
            {pi.email && <div><span className="text-[#484f58]">email:</span> {pi.email}</div>}
            {pi.phone && <div><span className="text-[#484f58]">phone:</span> {pi.phone}</div>}
            {pi.github && <div><span className="text-[#484f58]">github:</span> <span className="text-[#58a6ff]">{pi.github}</span></div>}
            {pi.website && <div><span className="text-[#484f58]">web:</span> <span className="text-[#58a6ff]">{pi.website}</span></div>}
            {pi.address && <div><span className="text-[#484f58]">loc:</span> {pi.address}</div>}
          </div>
        </header>
    ...
    return (
      <div ref={ref} className="resume-preview bg-white max-w-[210mm] mx-auto shadow-lg print:shadow-none overflow-hidden" style={{ fontFamily: theme.fontFamily }}>
        <header className={theme.headerStyle}>
          {hasPhoto && (
            <div className="flex justify-center mb-4">
              <img src={pi.photo} alt="" loading="lazy" decoding="async" className="w-[90px] h-[90px] object-cover rounded-full border-2 border-amber-300 shadow-md" />
            </div>
          )}
          <h1 className="text-3xl sm:text-4xl font-light text-slate-900 tracking-wide" style={{ fontFamily: "'Playfair Display', 'Noto Serif KR', serif" }}>
            {pi.name || '이름'}
          </h1>
        ...
        <div className={`px-8 sm:px-14 pb-10 ${theme.bodyStyle}`}>
          {pi.summary && (
            <Section title={t('resume.summary')} theme={theme} isFirst>
              <SafeHtml html={pi.summary} className="text-sm text-slate-600 leading-relaxed break-words text-center italic" />
            </Section>
          )}
          {experiences.length > 0 && (
            <Section title={t('resume.experience')} theme={theme} isFirst={!pi.summary}>
                  ...
                  <div className="space-y-1 text-sm">
                    {resume.certifications.map(cert => (
                      <div key={cert.id}>
                        ...
                        {cert.issuer && <span className="text-slate-500 ml-1">({cert.issuer})</span>}
                      </div>
                    ))}
                  </div>
                </section>
                    ...
                    <div className="space-y-1 text-sm">
                      {resume.awards.map(award => (
                        <div key={award.id}>
                          ...
                          {award.issuer && <span className="text-slate-500 ml-1">({award.issuer})</span>}
                        </div>
                      ))}
                    </div>
                  </section>
                    ...
                    <div className="space-y-1 text-sm">
                      {resume.activities.map(act => (
                        <div key={act.id}>
                          ...
                          {act.organization && <span className="text-slate-500 ml-1">({act.organization})</span>}
                        </div>
                      ))}
                    </div>
                  </section>
    ...
    return (
      <div ref={ref} className="resume-preview bg-white px-8 sm:px-14 py-10 sm:py-14 max-w-[210mm] mx-auto shadow-lg print:shadow-none print:p-0 overflow-hidden" style={{ fontFamily: theme.fontFamily }}>
        ...
        <div className={`${theme.bodyStyle} space-y-0`}>
          {pi.summary && (
            <Section title={t('resume.summary')} theme={theme} isFirst>
              <SafeHtml html={pi.summary} className="text-base text-slate-600 leading-[1.9] break-words" />
            </Section>
          )}
          {experiences.length > 0 && (
            <Section title={t('resume.experience')} theme={theme} isFirst={!pi.summary}>
          ...
          <div className="text-center">
            {hasPhoto && (
              <div className="flex justify-center mb-5">
                <img src={pi.photo} alt="" loading="lazy" decoding="async" className="w-[120px] h-[120px] object-cover rounded-full border-4 border-white/30 shadow-xl" />
              </div>
            )}
            <h1 className="text-4xl sm:text-5xl font-extrabold text-white tracking-tight">{pi.name || '이름'}</h1>
        ...
        <div className="p-6 sm:p-10">
          ...
          {projects.length > 0 && (
            <Section title={t('resume.projects')} theme={theme}>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {projects.map(proj => (
                  <div key={proj.id} className="border border-slate-200 rounded-xl p-4 bg-gradient-to-br from-slate-50 to-white shadow-sm hover:shadow-md transition-shadow">
                    ...
                    {proj.role && <div className="text-xs text-purple-600 mt-0.5">{proj.role}</div>}
                    {proj.description && <SafeHtml html={proj.description} className="text-xs text-slate-600 mt-2 leading-relaxed line-clamp-3 break-words" />}
                    {proj.techStack && <TechTags text={proj.techStack} color="blue" themeId={theme.id} />}
                    {proj.link && <p className="text-xs text-indigo-600 mt-2 break-all">{proj.link}</p>}
                  </div>
                ))}
              </div>
            </Section>
