...
export default function ResumeScoreCard({ resume, onClose }: Props) {
  ...
  const color = grade === 'S' ? '#7c3aed' : grade === 'A' ? '#3b82f6' : grade === 'B' ? '#22c55e' : grade === 'C' ? '#f97316' : '#ef4444';
  const topSkills = resume.skills.flatMap(s => s.items.split(',').map(i => i.trim()).filter(Boolean)).slice(0, 5);
  ...
  const shareText = [
    ...
    `https://이력서공방.com`,
  ].filter(l => l !== null && l !== undefined && !(l === '' && topSkills.length === 0)).join('\n');
  ...
  const handleShareLinkedIn = () => {
    const text = encodeURIComponent(`이력서공방에서 이력서 진단 ${pct}% (${grade}등급)을 받았습니다! 무료로 이력서 진단받기 → 이력서공방.com #이력서 #취업 #이직`);
    window.open(`https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent('https://이력서공방.com')}&summary=${text}`, '_blank');
  };

  const handleShareTwitter = () => {
    const text = encodeURIComponent(`이력서공방 이력서 진단 결과 🎯\n\n완성도 ${pct}% / ${grade}등급 달성!\n\n${getGradeMessage(grade, pct)}\n\n무료로 내 이력서도 진단받기 👇`);
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4" role="dialog" aria-modal="true">
      ...
      <div className="relative w-full max-w-sm bg-white dark:bg-slate-900 rounded-2xl shadow-2xl overflow-hidden animate-fade-in-up">
        {/* Close */}
        <button
          onClick={onClose}
        ...
        {/* Card visual */}
        <div ref={cardRef} style={{ background: `linear-gradient(135deg, ${color}22, ${color}11)` }}>
          <div className="px-6 pt-8 pb-6" style={{ borderBottom: `3px solid ${color}33` }}>
            {/* Logo + Grade */}
            <div className="flex items-start justify-between mb-6">
              <div>
                <div className="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-1">이력서공방</div>
                <div className="text-lg font-bold text-slate-800 dark:text-slate-200">Resume Score</div>
              </div>
              <div className="text-right">
                <div className="text-4xl font-black" style={{ color }}>{pct}%</div>
                <div className="text-sm font-bold" style={{ color }}>{grade}등급</div>
              </div>
            </div>

            {/* Name */}
            <div className="mb-5">
              <div className="text-xl font-bold text-slate-900 dark:text-slate-100 mb-0.5 truncate">{name}</div>
            ...
            <div className="space-y-2">
              {sections.slice(0, 4).map(s => (
                <div key={s.label} className="flex items-center gap-2">
                  <span className="text-xs text-slate-500 dark:text-slate-400 w-20 shrink-0">{s.label}</span>
            ...
            {topSkills.length > 0 && (
              <div className="mt-4 flex flex-wrap gap-1.5">
                {topSkills.map(skill => (
                  <span key={skill} className="px-2 py-0.5 text-xs rounded-full border" style={{ borderColor: `${color}40`, color, backgroundColor: `${color}10` }}>
                    {skill}
                  </span>
                ))}
              </div>
        ...
        <div className="p-4 border-t border-slate-100 dark:border-slate-700 bg-white dark:bg-slate-900">
          <p className="text-xs text-slate-500 dark:text-slate-400 text-center mb-3">이 결과를 공유해보세요!</p>
