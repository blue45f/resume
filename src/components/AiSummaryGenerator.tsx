...
export default function AiSummaryGenerator({ resumeId, resume, onAccept }: Props) {
  ...
  function buildContextForAI(): string {
    ...
    if (resume.experiences.length > 0) {
      const expSummary = resume.experiences.map(e => {
    ...
    if (resume.educations.length > 0) {
      const eduSummary = resume.educations.map(e =>
  ...
  const hasEnoughData = resume.experiences.length > 0 || resume.skills.length > 0 || resume.educations.length > 0;
    ...
    <div className="mt-2 rounded-xl border border-dashed border-purple-300 dark:border-purple-700 bg-purple-50/50 dark:bg-purple-900/10 p-4">
      ...
      {loading && (
        <div className="flex items-center justify-center gap-2 py-6">
          ...
          <span className="text-sm text-purple-600 dark:text-purple-400">AI가 자기소개를 작성 중입니다...</span>
        </div>
      )}
      ...
      {preview && !loading && (
        <div className="space-y-3">
          <div className="p-3 bg-white dark:bg-slate-800 rounded-lg border border-purple-200 dark:border-purple-800">
            <p className="text-xs font-medium text-slate-500 dark:text-slate-400 mb-1.5">AI 생성 결과:</p>
            <p className="text-sm text-slate-800 dark:text-slate-200 leading-relaxed whitespace-pre-wrap">{preview}</p>
          </div>
      ...
      {!preview && !loading && (
        <button
          ...
          className="w-full flex items-center justify-center gap-2 px-4 py-2.5 text-sm font-medium text-white bg-gradient-to-r from-purple-600 to-blue-600 rounded-lg hover:from-purple-700 hover:to-blue-700 disabled:opacity-40 disabled:cursor-not-allowed transition-all shadow-sm hover:shadow-md"
        >
