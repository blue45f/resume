...
export default function CommentSection({ resumeId, isPublic }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (content.trim().length < 5) { toast('5자 이상 입력해주세요', 'warning'); return; }
    setLoading(true);
    try {
  ...
  const handleReply = async (e: React.FormEvent, parentId: string) => {
    e.preventDefault();
    if (replyText.trim().length < 5) { toast('5자 이상 입력해주세요', 'warning'); return; }
    ...
    try {
      ...
      if (token) headers['Authorization'] = `Bearer ${token}`;
      const res = await fetch(`${API_URL}/api/resumes/${resumeId}/comments`, {
        method: 'POST', headers, body: JSON.stringify({ content: replyText, parentId }),
      });
  ...
  const renderNode = (node: CommentNode, depth: number) => {
    ...
    return (
      <div key={node.id}>
        <div className={`flex gap-${isRoot ? '3' : '2.5'} p-${isRoot ? '3' : '2.5'} ${
          isRoot
            ? 'bg-slate-50 dark:bg-slate-900 rounded-lg'
            : 'bg-slate-50/50 dark:bg-slate-800/60 rounded-lg'
        } animate-fade-in`}>
          {!isRoot && (
            <svg className="w-3 h-3 text-indigo-400 shrink-0 mt-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 10h10a8 8 0 018 8v2M3 10l6 6m-6-6l6-6" />
            </svg>
          )}
          <div className={`${isRoot ? 'w-8 h-8' : 'w-6 h-6'} ${
            isRoot
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-0.5">
              <span className={`${isRoot ? 'text-sm' : 'text-xs'} font-medium text-slate-700 dark:text-slate-200`}>
                {node.authorName}
              </span>
              {node.userId && (
                <SendMessageButton targetUserId={node.userId} targetUserName={node.authorName} variant="mini" />
              )}
              <span className={`${isRoot ? 'text-xs' : 'text-[10px]'} text-slate-400 dark:text-slate-500`}>
                {timeAgo(node.createdAt)}
              </span>
            </div>
            <p className={`${isRoot ? 'text-sm' : 'text-xs'} text-slate-600 dark:text-slate-300 whitespace-pre-wrap break-words`}>
              {node.content}
            </p>
            <div className="flex items-center gap-3 mt-1.5">
              {currentUser && depth < MAX_DEPTH && (
                ...
                >
                  ...
                  {replyingTo?.id === node.id ? '취소' : `답글${childCount > 0 ? ` (${childCount})` : ''}`}
                </button>
              )}
              {!currentUser && childCount > 0 && (
                <span className="text-xs text-slate-400">답글 {childCount}개</span>
              )}
              {(currentUser?.role === 'admin' || currentUser?.role === 'superadmin' || (currentUser?.id && currentUser.id === node.userId)) && (
                <button
            ...
            <form onSubmit={(e) => handleReply(e, node.id)} className="bg-indigo-50 dark:bg-indigo-900/20 border border-indigo-200 dark:border-indigo-800 rounded-xl overflow-hidden">
              ...
              <div className="flex items-center justify-between px-3 py-2 border-t border-indigo-100 dark:border-indigo-800/50">
                ...
                <div className="flex gap-2">
                  <button type="button" onClick={() => { setReplyingTo(null); setReplyText(''); }} className="px-3 py-1 text-xs text-slate-500 hover:text-slate-700 transition-colors">
                    취소
                  </button>
                  <button type="submit" disabled={submittingReply || replyText.trim().length < 5} className="px-3 py-1 bg-indigo-600 text-white text-xs rounded-lg hover:bg-indigo-700 disabled:opacity-50 transition-colors">
    ...
    <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-5 no-print">
      ...
      <form onSubmit={handleSubmit} className="mb-4">
        ...
        <div className="flex items-center justify-between mt-2">
          ...
          <button type="submit" disabled={loading || content.trim().length < 5} className="px-4 py-1.5 bg-blue-600 text-white text-sm rounded-lg hover:bg-blue-700 disabled:opacity-50 transition-colors">
            {loading ? '등록 중...' : '의견 남기기'}
          </button>
        </div>
      </form>
