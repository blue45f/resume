...
const DEFAULT_STEPS: Step[] = [
  { num: 1, title: '이력서 작성', desc: '템플릿 선택 후 9개 섹션 편집', href: '/resumes/new', color: 'from-blue-500 to-blue-600' },
  { num: 2, title: 'AI 분석', desc: '피드백, JD매칭, ATS 검사', href: '/tutorial', color: 'from-indigo-500 to-indigo-600' },
  { num: 3, title: '공유/지원', desc: '링크 공유, PDF 내보내기', href: '/explore', color: 'from-purple-500 to-purple-600' },
];
  ...
  return (
    <div className={`mb-6 bg-gradient-to-r from-blue-600 to-indigo-600 rounded-2xl p-5 sm:p-6 text-white relative overflow-hidden transition-all duration-300 ${hiding ? 'opacity-0 -translate-y-2' : 'animate-fade-in-up'}`}>
      ...
      <div className="absolute top-3 right-3 flex items-center gap-2">
        <button onClick={handleNeverShow} className="text-white/50 hover:text-white/80 text-xs transition-colors px-2 py-1 rounded hover:bg-white/10" title="다시 보지 않기">
          다시 보지 않기
        </button>
        <button onClick={handleDismiss} className="text-white/60 hover:text-white transition-colors p-1 rounded hover:bg-white/10" aria-label="닫기">
        ...
        {steps.map((step, i) => (
          <div key={step.num} className="flex items-center">
            <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold ${
              getStepStatus(step.href) === 'active' ? 'bg-white text-blue-600' : 'bg-white/20 text-white'
            }`}>
              {step.num}
            </div>
