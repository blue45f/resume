...
export default function FeatureGate({ feature, children, fallback }: Props) {
  ...
  return (
    <div className="p-4 bg-gradient-to-br from-indigo-50 to-purple-50 dark:from-indigo-900/20 dark:to-purple-900/20 border border-indigo-200 dark:border-indigo-800 rounded-xl text-center">
      ...
      <p className="text-xs text-indigo-600 dark:text-indigo-400 mt-1 mb-3">이 기능은 프로 이상 플랜에서 사용 가능합니다</p>
      <Link to="/pricing" className="inline-block px-4 py-1.5 bg-indigo-600 text-white text-xs font-medium rounded-lg hover:bg-indigo-700 transition-colors">
        플랜 업그레이드
      </Link>
    </div>
  );
