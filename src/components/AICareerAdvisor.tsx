...
const QUICK_QUESTIONS = [
  { label: '연봉 올리려면?', question: '현재 이력서를 기반으로 연봉을 올리기 위해 무엇을 해야 할까요?' },
  { label: '이직 시기?', question: '지금 이직하기 좋은 시기인가요? 시장 상황을 고려해서 알려주세요.' },
  { label: '부족한 스킬?', question: '현재 이력서에서 부족한 기술 스택은 무엇이고, 어떤 것을 배워야 할까요?' },
  { label: '커리어 방향', question: '현재 경력을 기반으로 가장 유망한 커리어 방향을 추천해주세요.' },
];
...
function generateLocalResponse(question: string, resume: Resume): string {
  ...
  if (q.includes('연봉') || q.includes('salary') || q.includes('돈')) {
    ...
    if (skills.length < 5) {
      tips.push('기술 스택이 다소 부족합니다. 클라우드(AWS, GCP), 컨테이너(Docker, K8s) 등 시장 수요가 높은 기술을 추가하면 연봉 협상에 큰 도움이 됩니다.');
    } else {
      tips.push(`${skills.length}개의 기술을 보유하고 있어 양호합니다. 이 중 시장 프리미엄이 높은 기술(AI/ML, 클라우드)에 집중하면 연봉 상승에 유리합니다.`);
    }
    if (certCount === 0) {
      tips.push('자격증이 없습니다. AWS SAA, CKA 등 업계 인정 자격증은 연봉 협상에서 5-10% 프리미엄을 기대할 수 있습니다.');
    }
    tips.push('이직 시에는 현재 연봉의 15-20% 인상을 목표로 하되, 시장 데이터를 기반으로 논리적 근거를 제시하세요.');
    return tips.join('\n\n');
  }

  if (q.includes('이직') || q.includes('시기') || q.includes('이동')) {
    const tips: string[] = [];
    tips.push('일반적으로 IT 업계에서는 상반기(3-4월)와 하반기(9-10월)가 가장 채용이 활발한 시기입니다.');
    if (expYears >= 3) {
      tips.push(`${expYears}년차 경력이면 시니어 포지션으로의 이직을 고려할 수 있습니다. 현재 포지션에서의 성과를 정량화하여 이력서에 추가하세요.`);
    } else {
      tips.push('아직 경력이 짧은 편이므로, 현재 회사에서 핵심 프로젝트 경험을 쌓은 후 이직하는 것이 유리합니다. 최소 2년 이상의 경력을 권장합니다.');
    }
    tips.push('이직 전 LinkedIn 프로필과 이력서를 업데이트하고, 포트폴리오를 정리하세요. 기술 면접 준비도 최소 1-2개월 전부터 시작하는 것이 좋습니다.');
    return tips.join('\n\n');
  }

  if (q.includes('스킬') || q.includes('기술') || q.includes('배우')) {
    const tips: string[] = [];
    const highDemand = ['typescript', 'kubernetes', 'aws', 'docker', 'python', 'terraform', 'react', 'go'];
    const missing = highDemand.filter(s => !skills.includes(s));

    if (missing.length > 0) {
      tips.push(`현재 보유하지 않은 수요 높은 기술: ${missing.slice(0, 4).join(', ')}`);
      tips.push(`특히 ${missing[0]}은(는) 현재 시장에서 가장 수요가 높은 기술 중 하나입니다. 우선적으로 학습을 추천합니다.`);
    } else {
      tips.push('주요 수요 기술을 대부분 보유하고 있습니다! 심화 학습이나 관련 자격증 취득을 고려해보세요.');
    }
    tips.push('2026년 기준으로 AI/ML 관련 기술(LLM, RAG, MLOps)과 클라우드 네이티브 기술이 가장 빠르게 성장하는 분야입니다.');
    return tips.join('\n\n');
  }
  ...
  if (q.includes('커리어') || q.includes('방향') || q.includes('진로') || q.includes('추천')) {
    ...
    if (skills.some(s => ['react', 'typescript', 'javascript', 'vue'].includes(s))) {
      tips.push('프론트엔드 기술을 보유하고 있으므로, 시니어 프론트엔드 -> 프론트엔드 리드 -> Engineering Manager 경로를 추천합니다.');
      tips.push('풀스택으로의 전환도 좋은 선택입니다. Node.js/NestJS 백엔드와 인프라 경험을 쌓으면 기회가 넓어집니다.');
    } else if (skills.some(s => ['python', 'java', 'node.js', 'spring'].includes(s))) {
      tips.push('백엔드 기술을 기반으로 시니어 백엔드 -> 테크 리드 -> 아키텍트 경로가 적합합니다.');
      tips.push('데이터 엔지니어링이나 DevOps/SRE 방향으로의 확장도 고려해보세요. 시장 수요와 연봉이 모두 높은 분야입니다.');
    } else {
      tips.push(`${expYears}년차 경력을 기반으로, 본인의 강점 분야에 집중하면서 시니어 레벨로의 성장을 목표로 하세요.`);
    }
    tips.push('어떤 경로를 선택하든 리더십 경험(팀 리딩, 멘토링, 코드 리뷰)이 다음 단계로의 핵심 역량이 됩니다.');
    return tips.join('\n\n');
  }
...
export default function AICareerAdvisor({ resume }: Props) {
  ...
  return (
    <>
      ...
      {/* Chat panel */}
      {isOpen && (
        <div className="no-print fixed bottom-20 right-6 z-50 w-80 sm:w-96 bg-white dark:bg-slate-800 rounded-xl shadow-2xl border border-slate-200 dark:border-slate-700 flex flex-col overflow-hidden animate-fade-in"
              ...
              <div className="flex flex-wrap gap-1">
                {QUICK_QUESTIONS.map(qq => (
                  <button
                    ...
                    className="px-2 py-1 text-[10px] font-medium bg-indigo-50 dark:bg-indigo-900/20 text-indigo-600 dark:text-indigo-400 rounded-full border border-indigo-200 dark:border-indigo-800 hover:bg-indigo-100 dark:hover:bg-indigo-900/40 transition-colors"
                  >
                    {qq.label}
                  </button>
                ))}
          ...
          <div className="px-3 py-2 border-t border-slate-200 dark:border-slate-700 shrink-0">
            ...
            >
              <input
                ...
                disabled={isLoading}
                className="flex-1 text-xs px-3 py-1.5 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-600 rounded-lg focus:outline-none focus:ring-1 focus:ring-indigo-500 text-slate-700 dark:text-slate-300 placeholder-slate-400 disabled:opacity-50"
              />
              <button
                type="submit"
                disabled={!input.trim() || isLoading}
                className="px-2.5 py-1.5 bg-indigo-500 text-white rounded-lg hover:bg-indigo-600 transition-colors disabled:opacity-40 disabled:cursor-not-allowed shrink-0"
              >
                <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
