...
const RESULT_OPTIONS = [
  { value: 'pass', label: '합격', color: 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400', icon: 'O' },
  { value: 'fail', label: '불합격', color: 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400', icon: 'X' },
  { value: 'pending', label: '대기', color: 'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400', icon: '...' },
] as const;
          ...
          {!review && !editing ? (
            ...
            <div className="bg-slate-50 dark:bg-slate-900/50 rounded-lg p-3 space-y-3">
              ...
              <div>
                ...
                <div className="flex gap-1">
                  ...
                  <span className="ml-2 text-xs text-slate-400 self-center">
                    {form.difficulty <= 1 ? '매우 쉬움' : form.difficulty <= 2 ? '쉬움' : form.difficulty <= 3 ? '보통' : form.difficulty <= 4 ? '어려움' : '매우 어려움'}
                  </span>
                </div>
              </div>
              ...
              <div className="flex gap-2 justify-end">
                <button
                  type="button"
                  onClick={() => { setEditing(false); if (!review) setForm({ difficulty: 3, questions: '', result: 'pending', tips: '' }); }}
            ...
            <div className="bg-slate-50 dark:bg-slate-900/50 rounded-lg p-3 space-y-2.5">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs text-slate-400">
                  ({review.difficulty <= 1 ? '매우 쉬움' : review.difficulty <= 2 ? '쉬움' : review.difficulty <= 3 ? '보통' : review.difficulty <= 4 ? '어려움' : '매우 어려움'})
                </span>
              </div>
              ...
              {review.questions && (
                <div>
                  <span className="text-xs text-slate-500 dark:text-slate-400 block mb-1">면접 질문</span>
              ...
              {review.tips && (
                <div>
                  <span className="text-xs text-slate-500 dark:text-slate-400 block mb-1">면접 팁</span>
                  <div className="text-sm text-slate-700 dark:text-slate-300 whitespace-pre-wrap bg-white dark:bg-slate-800 rounded-lg p-2.5 border border-slate-100 dark:border-slate-700">
                    {review.tips}
                  </div>
                </div>
