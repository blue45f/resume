...
function ProjectCard({ project, index }: { project: Project; index: number }) {
  ...
  return (
    <div
      className="group bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden shadow-sm hover:shadow-md transition-shadow duration-300 flex flex-col"
    >
      {/* Gradient header */}
      <div className={`h-24 bg-gradient-to-br ${gradient} relative flex items-end p-4`}>
        ...
        <div className="relative z-10 w-full">
          ...
          {project.role && (
            <p className="text-white/80 text-xs mt-0.5 truncate">{project.role}</p>
          )}
        </div>
      </div>
