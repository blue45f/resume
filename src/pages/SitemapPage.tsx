...
const SITEMAP = [
  ...
  },
  {
    ...
    links: [
      ...
      { to: '/applications', label: '지원 관리', desc: '칸반 보드, 타임라인, 면접 후기', auth: true },
    ],
  },
  {
    title: '소셜 / 네트워킹',
...
export default function SitemapPage() {
  ...
  return (
    <>
      <Header />
      <main id="main-content" className="flex-1 max-w-5xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-8" role="main">
        <div className="mb-8">
          <h1 className="text-2xl sm:text-3xl font-bold text-slate-900 dark:text-slate-100">사이트맵</h1>
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-2">이력서공방의 모든 페이지를 한눈에 확인하세요</p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {SITEMAP.map(section => (
            <div key={section.title} className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 overflow-hidden">
              {/* Section Header */}
              <div className={`bg-gradient-to-r ${section.color} px-5 py-3 flex items-center gap-2`}>
                ...
                <span className="ml-auto text-xs text-white/70">{section.links.length}개 페이지</span>
              </div>
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        {'auth' in link && link.auth && (
                          <span className="px-1.5 py-0.5 text-[9px] font-medium bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400 rounded">로그인</span>
                        )}
                      </div>
                      <p className="text-xs text-slate-400 dark:text-slate-500 mt-0.5">{link.desc}</p>
                    </div>
                    <svg className="w-4 h-4 text-slate-300 dark:text-slate-600 group-hover:text-blue-400 transition-colors shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
