...
const TYPE_META: Record<string, { icon: string; bg: string; label: string; color: string }> = {
  comment: { icon: '💬', bg: 'bg-blue-100 dark:bg-blue-900/30', label: '댓글', color: 'text-blue-600 dark:text-blue-400' },
  bookmark: { icon: '🔖', bg: 'bg-amber-100 dark:bg-amber-900/30', label: '북마크', color: 'text-amber-600 dark:text-amber-400' },
  scout: { icon: '📨', bg: 'bg-purple-100 dark:bg-purple-900/30', label: '스카우트', color: 'text-purple-600 dark:text-purple-400' },
  message: { icon: '✉️', bg: 'bg-green-100 dark:bg-green-900/30', label: '쪽지', color: 'text-green-600 dark:text-green-400' },
  application_comment: { icon: '📋', bg: 'bg-orange-100 dark:bg-orange-900/30', label: '지원', color: 'text-orange-600 dark:text-orange-400' },
  system: { icon: '⚙️', bg: 'bg-slate-100 dark:bg-slate-700', label: '시스템', color: 'text-slate-600 dark:text-slate-400' },
};
      ...
      <main id="main-content" className="flex-1 max-w-3xl mx-auto w-full px-4 sm:px-6 py-6 sm:py-8" role="main">
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center gap-2">
            {unreadCount > 0 && !selectMode && (
              <button onClick={markAllRead} className="text-xs text-indigo-600 dark:text-indigo-400 hover:text-indigo-800 font-medium px-3 py-1.5 rounded-lg hover:bg-indigo-50 dark:hover:bg-indigo-900/20 transition-colors">
                모두 읽음
              </button>
            )}
            <button
              onClick={() => { setSelectMode(!selectMode); setSelectedIds(new Set()); setExpandedAction(null); }}
        ...
        {selectMode && (
          <div className="flex items-center gap-2 mb-4 p-3 bg-indigo-50 dark:bg-indigo-900/20 border border-indigo-200 dark:border-indigo-800 rounded-xl animate-fade-in">
            ...
            <button
              onClick={() => selectedIds.size === filtered.length ? setSelectedIds(new Set()) : setSelectedIds(new Set(filtered.map(n => n.id)))}
        ...
        {loading ? (
          ...
          <div className="text-center py-16">
            <div className="w-16 h-16 bg-indigo-100 dark:bg-indigo-900/30 rounded-2xl flex items-center justify-center text-3xl mx-auto mb-4">🔔</div>
            <h2 className="text-lg font-semibold text-slate-700 dark:text-slate-300 mb-2">로그인이 필요합니다</h2>
            <p className="text-sm text-slate-500 dark:text-slate-400 mb-4">알림을 확인하려면 로그인해주세요</p>
            <Link to="/login" className="inline-flex items-center px-4 py-2 bg-indigo-600 text-white text-sm font-medium rounded-xl hover:bg-indigo-700 transition-colors">
              로그인
            </Link>
          </div>
        ) : filtered.length === 0 ? (
            ...
            {grouped.map(([group, items]) => (
              <div key={group}>
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <span className="text-[10px] text-slate-400 dark:text-slate-500">{items.length}개</span>
                </div>
                <div className="space-y-2">
                  {items.map(n => {
                    const meta = TYPE_META[n.type] || { icon: '🔔', bg: 'bg-slate-100 dark:bg-slate-700', label: '알림', color: 'text-slate-600' };
                    ...
                    return (
                      ...
                      >
                        ...
                        {/* Icon */}
                        <div className={`shrink-0 w-10 h-10 rounded-xl ${meta.bg} flex items-center justify-center text-lg shadow-sm`}>
                          {meta.icon}
                        </div>
                        ...
                        >
                          ...
                          <p className={`text-sm leading-relaxed ${!n.read ? 'font-medium text-slate-900 dark:text-slate-100' : 'text-slate-600 dark:text-slate-400'}`}>
                            {n.message}
                          </p>
                          ...
                          <div className="shrink-0 flex items-center gap-1">
                            ...
                            {isActionOpen && (
                              <div className="absolute right-3 top-12 z-10 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl shadow-lg py-1 min-w-[120px] animate-fade-in">
                                ...
                                {n.link && (
                                  <button
                                    onClick={() => { setExpandedAction(null); navigate(n.link!); }}
                                    className="w-full text-left px-3 py-2 text-xs text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 flex items-center gap-2"
                                  >
                                    <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
