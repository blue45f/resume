  ...
  return (
    <>
      ...
      <main id="main-content" className="flex-1 max-w-3xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-6 sm:py-8" role="main">
        <h1 className="text-xl sm:text-2xl font-bold text-slate-900 dark:text-slate-100 mb-6">태그 관리</h1>

        {/* Create form */}
        <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-4 sm:p-6 mb-6">
          <h2 className="text-sm font-semibold text-slate-900 dark:text-slate-100 mb-3">새 태그 추가</h2>
                ...
                {COLOR_PRESETS.map(c => (
                  <button
                    ...
                    className={`w-6 h-6 rounded-full border-2 transition-transform focus:outline-none ${
                      newColor === c ? 'border-slate-900 dark:border-white scale-110' : 'border-transparent hover:scale-105'
                    }`}
            ...
            {tags.map(tag => (
              <div key={tag.id} className="flex items-center justify-between px-4 sm:px-6 py-3">
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  />
                  <span className="text-sm font-medium text-slate-900 dark:text-slate-100 truncate">{tag.name}</span>
