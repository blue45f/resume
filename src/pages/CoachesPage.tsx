...
import { fetchCoaches, type CoachProfile } from '@/lib/api';
...
export default function CoachesPage() {
  ...
  useEffect(() => {
    ...
    fetchCoaches({
      ...
      .then(data => { if (!cancelled) setCoaches(Array.isArray(data) ? data : []); })
      .catch(err => { if (!cancelled) setError(err?.message || '코치 목록을 불러오지 못했습니다'); })
      .finally(() => { if (!cancelled) setLoading(false); });
    return () => { cancelled = true; };
  }, [specialty, minRate, maxRate]);
      ...
      <main id="main-content" className="flex-1 max-w-6xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-6 sm:py-8" role="main">
          ...
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-gradient-to-br from-rose-500 to-pink-600 rounded-xl flex items-center justify-center shadow-sm">
              <svg className="w-5 h-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 20h5v-2a4 4 0 00-3-3.87M9 20H4v-2a4 4 0 013-3.87m6 0a4 4 0 10-4-4 4 4 0 004 4z" />
              </svg>
            </div>
            <div>
              <h1 className="text-xl sm:text-2xl font-bold text-slate-900 dark:text-slate-100">전문 코치 찾기</h1>
              <p className="text-sm text-slate-500 dark:text-slate-400">이력서, 면접, 커리어 전문가와 1:1 매칭</p>
            </div>
          </div>
          <Link
        ...
        <div className="imp-card p-4 mb-6">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
            <div>
              <label className="block text-[11px] font-medium text-slate-500 dark:text-slate-400 mb-1">전문 분야</label>
              <select
                value={specialty}
            ...
            <div>
              <label className="block text-[11px] font-medium text-slate-500 dark:text-slate-400 mb-1">최소 시급 (원)</label>
              <input
                type="number"
                inputMode="numeric"
                min={0}
            ...
            <div>
              <label className="block text-[11px] font-medium text-slate-500 dark:text-slate-400 mb-1">최대 시급 (원)</label>
              <input
                type="number"
                inputMode="numeric"
                min={0}
            ...
            <div>
              <label className="block text-[11px] font-medium text-slate-500 dark:text-slate-400 mb-1">정렬</label>
              <select
                value={sortBy}
        ...
        {loading ? (
          ...
          <div className="imp-card p-8 text-center">
            ...
            <button
              onClick={() => { setSpecialty(''); setMinRate(''); setMaxRate(''); }}
          ...
          <div className="imp-card p-10 text-center">
            ...
            <h3 className="text-sm font-semibold text-slate-800 dark:text-slate-200 mb-1">조건에 맞는 코치가 없습니다</h3>
            <p className="text-xs text-slate-500 dark:text-slate-400 mb-4">필터를 조정하거나 모든 코치를 확인해보세요</p>
            <button
              onClick={resetFilters}
...
function CoachCard({ coach }: { coach: CoachProfile }) {
  ...
  const bioExcerpt = (coach.bio || '').length > 90
  ...
  return (
    <article className="imp-card p-5 flex flex-col card-hover">
      <div className="flex items-start gap-3">
        {avatar ? (
          <img src={avatar} alt={name} className="w-14 h-14 rounded-full object-cover bg-slate-100 dark:bg-slate-700" />
        ) : (
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-rose-600 dark:text-rose-400 font-medium truncate">{coach.specialty || '전문 분야 미설정'}</p>
          <div className="flex items-center gap-2 mt-1 text-[11px] text-slate-400 dark:text-slate-500">
            <span className="inline-flex items-center gap-0.5">
              ...
              <span className="text-slate-600 dark:text-slate-300 font-medium">{rating > 0 ? rating.toFixed(1) : '신규'}</span>
            </span>
      ...
      <p className="text-xs text-slate-600 dark:text-slate-400 mt-3 line-clamp-3 flex-1">{bioExcerpt}</p>

      <div className="mt-4 flex items-center justify-between pt-3 border-t border-slate-100 dark:border-slate-700/60">
        <div>
          <p className="text-[10px] text-slate-400 dark:text-slate-500 uppercase tracking-wider">시급</p>
          <p className="text-base font-bold text-slate-900 dark:text-slate-100">{rate.toLocaleString()}원</p>
        </div>
        <Link
          to={`/coaches/${coach.id}`}
