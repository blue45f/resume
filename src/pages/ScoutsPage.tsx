...
const STATUS_BADGE: Record<string, { label: string; className: string; dot: string }> = {
  pending:  { label: '대기중', className: 'bg-yellow-100 dark:bg-yellow-900/30 text-yellow-700 dark:text-yellow-400', dot: 'bg-yellow-400' },
  accepted: { label: '수락',   className: 'bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400', dot: 'bg-green-500' },
  rejected: { label: '거절',   className: 'bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400', dot: 'bg-red-500' },
  expired:  { label: '만료',   className: 'bg-slate-100 dark:bg-slate-700 text-slate-500 dark:text-slate-400', dot: 'bg-slate-400' },
};
...
export default function ScoutsPage() {
  ...
  const [tab, setTab] = useState<'sent' | 'received' | 'bookmarks'>(isRecruiter ? 'sent' : 'received');
  ...
  const addTemplate = useCallback(() => {
    ...
    if (editingTemplateId) {
      ...
      const tpl: ScoutTemplate = { id: Date.now().toString(), name: newTemplateName, message: newTemplateMessage, useCount: 0 };
        ...
        <div className="flex gap-1 mb-4 p-1 bg-slate-100 dark:bg-slate-800 rounded-xl w-fit">
          {([
            ...
            >
              ...
              {t.key === 'bookmarks' && <span className="ml-1.5 text-xs text-slate-400">({bookmarks.length})</span>}
            </button>
          ))}
        </div>
          ...
          <div className="mb-4 bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-4">
                ...
                {templates.map(tpl => (
                  <div key={tpl.id} className="flex items-start justify-between gap-2 p-3 bg-slate-50 dark:bg-slate-900 rounded-lg">
                    ...
                    <div className="flex gap-1 shrink-0">
                      <button onClick={() => applyTemplate(tpl)} className="text-[10px] px-2 py-1 rounded bg-blue-50 dark:bg-blue-900/20 text-blue-600 hover:bg-blue-100 transition-colors">
                        사용
                      </button>
                      <button onClick={() => editTemplate(tpl)} className="text-[10px] px-2 py-1 rounded bg-amber-50 dark:bg-amber-900/20 text-amber-600 hover:bg-amber-100 transition-colors">
                        수정
                      </button>
            ...
            <div className="space-y-2">
              ...
              <div className="flex gap-2">
                ...
                {editingTemplateId && (
                  <button
                    onClick={() => { setEditingTemplateId(null); setNewTemplateName(''); setNewTemplateMessage(''); }}
              ...
              <div className="mb-3 p-4 bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700">
                ...
                <div className="bg-slate-50 dark:bg-slate-900 rounded-xl p-4 border border-slate-100 dark:border-slate-800">
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    <div>
                      <p className="text-sm font-semibold text-slate-900 dark:text-slate-100">{user?.name || '채용담당자'}</p>
                      <p className="text-xs text-slate-500 dark:text-slate-400">{user?.companyName || '회사명'}</p>
                    </div>
                    {getStatusBadge('pending')}
                  </div>
                  <div className="p-3 bg-white dark:bg-slate-800 rounded-lg border border-slate-100 dark:border-slate-700">
                    <p className="text-sm text-slate-700 dark:text-slate-300 whitespace-pre-wrap leading-relaxed">{bulkMessage}</p>
                  </div>
                  <div className="flex gap-2 mt-3">
                    <span className="px-3 py-1.5 bg-green-600 text-white text-xs rounded-lg">수락</span>
                    <span className="px-3 py-1.5 bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-400 text-xs rounded-lg">거절</span>
                    <span className="px-3 py-1.5 bg-blue-600 text-white text-xs rounded-lg">답장하기</span>
                  </div>
                </div>
              </div>
            )}
                  ...
                  <div key={bm.id} className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-4 flex items-center justify-between">
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          <p className="text-sm font-medium text-slate-900 dark:text-slate-100">{bm.name}</p>
                          {bm.email && <p className="text-xs text-slate-500 dark:text-slate-400">{bm.email}</p>}
                        </div>
                      </div>
                  ...
                  >
                    ...
                    {daysLeft !== null && daysLeft > 0 && (
                      <div className="flex items-center gap-1 mt-1.5">
                        ...
                        <span className={`text-[10px] font-medium ${daysLeft <= 2 ? 'text-red-500' : 'text-amber-500'}`}>
                          {daysLeft}일 후 만료
                        </span>
                      </div>
                    )}
                ...
                <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-6">
                  <div className="flex items-start justify-between mb-4">
                    <div>
                      ...
                      {selected.status === 'pending' && getDaysRemaining(selected.createdAt) > 0 && (
