...
function renderMarkdown(text: string): string {
  ...
  for (const raw of lines) {
    ...
    // Headings
    if (raw.startsWith('### ')) { closeList(); closeQuote(); out.push(`<h3 class="text-lg font-bold mt-5 mb-2 text-slate-900 dark:text-slate-100">${escape(raw.slice(4))}</h3>`); continue; }
    if (raw.startsWith('## ')) { closeList(); closeQuote(); out.push(`<h2 class="text-xl font-bold mt-6 mb-2 text-slate-900 dark:text-slate-100">${escape(raw.slice(3))}</h2>`); continue; }
    if (raw.startsWith('# ')) { closeList(); closeQuote(); out.push(`<h1 class="text-2xl font-bold mt-6 mb-3 text-slate-900 dark:text-slate-100">${escape(raw.slice(2))}</h1>`); continue; }
    ...
    if (olMatch) {
      ...
      out.push(`<p class="my-1 ml-4 text-slate-700 dark:text-slate-300">${olMatch[1]}. ${inlineFormat(escape(olMatch[2]))}</p>`);
      continue;
    }
    ...
    // Empty line
    if (raw.trim() === '') { closeList(); closeQuote(); out.push('<div class="h-3" />'); continue; }

    // Normal paragraph
    out.push(`<p class="my-1.5 text-slate-700 dark:text-slate-300 leading-relaxed">${inlineFormat(escape(raw))}</p>`);
  }
  closeList(); closeQuote();
...
function inlineFormat(s: string): string {
  ...
  // Auto URL
  s = s.replace(/(^|[\s])(https?:\/\/[^\s<]+)/g, '$1<a href="$2" target="_blank" rel="noopener" class="text-indigo-600 dark:text-indigo-400 underline underline-offset-2 hover:opacity-80">$2</a>');
  return s;
}
...
export default function CommunityWritePage() {
  ...
  const handleFileUpload = async (files: FileList | null) => {
    ...
    for (const file of Array.from(files)) {
      if (file.size > 20 * 1024 * 1024) { setError(`${file.name}: 20MB 이하 파일만 첨부 가능합니다.`); continue; }
      if (attachments.length >= 5) { setError('첨부파일은 최대 5개까지 가능합니다.'); break; }
  ...
  const handleKeyDown = useCallback((e: React.KeyboardEvent<HTMLTextAreaElement>) => {
    if ((e.ctrlKey || e.metaKey)) {
      if (e.key === 'b') { e.preventDefault(); applyAction({ label: 'Bold', icon: 'B', title: '', wrap: ['**', '**'] }); }
          ...
          <div className="border border-slate-200 dark:border-slate-700 rounded-xl overflow-hidden bg-white dark:bg-slate-800">
            ...
            <div className="flex flex-wrap items-center gap-0.5 px-3 py-2 border-b border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/80">
              {TOOLBAR.map((action, idx) =>
                action === null ? (
                  <div key={`sep-${idx}`} className="w-px h-5 bg-slate-200 dark:bg-slate-700 mx-1" />
                ) : (
                  <button
                    key={action.label}
                    type="button"
                    ...
                    className={`px-2 py-1 text-xs rounded-lg transition-colors hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300 font-mono select-none ${
                      ...
                      action.label === 'Strike' ? 'line-through' : ''
                    }`}
                  >
                    {action.icon}
                  </button>
            ...
            {preview ? (
              <div
                ...
                dangerouslySetInnerHTML={{ __html: content ? renderMarkdown(content) : '<p class="text-slate-400">내용이 없습니다.</p>' }}
              />
            ) : (
              <textarea
                {...contentRegister}
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <label className="text-sm font-medium text-slate-700 dark:text-slate-300">
                첨부파일 <span className="text-xs text-slate-400 font-normal">({attachments.length}/5 · 최대 20MB)</span>
              </label>
              <button
                type="button"
            ...
            {attachments.length > 0 && (
              <div className="space-y-1.5">
                {attachments.map((att, idx) => (
                  <div key={idx} className="flex items-center gap-2.5 p-2.5 bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 rounded-xl text-sm">
                    <span className="text-base flex-shrink-0">
                      {att.type.startsWith('image/') ? '🖼️' : att.type === 'application/pdf' ? '📄' : '📎'}
                    </span>
                    <span className="flex-1 truncate text-slate-700 dark:text-slate-300">{att.name}</span>
