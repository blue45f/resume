...
const features = [
  { icon: featureIcons.resume, title: '이력서 관리', desc: '9개 섹션(경력, 학력, 기술, 프로젝트 등)을 체계적으로 관리. 부서, 성과, 기술 스택까지 상세 기록.' },
  { icon: featureIcons.ai, title: 'AI 양식 변환', desc: '무료 AI(Gemini, Groq)로 표준 이력서, 경력기술서, 자기소개서, LinkedIn 등 8가지 양식으로 변환.' },
  { icon: featureIcons.share, title: '공유 & 공개', desc: '비밀번호 보호 공유 링크, 만료 시간 설정. 공개 이력서는 탐색 페이지에서 검색 가능.' },
  { icon: featureIcons.version, title: '버전 관리', desc: '수정할 때마다 자동 스냅샷. 실수로 삭제해도 이전 버전으로 복원 가능.' },
  { icon: featureIcons.tag, title: '태그 & 템플릿', desc: '태그로 이력서 분류, 커스텀 템플릿으로 LLM 없이 무료 양식 변환.' },
  { icon: featureIcons.attach, title: '첨부파일', desc: 'PDF, 이미지, 문서 파일 첨부. 포트폴리오, 자격증 등을 함께 관리.' },
  { icon: featureIcons.auth, title: '소셜 로그인', desc: 'Google, GitHub, Kakao 계정으로 간편 로그인. 비로그인으로도 사용 가능.' },
  { icon: featureIcons.print, title: 'PDF/인쇄', desc: '미리보기 페이지에서 브라우저 인쇄 기능으로 깔끔한 PDF 저장.' },
];
