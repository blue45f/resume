...
const LANGUAGE_PAIRS = [
  { from: 'ko', to: 'en', fromFlag: '\u{1F1F0}\u{1F1F7}', toFlag: '\u{1F1FA}\u{1F1F8}', label: '한국어 \u2192 English' },
  { from: 'ko', to: 'ja', fromFlag: '\u{1F1F0}\u{1F1F7}', toFlag: '\u{1F1EF}\u{1F1F5}', label: '한국어 \u2192 日本語' },
  { from: 'en', to: 'ko', fromFlag: '\u{1F1FA}\u{1F1F8}', toFlag: '\u{1F1F0}\u{1F1F7}', label: 'English \u2192 한국어' },
  { from: 'en', to: 'ja', fromFlag: '\u{1F1FA}\u{1F1F8}', toFlag: '\u{1F1EF}\u{1F1F5}', label: 'English \u2192 日本語' },
  { from: 'ja', to: 'ko', fromFlag: '\u{1F1EF}\u{1F1F5}', toFlag: '\u{1F1F0}\u{1F1F7}', label: '日本語 \u2192 한국어' },
  { from: 'ja', to: 'en', fromFlag: '\u{1F1EF}\u{1F1F5}', toFlag: '\u{1F1FA}\u{1F1F8}', label: '日本語 \u2192 English' },
];
...
export default function TranslatePage() {
  ...
  const [selectedSections, setSelectedSections] = useState<string[]>(TRANSLATABLE_SECTIONS.map(s => s.key));
  ...
  // Load original resume for side-by-side preview
  useEffect(() => {
    if (!selectedResumeId) {
    ...
    fetchResume(selectedResumeId).then(r => {
      setOriginalResume(r);
      setOriginalText(buildOriginalText(r));
    }).catch(() => {});
  }, [selectedResumeId]);

  const buildOriginalText = (r: Resume): string => {
    const lines: string[] = [];
    ...
    if (r.experiences?.length) {
      ...
      r.experiences.forEach(e => {
        ...
        if (e.startDate || e.endDate) lines.push(`  ${e.startDate} ~ ${e.current ? '현재' : e.endDate}`);
  ...
  const handleTranslate = async () => {
    ...
    try {
      ...
      const langName = TARGET_LANGUAGES.find(l => l.code === selectedPair.to)?.name || selectedPair.to;
      ...
      <main id="main-content" className="flex-1 max-w-6xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-6 sm:py-8" role="main">
        ...
        <div className="space-y-4 mb-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-sm font-medium text-slate-700 dark:text-slate-200 mb-1">언어 방향</label>
              <div className="grid grid-cols-3 gap-1.5">
                {LANGUAGE_PAIRS.map((pair, i) => (
                  ...
                  >
                    ...
                    <p className="text-[10px] text-slate-500 dark:text-slate-400 mt-0.5 leading-tight">{pair.label}</p>
                  </button>
                ))}
              </div>
            </div>
          ...
          <div className="flex items-center gap-3">
            <label className="flex items-center gap-2 cursor-pointer">
              ...
              />
              <span className="text-sm text-slate-700 dark:text-slate-300">부분 번역 (특정 섹션만 번역)</span>
            </label>
          </div>

          {partialMode && (
            <div className="p-3 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-200 dark:border-slate-700">
              <p className="text-xs text-slate-500 dark:text-slate-400 mb-2">번역할 섹션을 선택하세요:</p>
          ...
          <FeatureGate feature="translation">
            ...
            >
              {loading ? 'AI 번역 중...' : `${selectedPair.fromFlag} ${selectedPair.toFlag} 번역 시작`}
            </button>
          </FeatureGate>
        </div>
            ...
            <div className="flex items-center justify-between mb-1.5">
              ...
              <div className="flex items-center gap-2">
                {confidenceScore !== null && (
                  <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium ${getConfidenceColor(confidenceScore)}`}>
            ...
            <div className="imp-card p-5 min-h-[400px] max-h-[600px] overflow-y-auto">
              {loading ? (
                <div className="flex flex-col items-center justify-center h-64 gap-3">
                  ...
                  <p className="text-xs text-slate-400 dark:text-slate-500">예상 소요 시간: 15~30초</p>
                </div>
              ) : result ? (
