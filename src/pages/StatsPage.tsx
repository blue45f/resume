...
function JobStatsSection() {
  ...
  const TYPE_LABELS: Record<string, string> = { full_time: '정규직', contract: '계약직', intern: '인턴', part_time: '파트타임', freelance: '프리랜서' };
    ...
    <div className="mt-8 space-y-6">
      ...
      <div className="flex flex-wrap gap-2 imp-card p-3">
        ...
        {(filterLocation || filterType || filterSkill) && (
          <button onClick={() => { setFilterLocation(''); setFilterType(''); setFilterSkill(''); }} className="px-3 py-1.5 text-xs text-slate-500 hover:text-slate-700 dark:hover:text-slate-300">
            초기화
          </button>
        )}
      </div>
              ...
              <div key={c.name} className="flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="h-1.5 bg-slate-100 dark:bg-slate-700 rounded-full overflow-hidden">
                    <div className="h-full bg-gradient-to-r from-amber-400 to-orange-500 rounded-full" style={{ width: `${(c.count / (data.byCompany[0]?.count || 1)) * 100}%` }} />
                  </div>
                </div>
              </div>
            ))}
            {data.byCompany.length === 0 && <p className="text-sm text-slate-400 text-center py-4">데이터 없음</p>}
          </div>
        </div>
            ...
            {data.byLocation.map((l, i) => (
              <div key={l.name} className="flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="h-1.5 bg-slate-100 dark:bg-slate-700 rounded-full overflow-hidden">
                    <div className="h-full bg-gradient-to-r from-blue-400 to-indigo-500 rounded-full" style={{ width: `${(l.count / (data.byLocation[0]?.count || 1)) * 100}%` }} />
                  </div>
                </div>
              </div>
            ))}
            {data.byLocation.length === 0 && <p className="text-sm text-slate-400 text-center py-4">데이터 없음</p>}
          </div>
        </div>
            ...
            {data.bySkill.map((s, i) => {
              ...
              return (
                <span
                  ...
                  style={{ opacity }}
                >
                  <span className="font-medium text-purple-700 dark:text-purple-300">{s.skill || s.name}</span>
                  <span className="text-xs text-purple-400">{s.count}</span>
                </span>
              );
            })}
            {data.bySkill.length === 0 && <p className="text-sm text-slate-400 py-4">데이터 없음</p>}
          </div>
        </div>
      </div>
    </div>
...
export default function StatsPage() {
  ...
  const [popular, setPopular] = useState<PopularResume[]>([]);
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetch(`${API_URL}/api/health/stats`).then(r => r.ok ? r.json() : null),
      fetch(`${API_URL}/api/resumes?visibility=public&limit=10&sort=viewCount`).then(r => r.ok ? r.json() : { data: [] }).catch(() => ({ data: [] })),
      fetch(`${API_URL}/api/resumes/popular-skills`).then(r => r.ok ? r.json() : []).catch(() => []),
    ]).then(([s, p, sk]) => {
      if (s) setStats(s);
      setPopular((p.data || p.items || []).slice(0, 10));
      setSkills(Array.isArray(sk) ? sk.slice(0, 15) : []);
  ...
  return (
    <>
      <Header />
      <main id="main-content" className="flex-1 max-w-5xl mx-auto w-full px-4 sm:px-6 py-8" role="main">
        <div className="flex items-center gap-3 mb-8">
          ...
          <div>
            <h1 className="text-xl sm:text-2xl font-bold text-slate-900 dark:text-slate-100">이력서공방 통계</h1>
            <p className="text-sm text-slate-500 dark:text-slate-400">플랫폼 전체 현황을 한눈에 확인하세요</p>
          </div>
        </div>
        ...
        {stats && (
          <>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
              {[
                { label: '전체 회원', value: stats.users.total, sub: `오늘 +${stats.users.today}`, color: 'from-blue-500 to-indigo-600', icon: '👥' },
                { label: '이력서', value: stats.resumes.total, sub: `공개 ${stats.resumes.public}개`, color: 'from-emerald-500 to-teal-600', icon: '📄' },
                ...
                <div key={s.label} className="relative overflow-hidden imp-card p-5">
                  ...
                  {s.sub && <p className="text-[10px] text-slate-400 dark:text-slate-500 mt-1">{s.sub}</p>}
                </div>
              ))}
            </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-8">
              {[
                ...
                <div key={s.label} className="bg-slate-50 dark:bg-slate-800/50 rounded-xl p-4 text-center border border-slate-100 dark:border-slate-700/50">
                  <span className="text-lg">{s.icon}</span>
                  <p className="text-lg font-bold text-slate-800 dark:text-slate-200 mt-1"><CountUp target={s.value} /></p>
            ...
            {(() => {
              const filtered = resumeFilter.trim()
                ? popular.filter(r => {
                    ...
                    const skillText = r.skills?.map(s => s.items).join(',').toLowerCase() || '';
                    return r.title.toLowerCase().includes(q) || skillText.includes(q) || (r.personalInfo?.name || '').toLowerCase().includes(q);
                  })
                : popular;
              return filtered.length === 0 ? (
              <p className="text-sm text-slate-400 text-center py-6">{resumeFilter ? '필터 결과 없음' : '데이터가 없습니다'}</p>
            ) : (
              <div className="space-y-2">
                {filtered.map((r, i) => (
                  <Link
                  ...
                  >
                    <span className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold shrink-0 ${
                      i < 3 ? 'bg-gradient-to-br from-amber-400 to-orange-500 text-white' : 'bg-slate-100 dark:bg-slate-700 text-slate-500 dark:text-slate-400'
                    }`}>
                      {i + 1}
                    </span>
                    <div className="flex-1 min-w-0">
                ...
                {skills.map((s, i) => {
                  ...
                  return (
                    <div key={s.skill} className="flex items-center gap-3">
                      <span className={`w-6 text-xs font-bold text-right shrink-0 ${i < 3 ? 'text-emerald-600 dark:text-emerald-400' : 'text-slate-400'}`}>
                        {i + 1}
                      </span>
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between mb-0.5">
