...
const TYPE_CONFIG = {
  bug: { label: '버그 신고', color: 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400', icon: '🐛' },
  feature: { label: '기능 제안', color: 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400', icon: '💡' },
  opinion: { label: '의견', color: 'bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-400', icon: '💬' },
  question: { label: '질문', color: 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400', icon: '❓' },
};

const STATUS_CONFIG = {
  open: { label: '접수', color: 'bg-slate-100 text-slate-600 dark:bg-slate-700 dark:text-slate-300' },
  'in-progress': { label: '처리 중', color: 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400' },
  resolved: { label: '해결됨', color: 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400' },
    ...
    <>
      ...
      <main id="main-content" className="flex-1 max-w-4xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-6 sm:py-8" role="main">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-xl sm:text-2xl font-bold text-slate-900 dark:text-slate-100">피드백 게시판</h1>
            <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">버그 신고, 기능 제안, 의견을 남겨주세요</p>
          </div>
          <button
        ...
        <div className="flex gap-1.5 mb-4 overflow-x-auto py-1">
          <button onClick={() => setFilterType('all')} className={`px-3 py-1.5 text-xs rounded-full whitespace-nowrap transition-colors ${filterType === 'all' ? 'bg-slate-900 dark:bg-slate-100 text-white dark:text-slate-900 font-medium' : 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400'}`}>
            전체 ({items.length})
          </button>
          {(Object.entries(TYPE_CONFIG) as [string, any][]).map(([key, cfg]) => (
            <button key={key} onClick={() => setFilterType(key)} className={`px-3 py-1.5 text-xs rounded-full whitespace-nowrap transition-colors ${filterType === key ? cfg.color + ' font-medium' : 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400'}`}>
        ...
        {loading ? (
          ...
          <div className="text-center py-16 animate-fade-in">
            ...
            <p className="text-xs text-slate-400 dark:text-slate-500">첫 번째 피드백을 남겨주세요!</p>
          </div>
        ) : (
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1 flex-wrap">
                        <span className={`px-2 py-0.5 text-[10px] font-medium rounded-full ${typeCfg.color}`}>{typeCfg.icon} {typeCfg.label}</span>
                        <span className={`px-2 py-0.5 text-[10px] font-medium rounded-full ${statusCfg.color}`}>{statusCfg.label}</span>
                      </div>
                      <h3 className="text-sm font-semibold text-slate-900 dark:text-slate-100 mb-1">{item.title}</h3>
