    ...
    <>
      ...
      <main id="main-content" className="flex-1 max-w-3xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-6 sm:py-8" role="main">
        ...
        <div className="flex gap-1 mb-4 p-1 bg-slate-100 dark:bg-slate-800 rounded-xl w-fit">
          <button
            onClick={() => { setTab('followers'); setSearch(''); }}
          ...
          <button
            onClick={() => { setTab('following'); setSearch(''); }}
            className={`px-4 py-2 text-sm rounded-lg transition-colors ${
              tab === 'following'
                ? 'bg-white dark:bg-slate-700 text-slate-900 dark:text-slate-100 shadow-sm font-medium'
                : 'text-slate-500 dark:text-slate-400 hover:text-slate-700'
            ...
            {search.trim() ? (
              ...
              <>
                <p className="text-sm text-slate-400 dark:text-slate-500">
                  {tab === 'followers' ? '아직 팔로워가 없습니다' : '아직 팔로잉하는 사용자가 없습니다'}
                </p>
                <p className="text-xs text-slate-400 mt-1">탐색 페이지에서 다른 사용자를 찾아보세요</p>
                <Link to="/explore" className="inline-block mt-3 px-4 py-2 text-sm bg-blue-600 text-white rounded-xl hover:bg-blue-700 transition-colors">
                  탐색하기
                </Link>
              </>
            )}
