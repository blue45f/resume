  ...
  return (
    <>
      ...
      <main id="main-content" className="flex-1 max-w-2xl mx-auto w-full px-4 sm:px-6 py-6 sm:py-8" role="main">
        <Link to="/pricing" className="text-sm text-slate-500 dark:text-slate-400 hover:text-slate-700 mb-4 inline-block">&larr; 요금제로 돌아가기</Link>
        ...
        <div className="grid grid-cols-1 gap-6">
          ...
          <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-6">
            <h2 className="text-sm font-semibold text-slate-700 dark:text-slate-300 mb-3">주문 요약</h2>
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-2">
                <span className="text-lg">{plan.badge}</span>
                <span className="font-medium text-slate-900 dark:text-slate-100">{plan.name} 플랜</span>
              </div>
            ...
            {period === 'yearly' && (
              <p className="text-xs text-green-600 dark:text-green-400 mb-2">연간 결제 17% 할인 적용</p>
            )}
            <div className="border-t border-slate-100 dark:border-slate-700 pt-3 mt-3">
              <div className="flex justify-between items-center">
                ...
                <span className="text-2xl font-bold text-slate-900 dark:text-slate-100">{formatPrice(amount)}</span>
              </div>
              {period === 'yearly' && (
                <p className="text-xs text-slate-400 text-right mt-0.5">월 {formatPrice(Math.round(amount / 12))}</p>
              )}
            </div>
          </div>
          ...
          {/* Payment Methods */}
          <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-6">
            <h2 className="text-sm font-semibold text-slate-700 dark:text-slate-300 mb-3">결제 수단</h2>
          ...
          <div>
            <label className="flex items-start gap-2 mb-4">
              ...
              <span className="text-xs text-slate-500 dark:text-slate-400 leading-relaxed">
                결제 진행 시 <Link to="/terms" className="text-blue-600 underline">이용약관</Link> 및
                자동 갱신에 동의합니다. 구독은 언제든 설정에서 취소할 수 있습니다.
              </span>
            </label>
