...
다크 모드는 디자인 토큰(`packages/client/src/index.css`)으로 구현됩니다. `:root` 가 라이트
램프를, `.dark` 클래스가 다크 램프를 정의하며, 컴포넌트는 `var(--color-*)` 만 참조하므로
`documentElement` 의 `.dark` 클래스 유무 하나로 전체 테마가 전환됩니다.

**계약 (단일 소스: `src/lib/theme.ts`)**

- 저장: `localStorage['resume-theme']` — 값은 `light` | `dark` | `system` (기본 `system`)
- 적용: `applyTheme()` 가 `system` 을 `prefers-color-scheme` 로 해석해 `.dark` 클래스를 토글
- 런타임 진입점: `main.tsx` 의 `initTheme()` — 최초 적용 + OS 테마 변경 구독
- UI: `Header` 의 테마 버튼이 light → dark → system 순환, `CommandPalette` 에서도 토글

**FOUC(흰 화면 깜빡임) 방지** — `packages/client/index.html` `<head>` 의 인라인 스크립트가
번들 실행 **이전, 페인트 전에** `.dark` 클래스를 결정합니다. 이 스크립트는 `lib/theme.ts` 의
`applyTheme` 와 **동일한 판정식**을 재현하므로 둘이 어긋나면 다크 사용자가 깜빡임을 겪습니다.
계약은 `src/lib/theme.test.ts` 의 "No-FOUC inline-script parity" 케이스가 고정합니다 —
`theme.ts` 의 판정 로직을 바꾸면 인라인 스크립트(`index.html`)도 함께 수정하세요.

> 주의: 이 저장소는 동시 디자인 세션 + 백그라운드 자동 커밋 sync 가 돌 수 있습니다. 테마
> 작업은 토큰 블록(`:root`/`.dark`)과 위 두 진입점에 국한된 가산적 변경으로 유지하고,
> 기존 화면 스타일은 건드리지 마세요.
